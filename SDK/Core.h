...
namespace SDK
{
	// Property offsets are stored directly on every supported build. Keep reads
	// behind one identity helper so reflection call sites use a single contract.
	...
	public:
		...
		Params GetParams() const
		{
			...
			if (VersionInfo.FortniteVersion >= 12.10)
				for (const UField* _Pr = GetChildProperties(); _Pr; _Pr = _Pr->FField_GetNext())
					p.NameOffsetMap.push_back({ ReadPropertyOffset(GetFromOffset<uint32>(_Pr, Offsets::Offset_Internal)), GetFromOffset<uint64>(_Pr, Offsets::PropertyFlags), GetFromOffset<uint32>(_Pr, Offsets::ElementSize) });
			else
		...
		ParamsNamed GetParamsNamed() const
		{
			...
			if (VersionInfo.FortniteVersion >= 12.10)
				for (const UField* _Pr = GetChildProperties(); _Pr; _Pr = _Pr->FField_GetNext())
					p.NameOffsetMap.push_back({ _Pr->FField_GetName().ToSDKString(), ReadPropertyOffset(GetFromOffset<uint32>(_Pr, Offsets::Offset_Internal)), GetFromOffset<uint64>(_Pr, Offsets::PropertyFlags), GetFromOffset<uint32>(_Pr, Offsets::ElementSize) });
			else
				for (const UField* _Pr = GetChildren(); _Pr; _Pr = _Pr->GetNext())
					p.NameOffsetMap.push_back({ _Pr->GetName().ToSDKString(), ReadPropertyOffset(GetFromOffset<uint32>(_Pr, Offsets::Offset_Internal)), GetFromOffset<uint64>(_Pr, Offsets::PropertyFlags), GetFromOffset<uint32>(_Pr, Offsets::ElementSize) });
