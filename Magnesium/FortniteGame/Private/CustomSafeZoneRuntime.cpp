...
namespace
{
    ...
    struct FCustomSafeZoneWorldState
    {
        ...
        std::vector<FCapturedManagedPhaseArray> CapturedManagedPhaseArrays;
    ...
    std::atomic<ECustomSafeZoneLegacyPhaseOwnerPath> GLegacyPhaseOwnerPath{
    ...
    void SetMatchRestartPreflightPending(UWorld* World, bool bPending)
    {
        if (bPending && World)
        {
            GMatchRestartPreflightGameMode.store((AFortGameMode*)World->AuthorityGameMode,
                std::memory_order_relaxed);
            GMatchRestartPreflightGameState.store((AFortGameStateAthena*)World->GameState,
                std::memory_order_relaxed);
            GMatchRestartPreflightWorld.store(World, std::memory_order_relaxed);
            GMatchRestartPreflightPending.store(true, std::memory_order_release);
            return;
        }

        GMatchRestartPreflightPending.store(false, std::memory_order_release);
        GMatchRestartPreflightWorld.store(nullptr, std::memory_order_relaxed);
        GMatchRestartPreflightGameMode.store(nullptr, std::memory_order_relaxed);
        GMatchRestartPreflightGameState.store(nullptr, std::memory_order_relaxed);
    }

    void PublishVisibleStatus(const FCustomSafeZoneRuntimeStatus& Status)
    {
        ...
        Visible.RuntimeStartPhase.store(Status.RuntimeStartPhase, std::memory_order_relaxed);
        Visible.NodeCount.store(Status.NodeCount, std::memory_order_relaxed);
        Visible.PhaseCapacity.store(Status.PhaseCapacity, std::memory_order_relaxed);
        Visible.CloseFinalCircle.store(Status.bCloseFinalCircle, std::memory_order_relaxed);
        Visible.OffendingEdge.store(Status.OffendingEdge, std::memory_order_relaxed);
        Visible.CanCorrectBeforeListen.store(Status.bCanCorrectBeforeListen,
            std::memory_order_relaxed);
    ...
    {
        return VersionInfo.FortniteVersion >= 0.f && VersionInfo.FortniteVersion < 31.f;
    }

    bool IsMovingConfigurationEnabled(
        const std::shared_ptr<const FCustomSafeZoneSequence>& Snapshot)
    {
        return FConfiguration::bLateGame.load(std::memory_order_acquire) &&
            FConfiguration::bCustomSafeZone.load(std::memory_order_acquire) &&
            Snapshot && Snapshot->bMovingZoneEnabled;
    }

    int32 EffectiveNodeCount(int32 AuthoredNodeCount, bool bCloseFinalCircle)
    {
    ...
    {
        return EffectiveNodeCount((int32)Sequence.Nodes.size(), Sequence.bCloseFinalCircle);
    }

    bool HasMovingTransition(const std::shared_ptr<const FCustomSafeZoneSequence>& Snapshot)
    {
        return Snapshot && EffectiveNodeCount(*Snapshot) >= 2;
    }

    int32 LogicalToRuntimeStartPhase(int32 LogicalPhase, double FortniteVersion)
    {
        return LogicalPhase + (FortniteVersion >= 24.f ? 3 : 0);
    }

    float ResolveAuthoredDuration(const std::optional<float>& Authored, float NativeDuration)
    {
        return Authored.value_or(NativeDuration);
    }

    int32 GeometryNodeIndexForPhase(int32 SourcePhase, int32 NodeCount, int32 Phase)
    {
    ...
    int32 TimingSourceNodeIndexForPhase(int32 RuntimeStartPhase, int32 NodeCount, int32 Phase)
    {
        const int32 EdgeIndex = Phase - RuntimeStartPhase;
        if (NodeCount < 2 || EdgeIndex < 0 || EdgeIndex >= NodeCount - 1)
            return -1;

        return EdgeIndex;
    }

    bool HasCapacityForPlan(int32 RuntimeStartPhase, int32 NodeCount, int32 PhaseCapacity)
    {
        if (RuntimeStartPhase < 1 || NodeCount < 2 || PhaseCapacity <= 0)
        {
            return false;
        }

        const int64 LastDistinctPhase = (int64)RuntimeStartPhase + NodeCount - 2;
        return LastDistinctPhase >= 0 && LastDistinctPhase < PhaseCapacity;
    }

    int32 ResolvePhaseCapacity(AFortAthenaMapInfo* MapInfo, int32 Hint)
    {
        if (Hint > 0 && Hint <= 64)
            return Hint;
        ...
        {
            const int32 DefinitionCount = MapInfo->SafeZoneDefinitions.Num();
        ...
        if (!MapInfo->HasSafeZoneDefinition() || !FFortSafeZoneDefinition::HasCount())
        {
            return 0;
        }

        const float EvaluatedCount = MapInfo->SafeZoneDefinition.Count.Evaluate();
        const float RoundedCount = std::round(EvaluatedCount);
        if (!std::isfinite(EvaluatedCount) || std::abs(EvaluatedCount - RoundedCount) > 0.01f ||
    ...
    void ResetStateForWorld(UWorld* World)
    {
        ...
        GCustomSafeZoneWorldState.Status.Failure = ECustomSafeZoneRuntimeFailure::NotRequested;
        PublishVisibleStatus(GCustomSafeZoneWorldState.Status);
    }

    void ResetStateForCacheKey(AFortAthenaMapInfo* MapInfo,
        std::shared_ptr<const FCustomSafeZoneSequence> Snapshot, int32 RuntimeStartPhase,
        int32 PhaseCapacity)
    {
        auto& State = GCustomSafeZoneWorldState;
        if (State.MapInfo == MapInfo && State.Snapshot == Snapshot &&
            State.CacheStartPhase == RuntimeStartPhase && State.CachePhaseCapacity == PhaseCapacity)
        {
            return;
        }
        ...
        State.Status.Failure = ECustomSafeZoneRuntimeFailure::NotRequested;
    ...
    void SetFailure(ECustomSafeZoneRuntimeFailure Failure, int32 RuntimeStartPhase, int32 NodeCount,
        int32 PhaseCapacity, const char* Detail, int32 OffendingEdge = -1)
    {
        auto& State = GCustomSafeZoneWorldState;
        if (State.bLegacyLocationsCommitted && State.Plan &&
            !State.Plan->Nodes.empty() && State.GameMode && State.GameMode->HasSafeZoneLocations())
        {
            auto& Locations = State.GameMode->SafeZoneLocations;
            const int32 Count = Locations.Num();
            const int32 VectorSize = FVector::Size();
            const int32 Begin = (std::max)(0, State.LegacyCommittedStartPhase);
            const int32 End = (std::min)(Count, State.LegacyCommittedLocationCount);
            if (Begin < End && VectorSize > 0 && VectorSize <= 0x40 &&
                Locations.Max() >= Count && Locations.Max() <= 128 && SDK::MemReadable(
                    Locations.GetData(), (size_t)Count * VectorSize))
            {
                const FVector FirstCenter(State.Plan->Nodes.front().Center);
                ...
                SDK::DbgLog("[CustomSafeZonePlan] rolled back legacy locations "
        ...
        State.Status.bCloseFinalCircle = State.Snapshot && State.Snapshot->bCloseFinalCircle;
        State.Status.OffendingEdge = OffendingEdge >= 0 && OffendingEdge + 1 < NodeCount
                ? OffendingEdge : -1;
        ...
        if (!State.bFailureLogged)
        {
            ...
            SDK::DbgLog("[CustomSafeZonePlan] status=failed code=%s "
                "version=%.2f start=%d nodes=%d capacity=%d detail=%s\n",
                CustomSafeZoneRuntime::GetFailureName(Failure), VersionInfo.FortniteVersion,
                RuntimeStartPhase, NodeCount, PhaseCapacity, Detail ? Detail : "none");
        }
    }
    ...
    {
        auto& State = GCustomSafeZoneWorldState;
        State.Status.bCanCorrectBeforeListen = bCanCorrectBeforeListen;
        PublishVisibleStatus(State.Status);
    }
    ...
    void MarkPhysicalWallPublished(const FCompiledCustomSafeZonePlan& Plan,
    ...
    std::shared_ptr<const FCompiledCustomSafeZonePlan> GetOrCompilePlan(UWorld* World,
        AFortAthenaMapInfo* MapInfo, int32 PhaseCapacityHint)
    {
        ResetStateForWorld(World);
        auto& State = GCustomSafeZoneWorldState;
        if (State.bPlanCommitted && State.Plan)
            return State.Plan;
        if (State.bRequiresPreflightBeforeApply && !State.bPreflightAccepted &&
            !State.bPreflightValidationInProgress)
        {
            return nullptr;
        }

        const bool bUseAcceptedPreflight = State.bPreflightAccepted && State.Snapshot;
        const int32 RuntimeStartPhase = bUseAcceptedPreflight ? State.CacheStartPhase
            : CustomSafeZoneRuntime::ResolveRuntimeStartPhase();
        auto Snapshot = bUseAcceptedPreflight ? State.Snapshot
            : FConfiguration::GetCustomSafeZoneSequenceSnapshot();
        const int32 PhaseCapacity = bUseAcceptedPreflight ? State.CachePhaseCapacity
            : ResolvePhaseCapacity(MapInfo, PhaseCapacityHint);
        if (!bUseAcceptedPreflight)
        {
            ResetStateForCacheKey(MapInfo, Snapshot, RuntimeStartPhase, PhaseCapacity);
        }
        ...
        if (!IsMovingConfigurationEnabled(Snapshot))
        {
            ...
            State.Status.Failure = ECustomSafeZoneRuntimeFailure::NotRequested;
            State.Status.RuntimeStartPhase = RuntimeStartPhase;
            State.Status.NodeCount = Snapshot ? (int32)Snapshot->Nodes.size() : 0;
            State.Status.PhaseCapacity = PhaseCapacity;
            State.Status.bCloseFinalCircle = Snapshot && Snapshot->bCloseFinalCircle;
            State.Status.OffendingEdge = -1;
            State.Status.bCanCorrectBeforeListen = false;
            PublishVisibleStatus(State.Status);
            return nullptr;
        ...
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::UnsupportedVersion, RuntimeStartPhase,
                Snapshot ? (int32)Snapshot->Nodes.size() : 0, PhaseCapacity,
        ...
        if (!World)
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::MissingWorld,
                RuntimeStartPhase, 0, 0, "world is null");
            return nullptr;
        }
        if (!MapInfo)
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::MissingMapInfo,
        ...
        if (!Snapshot)
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::MissingSnapshot, RuntimeStartPhase, 0, 0,
        ...
        if (CompiledNodeCount < 2)
        {
            ...
            State.Status.NodeCount = NodeCount;
            State.Status.PhaseCapacity = PhaseCapacity;
            State.Status.bCloseFinalCircle = Snapshot->bCloseFinalCircle;
            State.Status.OffendingEdge = -1;
            State.Status.bCanCorrectBeforeListen = false;
            PublishVisibleStatus(State.Status);
            return nullptr;
        }
        if (NodeCount > (int32)FCustomSafeZoneSequence::MaximumNodeCount)
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::TooManyNodes, RuntimeStartPhase, NodeCount, 0,
                "node count exceeds schema maximum");
            return nullptr;
        }
        const auto RadiusIncreaseEdge = Snapshot->FindFirstRadiusIncreaseEdge();
        if (RadiusIncreaseEdge.has_value())
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::RadiusIncreaseNotAllowed,
                RuntimeStartPhase, NodeCount, PhaseCapacity,
                "a circle is larger than its preceding circle", (int32)*RadiusIncreaseEdge);
            return nullptr;
        }

        if (!CustomSafeZoneRuntime::IsAuthoritativePhasePublisherAvailable())
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhasePublisher,
                RuntimeStartPhase, NodeCount, PhaseCapacity,
                "authoritative moving-zone owner hook is unavailable");
            return nullptr;
        }

        if (!HasCapacityForPlan(RuntimeStartPhase, CompiledNodeCount, PhaseCapacity))
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::InsufficientPhaseCapacity,
                RuntimeStartPhase, NodeCount, PhaseCapacity,
                "distinct authored nodes do not fit native phases");
            return nullptr;
        }

        auto MutablePlan = std::make_shared<FCompiledCustomSafeZonePlan>();
        ...
        MutablePlan->bCloseFinalCircle = Snapshot->bCloseFinalCircle;
        MutablePlan->Nodes.reserve((size_t)CompiledNodeCount);

        std::vector<FVector> ResolvedCenters;
        if (!GUI::TryResolveSafeZoneMapPoints(MapInfo, Snapshot->Nodes, ResolvedCenters) ||
            ResolvedCenters.size() != Snapshot->Nodes.size())
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::MapProjectionUnavailable,
                RuntimeStartPhase, NodeCount, PhaseCapacity,
            ...
            NodeIndex < Snapshot->Nodes.size(); ++NodeIndex)
        {
            ...
            Node.HoldBeforeNextSeconds = SourceNode.HoldBeforeNextSeconds;
            Node.MoveToNextSeconds = SourceNode.MoveToNextSeconds;
            const auto ValidDuration = [](const std::optional<float>& Value)
            {
                return !Value.has_value() || (std::isfinite(*Value) && *Value >= 0.f &&
                        *Value <= FCustomSafeZoneSequence::MaximumDurationSeconds);
            };
            if (!IsFiniteVector(Node.Center) || !std::isfinite(Node.Radius) || Node.Radius <
                    FCustomSafeZoneSequence::MinimumRadiusCm || Node.Radius >
                    FCustomSafeZoneSequence::MaximumRadiusCm ||
                !ValidDuration(Node.HoldBeforeNextSeconds) ||
                !ValidDuration(Node.MoveToNextSeconds))
            {
                SetFailure(ECustomSafeZoneRuntimeFailure::InvalidNode,
                    RuntimeStartPhase, NodeCount, PhaseCapacity,
        ...
        SDK::DbgLog("[CustomSafeZonePlan] status=ready version=%.2f "
            "start=%d source=%d authoredNodes=%d compiledNodes=%d "
            "closeFinal=%d capacity=%d\n", VersionInfo.FortniteVersion,
            State.Plan->RuntimeStartPhase, State.Plan->SourcePhase, State.Plan->AuthoredNodeCount,
            (int32)State.Plan->Nodes.size(), (int)State.Plan->bCloseFinalCircle,
            State.Plan->PhaseCapacity);
        return State.Plan;
    }

    bool ValidatePhaseArray(TArray<FFortSafeZonePhaseInfo>& Phases)
    {
        const int32 PhaseCount = Phases.Num();
        const int32 PhaseMax = Phases.Max();
        const int32 PhaseSize = FFortSafeZonePhaseInfo::Size();
        return FFortSafeZonePhaseInfo::HasCenter() && FFortSafeZonePhaseInfo::HasRadius() &&
            PhaseCount > 0 && PhaseCount <= 64 && PhaseMax >= PhaseCount && PhaseMax <= 128 &&
            PhaseSize > 0 && PhaseSize <= 0x400 && SDK::MemReadable(Phases.GetData(),
                (size_t)PhaseCount * PhaseSize);
    }

    bool IsLiveRuntimeObject(const UObject* Object)
    {
        if (!Object || !SDK::MemReadable(Object, sizeof(UObject)) || Object->Index < 0 ||
    ...
    bool IsOwnedByCurrentGameState(const UObject* Object, const AFortGameStateAthena* GameState)
    {
        ...
        ResolveLiveComponentPhaseLogicInternal(UWorld* World)
    {
        ...
        if (!IsLiveRuntimeObject(PhaseLogicClass))
            return nullptr;
        if (!UFortGameStateComponent_BattleRoyaleGamePhaseLogic::Get__Initialized ||
            !UFortGameStateComponent_BattleRoyaleGamePhaseLogic::Get__Ptr)
        {
            ...
            if (!GetFunction)
                return nullptr;
            UFortGameStateComponent_BattleRoyaleGamePhaseLogic::Get__Ptr = GetFunction;
            UFortGameStateComponent_BattleRoyaleGamePhaseLogic::Get__Initialized = true;
        }

        auto PhaseLogic = UFortGameStateComponent_BattleRoyaleGamePhaseLogic::Get(World);
        if (!IsLiveRuntimeObject(PhaseLogic) || PhaseLogic->IsDefaultObject() ||
            !IsOwnedByCurrentGameState(PhaseLogic, GameState))
        {
            PhaseLogic = IsLiveRuntimeObject(GameState)
                ? (UFortGameStateComponent_BattleRoyaleGamePhaseLogic*)
                    GameState->GetComponentByClass(PhaseLogicClass) : nullptr;
        }
        return IsLiveRuntimeObject(PhaseLogic) && !PhaseLogic->IsDefaultObject() &&
            IsOwnedByCurrentGameState(PhaseLogic, GameState) ? PhaseLogic : nullptr;
    }

    bool IsLiveCurrentIndicator(UWorld* World, AFortSafeZoneIndicator* Indicator)
    {
        ...
        auto PhaseLogic = ResolveLiveComponentPhaseLogicInternal(World);
        return PhaseLogic && PhaseLogic->HasSafeZoneIndicator() &&
            PhaseLogic->SafeZoneIndicator == Indicator;
    }

    bool CaptureManagedPhaseArray(AFortSafeZoneIndicator* Indicator,
        TArray<FFortSafeZonePhaseInfo>& Phases, bool bStableProcessArray)
    {
        if (!ValidatePhaseArray(Phases))
            return false;
        const bool bIndicatorOwned = Indicator && Indicator->HasSafeZonePhases() &&
            &Indicator->SafeZonePhases == &Phases;
        if (!bIndicatorOwned && !bStableProcessArray)
            return false;

        auto& State = GCustomSafeZoneWorldState;
        for (const auto& Captured : State.CapturedManagedPhaseArrays)
        {
            if (Captured.Array == &Phases && Captured.Data == Phases.GetData() &&
                Captured.Phases.size() == (size_t)Phases.Num() && Captured.bStableProcessArray ==
                    bStableProcessArray && (bStableProcessArray ||
        ...
        Captured.Owner = bIndicatorOwned ? TWeakObjectPtr<AFortSafeZoneIndicator>(Indicator)
            : TWeakObjectPtr<AFortSafeZoneIndicator>{};
        Captured.bStableProcessArray = bStableProcessArray;
        Captured.bHasWaitTime = FFortSafeZonePhaseInfo::HasWaitTime();
        Captured.bHasShrinkTime = FFortSafeZonePhaseInfo::HasShrinkTime();
        Captured.Phases.reserve((size_t)Phases.Num());
        for (int32 Index = 0; Index < Phases.Num(); ++Index)
        {
            const auto& Phase = Phases.Get(Index, FFortSafeZonePhaseInfo::Size());
        ...
        State.CapturedManagedPhaseArrays.push_back(std::move(Captured));
        return true;
    }
    ...
    void RestoreCapturedNativeSchedules(UWorld* World)
    {
        ...
        for (auto& Captured : State.CapturedManagedPhaseArrays)
        {
            if (!Captured.bStableProcessArray)
            {
                ...
                if (!IsLiveCurrentIndicator(World, Owner) || !Owner->HasSafeZonePhases() ||
                    &Owner->SafeZonePhases != Captured.Array)
                {
                    continue;
                }
            }
            if (!Captured.Array || !SDK::MemReadable(Captured.Array,
            ...
            if (Phases.GetData() != Captured.Data ||
                Phases.Num() != (int32)Captured.Phases.size() || !ValidatePhaseArray(Phases))
            {
                continue;
            }
            ...
            for (int32 Index = 0; Index < Phases.Num(); ++Index)
            {
                auto& Phase = Phases.Get(Index, FFortSafeZonePhaseInfo::Size());
            ...
            SDK::DbgLog("[CustomSafeZonePlan] restored native managed schedule phases=%d\n",
                Phases.Num());
        }

        if (State.bLegacyLocationBaselineCaptured && State.GameMode &&
            State.GameMode == World->AuthorityGameMode && State.GameMode->HasSafeZoneLocations())
        {
            auto& Locations = State.GameMode->SafeZoneLocations;
            const bool bCanReplace = Locations.Num() == 0 || Locations.IsValid();
            if (bCanReplace)
            {
                if (Locations.IsValid())
                    Locations.Free();
                for (const auto& Entry : State.LegacyLocationBaseline)
                {
                    FVector Center(Entry.X, Entry.Y, Entry.Z);
                    Locations.Add(Center, FVector::Size());
                }
                if (State.bLegacyHadInitializedProperty && State.GameMode->
                        HasbSafeZoneLocationsInitialized())
                {
                    State.GameMode->bSafeZoneLocationsInitialized = State.bLegacyWasInitialized;
                }
                SDK::DbgLog("[CustomSafeZonePlan] restored legacy center schedule locations=%d\n",
                    Locations.Num());
            }
        }
    }
    ...
    {
        return Plan.Nodes.size() > 1 && std::any_of(Plan.Nodes.begin(), Plan.Nodes.end() - 1,
    ...
    bool ValidatePhaseTimingFields(const FCompiledCustomSafeZonePlan& Plan)
    {
        return (!HasAuthoredHoldTiming(Plan) || FFortSafeZonePhaseInfo::HasWaitTime()) &&
            (!HasAuthoredMoveTiming(Plan) || FFortSafeZonePhaseInfo::HasShrinkTime());
    }

    bool ValidateNativeTimingFields(AFortSafeZoneIndicator* Indicator,
        const FCompiledCustomSafeZonePlan& Plan)
    {
        if (!HasAuthoredHoldTiming(Plan) && !HasAuthoredMoveTiming(Plan))
        {
            return true;
        }

        return Indicator && Indicator->HasSafeZoneStartShrinkTime() &&
            Indicator->HasSafeZoneFinishShrinkTime();
    }

    bool ValidateIndicatorForMovingPhase(AFortSafeZoneIndicator* Indicator)
    {
        if (!Indicator || !Indicator->HasNextCenter() || !Indicator->HasNextRadius())
        {
            return false;
        }

        const bool bHasSourcePair = (Indicator->HasPreviousCenter() &&
                Indicator->HasPreviousRadius()) || (Indicator->HasLastCenter() &&
                Indicator->HasLastRadius());
        return bHasSourcePair;
    }

    void MarkIndicatorDirty(AFortSafeZoneIndicator* Indicator, const wchar_t* PropertyName)
    {
        if (Indicator && PropertyName)
        {
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(Indicator, PropertyName);
        }
    }

    void PublishCompiledGeometry(AFortSafeZoneIndicator* Indicator,
        const FCompiledCustomSafeZonePlan& Plan, int32 ActivePhase)
    {
        ...
        const int32 LiveIndex = SourceIndex >= 0 ? SourceIndex : TargetIndex;
        const auto& Live = Plan.Nodes[LiveIndex];

        FVector MutableLiveCenter(Live.Center);
        const bool bUsedNativePhysicalSetter = Indicator->TrySetSafeZoneRadiusAndCenter(
                Live.Radius, MutableLiveCenter);
        if (!bUsedNativePhysicalSetter)
        {
            // The 1.x indicator has no native setter, so fall back to the actor transform plus reflected radii.
            Indicator->K2_SetActorLocation(MutableLiveCenter, false, nullptr, true);
        }
        ...
        if (FutureIndex >= 0)
        {
            ...
            if (Indicator->HasFutureReplicator() && Indicator->FutureReplicator)
            {
                ...
                {
                    Indicator->FutureReplicator->NextNextCenter = FVector(Future.Center);
                    bFutureGeometryWritten = true;
                    VersionFeatureAdapter::MarkReplicatedPropertyDirty(Indicator->FutureReplicator,
                        L"NextNextCenter");
                }
                if (Indicator->FutureReplicator->HasNextNextRadius())
                {
                    Indicator->FutureReplicator->NextNextRadius = float(Future.Radius);
                    bFutureGeometryWritten = true;
                    VersionFeatureAdapter::MarkReplicatedPropertyDirty(Indicator->FutureReplicator,
                        L"NextNextRadius");
                }
    ...
    void ApplyNativeTimingOverride(AFortSafeZoneIndicator* Indicator,
        const FCompiledCustomSafeZonePlan& Plan, int32 ActivePhase, float TimeSeconds)
    {
        const int32 TimingIndex = TimingSourceNodeIndexForPhase(Plan.RuntimeStartPhase,
                (int32)Plan.Nodes.size(), ActivePhase);
        if (TimingIndex < 0)
            return;

        const auto& Source = Plan.Nodes[TimingIndex];
        if (!Source.HoldBeforeNextSeconds.has_value() && !Source.MoveToNextSeconds.has_value())
        {
            return;
        }
        if (!Indicator->HasSafeZoneStartShrinkTime() || !Indicator->HasSafeZoneFinishShrinkTime() ||
            !std::isfinite(TimeSeconds))
        {
            return;
        }

        const float NativeWait = (std::max)(0.f, Indicator->SafeZoneStartShrinkTime - TimeSeconds);
        const float NativeMove = (std::max)(0.f, Indicator->SafeZoneFinishShrinkTime -
                Indicator->SafeZoneStartShrinkTime);
        const float Hold = ResolveAuthoredDuration(Source.HoldBeforeNextSeconds, NativeWait);
        const float Move = ResolveAuthoredDuration(Source.MoveToNextSeconds, NativeMove);
        Indicator->SafeZoneStartShrinkTime = TimeSeconds + Hold;
        Indicator->SafeZoneFinishShrinkTime = Indicator->SafeZoneStartShrinkTime + Move;
        MarkIndicatorDirty(Indicator, L"SafeZoneStartShrinkTime");
        MarkIndicatorDirty(Indicator, L"SafeZoneFinishShrinkTime");
    }
}
...
void CustomSafeZoneRuntime::ResetForMatch(UWorld* World)
{
    ...
    const auto RestartSnapshot = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
    const bool bNeedsRestartPreflight = IsMovingConfigurationEnabled(RestartSnapshot) &&
        HasMovingTransition(RestartSnapshot);
    FConfiguration::ReleaseCustomSafeZoneSequenceForMatch();
    GCustomSafeZoneWorldState = {};
    GCustomSafeZoneWorldState.World = World;
    GCustomSafeZoneWorldState.GameMode = World ? (AFortGameMode*)World->AuthorityGameMode : nullptr;
    GCustomSafeZoneWorldState.GameState = World ? (AFortGameStateAthena*)World->GameState : nullptr;
    GCustomSafeZoneWorldState.Status.World = World;
    GCustomSafeZoneWorldState.Status.Failure = ECustomSafeZoneRuntimeFailure::NotRequested;
    GCustomSafeZoneWorldState.bRequiresPreflightBeforeApply = bNeedsRestartPreflight;
    PublishVisibleStatus(GCustomSafeZoneWorldState.Status);
    SetMatchRestartPreflightPending(World, bNeedsRestartPreflight);

    SDK::DbgLog("[CustomSafeZonePlan] match state reset world=%p\n", (void*)World);
}

bool CustomSafeZoneRuntime::IsMatchRestartPreflightPending(UWorld* World)
{
    if (!World || !GMatchRestartPreflightPending.load(std::memory_order_acquire))
    {
        return false;
    }

    const bool bSameGeneration = GMatchRestartPreflightWorld.load(
            std::memory_order_relaxed) == World && GMatchRestartPreflightGameMode.load(
            std::memory_order_relaxed) == (AFortGameMode*)World->AuthorityGameMode &&
        GMatchRestartPreflightGameState.load(std::memory_order_relaxed) ==
...
void CustomSafeZoneRuntime::SetAuthoritativePhasePublisherAvailable(bool bAvailable)
{
    GAuthoritativePhasePublisherCapability.store(bAvailable ? 1 : 0, std::memory_order_release);
}

bool CustomSafeZoneRuntime::IsAuthoritativePhasePublisherCapabilityKnown()
{
    return GAuthoritativePhasePublisherCapability.load(std::memory_order_acquire) >= 0;
}
...
void CustomSafeZoneRuntime::SetLegacyPhaseOwnerPath(ECustomSafeZoneLegacyPhaseOwnerPath OwnerPath)
{
    GLegacyPhaseOwnerPath.store(OwnerPath, std::memory_order_release);
}

ECustomSafeZoneLegacyPhaseOwnerPath
    CustomSafeZoneRuntime::GetLegacyPhaseOwnerPath()
{
    return GLegacyPhaseOwnerPath.load(std::memory_order_acquire);
}

UFortGameStateComponent_BattleRoyaleGamePhaseLogic*
    CustomSafeZoneRuntime::ResolveLiveComponentPhaseLogic(UWorld* World)
{
...
bool CustomSafeZoneRuntime::IsMovingModeRequested()
{
    ...
    if (GCustomSafeZoneWorldState.bPlanCommitted && GCustomSafeZoneWorldState.Plan &&
        GCustomSafeZoneWorldState.World == World)
    {
        return true;
    }
    if (GCustomSafeZoneWorldState.bPreflightAccepted && GCustomSafeZoneWorldState.Snapshot &&
        GCustomSafeZoneWorldState.World == World)
    {
        return true;
    }
    auto Snapshot = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
    if (!IsMovingConfigurationEnabled(Snapshot))
        return false;
    ...
    if (!IsSupportedMovingZoneVersion())
    {
        ...
        if (!bLoggedUnsupportedVersion)
        {
            ...
            SDK::DbgLog("[CustomSafeZonePlan] status=ignored code=%s "
...
{
    const int32 ConfiguredStart = FConfiguration::LateGameZone.load(std::memory_order_acquire);
    return LogicalToRuntimeStartPhase(ConfiguredStart, VersionInfo.FortniteVersion);
}

int32 CustomSafeZoneRuntime::ResolveNativePhaseCapacity(AFortAthenaMapInfo* MapInfo)
{
    return ResolvePhaseCapacity(MapInfo, 0);
}

bool CustomSafeZoneRuntime::DoesSequenceFitPhaseCapacity(int32 NodeCount, int32 PhaseCapacity,
    bool bCloseFinalCircle)
{
    return HasCapacityForPlan(ResolveRuntimeStartPhase(),
        EffectiveNodeCount(NodeCount, bCloseFinalCircle), PhaseCapacity);
}

ECustomSafeZonePreflightResult
    CustomSafeZoneRuntime::PreflightForServerStart(UWorld* World, AFortGameMode* GameMode,
        AFortAthenaMapInfo* MapInfo, int32 PhaseCapacity, bool bTreatUnavailableAsInvalid,
        bool bCanCorrectBeforeListen)
{
    ...
    if (!IsMovingConfigurationEnabled(Snapshot) || !HasMovingTransition(Snapshot))
    {
    ...
    if (!World || !MapInfo)
    {
        ...
        {
            SetFailure(World ? ECustomSafeZoneRuntimeFailure::MissingMapInfo
                    : ECustomSafeZoneRuntimeFailure::MissingWorld, RuntimeStartPhase, NodeCount, 0,
                World ? "live Athena map info did not appear before timeout"
    ...
    if (PhaseCapacity <= 0)
    {
        ...
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhaseFields,
            RuntimeStartPhase, NodeCount, PhaseCapacity,
    ...
    const bool bNeedsMapProjection = std::any_of(Snapshot->Nodes.begin(), Snapshot->Nodes.end(),
        [](const FCustomSafeZoneNode& Node)
        {
            return Node.bHasNormalizedCenter;
        });
    if (bNeedsMapProjection && !GUI::IsCustomSafeZoneMapProjectionReady(MapInfo))
    {
        if (!bTreatUnavailableAsInvalid)
        {
            PublishPending();
    ...
    if (VersionInfo.FortniteVersion >= 25.20f)
    {
        ...
        bOwnerReady = PhaseLogic && PhaseLogic->Class && !PhaseLogic->IsDefaultObject();
        if (bOwnerReady && PhaseLogic->HasSafeZoneIndicator() && PhaseLogic->SafeZoneIndicator &&
            PhaseLogic->SafeZoneIndicator->Class)
        {
            IndicatorPreflight = PhaseLogic->SafeZoneIndicator;
        }
        if (!IndicatorPreflight && bOwnerReady && PhaseLogic->HasSafeZoneIndicatorClass())
        {
            const UClass* IndicatorClass = PhaseLogic->SafeZoneIndicatorClass.Get();
    ...
    else if (bOwnerReady)
    {
        ...
        {
            const UClass* IndicatorClass = GameMode->SafeZoneIndicatorClass.Get();
            if (IndicatorClass && IndicatorClass->Class)
            {
                IndicatorPreflight = (AFortSafeZoneIndicator*)
                    IndicatorClass->GetDefaultObj();
    ...
    if (!bOwnerReady || !IndicatorPreflight || !IndicatorPreflight->Class)
    {
        if (!bTreatUnavailableAsInvalid)
        {
            PublishPending();
        ...
        SetFailure(bOwnerReady ? ECustomSafeZoneRuntimeFailure::MissingIndicatorFields
                : ECustomSafeZoneRuntimeFailure::MissingPhasePublisher,
            RuntimeStartPhase, NodeCount, PhaseCapacity, bOwnerReady
    ...
    const auto ValidatedPlan = GetOrCompilePlan(World, MapInfo, PhaseCapacity);
    State.bPreflightValidationInProgress = false;
    auto CurrentSnapshot = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
    if (CurrentSnapshot != Snapshot || ResolveRuntimeStartPhase() != RuntimeStartPhase ||
        !IsMovingConfigurationEnabled(CurrentSnapshot))
    {
        ResetStateForCacheKey(MapInfo, CurrentSnapshot, ResolveRuntimeStartPhase(), PhaseCapacity);
        return ECustomSafeZonePreflightResult::Pending;
    }
    if (!ValidatedPlan)
    {
    ...
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingIndicatorFields,
            RuntimeStartPhase, NodeCount, PhaseCapacity,
    ...
    {
        // FN21.10-25.19 keeps the managed schedule in the GameMode fallback array; 25.20+ needs it on the indicator.
        const bool bRequiresIndicatorOwnedPhases = VersionInfo.FortniteVersion >= 25.20f;
        if ((bRequiresIndicatorOwnedPhases && !IndicatorPreflight->HasSafeZonePhases()) ||
            !FFortSafeZonePhaseInfo::HasCenter() || !FFortSafeZonePhaseInfo::HasRadius() ||
    ...
    else if (!ValidateNativeTimingFields(IndicatorPreflight, *ValidatedPlan))
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingIndicatorFields,
            RuntimeStartPhase, NodeCount, PhaseCapacity,
    ...
    CurrentSnapshot = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
    if (CurrentSnapshot != Snapshot || ResolveRuntimeStartPhase() != RuntimeStartPhase ||
        !IsMovingConfigurationEnabled(CurrentSnapshot) ||
        !FConfiguration::FreezeCustomSafeZoneSequenceForMatch(Snapshot))
    {
        ResetStateForCacheKey(MapInfo, CurrentSnapshot, ResolveRuntimeStartPhase(), PhaseCapacity);
        return ECustomSafeZonePreflightResult::Pending;
    }
...
bool CustomSafeZoneRuntime::ApplyToPhaseArray(UWorld* World, AFortAthenaMapInfo* MapInfo,
    AFortSafeZoneIndicator* Indicator, TArray<FFortSafeZonePhaseInfo>& Phases, const char* Source,
    bool bStableProcessArray)
{
    ...
    if (!Plan)
        return false;
    if (!HasCapacityForPlan(Plan->RuntimeStartPhase, (int32)Plan->Nodes.size(), Phases.Num()) ||
        !ValidatePhaseTimingFields(*Plan))
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhaseFields, Plan->RuntimeStartPhase,
    ...
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingIndicatorFields, Plan->RuntimeStartPhase,
            Plan->AuthoredNodeCount, Phases.Num(), "indicator lacks source/target geometry fields");
        return false;
    }
    if (!CaptureManagedPhaseArray(Indicator, Phases, bStableProcessArray))
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhaseFields, Plan->RuntimeStartPhase,
            Plan->AuthoredNodeCount, Phases.Num(),
    ...
    {
        const int32 GeometryIndex = GeometryNodeIndexForPhase(Plan->SourcePhase,
        ...
        if (TimingNode.HoldBeforeNextSeconds.has_value())
            PhaseInfo.WaitTime = float(*TimingNode.HoldBeforeNextSeconds);
        if (TimingNode.MoveToNextSeconds.has_value())
            PhaseInfo.ShrinkTime = float(*TimingNode.MoveToNextSeconds);
    }
    MarkPlanActivated(*Plan);
    ...
    if (!State.bPhaseArrayLogged)
    {
        ...
        SDK::DbgLog("[CustomSafeZonePlan] applied phase-array source=%s "
            "start=%d sourcePhase=%d nodes=%d phases=%d\n", Source ? Source : "unknown",
            Plan->RuntimeStartPhase, Plan->SourcePhase, (int32)Plan->Nodes.size(), PhaseCount);
    }
    return true;
}

bool CustomSafeZoneRuntime::ApplyToLegacyLocations(AFortGameMode* GameMode,
    AFortAthenaMapInfo* MapInfo, int32 RequiredLocationCount, bool bHasNativePhasePublisher)
{
    if (!IsMovingModeRequested())
        return false;
    auto World = UWorld::GetWorld();
    ...
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhasePublisher,
            ResolveRuntimeStartPhase(), 0, RequiredLocationCount,
            "native moving-zone phase publisher is unavailable");
        return false;
    }
    if (!GameMode || !GameMode->HasSafeZoneLocations() || RequiredLocationCount <= 0 ||
        RequiredLocationCount > 64 || FVector::Size() <= 0 || FVector::Size() > 0x40)
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::InvalidLegacyLocations,
            ResolveRuntimeStartPhase(), 0, RequiredLocationCount,
    ...
    const int32 OutputCount = (std::max)(ExistingCount, RequiredLocationCount);
    if (ExistingCount < 0 || OutputCount > 64 || Locations.Max() < ExistingCount ||
        Locations.Max() > 128 || (ExistingCount > 0 && !SDK::MemReadable(Locations.GetData(),
    ...
    AFortSafeZoneIndicator* IndicatorPreflight = GameMode->HasSafeZoneIndicator()
    ...
    if (IndicatorPreflight && (!ValidateIndicatorForMovingPhase(IndicatorPreflight) ||
         !ValidateNativeTimingFields(IndicatorPreflight, *Plan)))
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingIndicatorFields, Plan->RuntimeStartPhase,
            Plan->AuthoredNodeCount, OutputCount, "legacy indicator CDO lacks moving-zone fields");
        return false;
    }
    ...
    if (!GCustomSafeZoneWorldState.bLegacyLocationBaselineCaptured)
    {
        ...
        CaptureState.LegacyLocationBaseline.reserve((size_t)ExistingCount);
        for (int32 Index = 0; Index < ExistingCount; ++Index)
        {
            const auto& Center = Locations.Get(Index, FVector::Size());
            CaptureState.LegacyLocationBaseline.push_back({
                Center.X, Center.Y, Center.Z
            });
        }
        CaptureState.bLegacyHadInitializedProperty = GameMode->HasbSafeZoneLocationsInitialized();
        CaptureState.bLegacyWasInitialized = CaptureState.bLegacyHadInitializedProperty
                ? (bool)GameMode->bSafeZoneLocationsInitialized : false;
    ...
    for (int32 Index = 0; Index < OutputCount; ++Index)
        StagedLocations[Index] = FVector(Plan->Nodes.front().Center);
    for (int32 Index = 0; Index < ExistingCount; ++Index)
        StagedLocations[Index] = Locations.Get(Index, FVector::Size());
    for (int32 Phase = Plan->SourcePhase;
        Phase < OutputCount; ++Phase)
    {
        const int32 NodeIndex = GeometryNodeIndexForPhase(Plan->SourcePhase,
            (int32)Plan->Nodes.size(), Phase);
        StagedLocations[Phase] = FVector(Plan->Nodes[NodeIndex].Center);
    }
    ...
    if (!State.bLegacyLocationsLogged)
    {
        ...
        SDK::DbgLog("[CustomSafeZonePlan] applied legacy locations "
            "start=%d sourcePhase=%d nodes=%d locations=%d->%d\n",
            Plan->RuntimeStartPhase, Plan->SourcePhase, (int32)Plan->Nodes.size(), ExistingCount,
            Locations.Num());
    }
    return true;
}

bool CustomSafeZoneRuntime::ApplyNativePhase(AFortGameMode* GameMode, AFortAthenaMapInfo* MapInfo,
    int32 ActivePhase, float TimeSeconds, const char* Source, bool bHasNativePhasePublisher)
{
    if (!IsMovingModeRequested())
        return false;
    auto World = UWorld::GetWorld();
    ...
    if (PhaseCapacity <= 0 && GameMode && GameMode->HasSafeZoneLocations())
    {
    ...
    if (!bHasNativePhasePublisher)
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhasePublisher, Plan->RuntimeStartPhase,
    ...
    if (Indicator->HasSafeZonePhases() && Indicator->SafeZonePhases.IsValid() &&
        (!ValidatePhaseArray(Indicator->SafeZonePhases) || !HasCapacityForPlan(
                Plan->RuntimeStartPhase, (int32)Plan->Nodes.size(),
                Indicator->SafeZonePhases.Num()) || !ValidatePhaseTimingFields(*Plan)))
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhaseFields, Plan->RuntimeStartPhase,
            Plan->AuthoredNodeCount, Indicator->SafeZonePhases.Num(),
            "native phase array lacks moving-zone fields");
        return false;
    }
    if (Indicator->HasSafeZonePhases() && Indicator->SafeZonePhases.IsValid())
    {
        if (!ApplyToPhaseArray(World, MapInfo, Indicator, Indicator->SafeZonePhases, Source))
        {
            return false;
        }
    }
    ...
    ApplyNativeTimingOverride(Indicator, *Plan, ActivePhase, TimeSeconds);
    ...
    if (State.LastPublishedPhase != ActivePhase)
    {
        ...
        const int32 TargetIndex = GeometryNodeIndexForPhase(Plan->SourcePhase,
            (int32)Plan->Nodes.size(), ActivePhase);
        const auto& Target = Plan->Nodes[TargetIndex];
        SDK::DbgLog("[CustomSafeZonePlan] published native phase source=%s "
            "phase=%d node=%d center=(%.1f,%.1f,%.1f) radius=%.1f\n",
            Source ? Source : "unknown", ActivePhase, TargetIndex, Target.Center.X, Target.Center.Y,
            Target.Center.Z, Target.Radius);
    }
    return true;
}

bool CustomSafeZoneRuntime::PublishManagedPhase(UWorld* World, AFortAthenaMapInfo* MapInfo,
    AFortSafeZoneIndicator* Indicator, int32 ActivePhase, const char* Source,
    TArray<FFortSafeZonePhaseInfo>* ManagedPhases, bool bStableProcessArray)
{
    if (!IsMovingModeRequested())
        return false;
    ResetStateForWorld(World);
    auto IndicatorPhases = Indicator && Indicator->HasSafeZonePhases() &&
            Indicator->SafeZonePhases.IsValid() ? &Indicator->SafeZonePhases : nullptr;
    auto PhaseArray = ManagedPhases && ManagedPhases->IsValid() ? ManagedPhases : IndicatorPhases;
    const int32 PhaseCapacity = PhaseArray ? PhaseArray->Num() : ResolvePhaseCapacity(MapInfo, 0);
    auto Plan = GetOrCompilePlan(World, MapInfo, PhaseCapacity);
    if (!Plan)
        return false;
    if (!HasCapacityForPlan(Plan->RuntimeStartPhase, (int32)Plan->Nodes.size(), PhaseCapacity))
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::InsufficientPhaseCapacity,
            Plan->RuntimeStartPhase, Plan->AuthoredNodeCount, PhaseCapacity,
    ...
    if (!State.bActivated)
    {
        if (!PhaseArray || !ApplyToPhaseArray(World, MapInfo, Indicator, *PhaseArray, Source,
                bStableProcessArray))
        {
            SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhaseFields, Plan->RuntimeStartPhase,
                Plan->AuthoredNodeCount, PhaseCapacity,
    ...
    if (State.LastPublishedPhase != ActivePhase)
    {
        ...
        const int32 TargetIndex = GeometryNodeIndexForPhase(Plan->SourcePhase,
            (int32)Plan->Nodes.size(), ActivePhase);
        const auto& Target = Plan->Nodes[TargetIndex];
        SDK::DbgLog("[CustomSafeZonePlan] published managed phase source=%s "
            "phase=%d node=%d center=(%.1f,%.1f,%.1f) radius=%.1f\n",
            Source ? Source : "unknown", ActivePhase, TargetIndex, Target.Center.X, Target.Center.Y,
            Target.Center.Z, Target.Radius);
    }
...
bool CustomSafeZoneRuntime::TryGetSourceCircle(UWorld* World, AFortAthenaMapInfo* MapInfo,
    FVector& OutCenter, float& OutRadius, int32 PhaseCapacityHint, bool bHasNativePhasePublisher)
{
    OutCenter = {};
    OutRadius = 0.f;
    if (!IsMovingModeRequested())
    ...
    auto Plan = GetOrCompilePlan(World, MapInfo, ResolvePhaseCapacity(MapInfo, PhaseCapacityHint));
    if (!Plan || Plan->Nodes.empty())
        return false;
    if (VersionInfo.FortniteVersion < 21.10f && !bHasNativePhasePublisher)
    {
        SetFailure(ECustomSafeZoneRuntimeFailure::MissingPhasePublisher, Plan->RuntimeStartPhase,
...
bool CustomSafeZoneRuntime::IsMovingPlanActive(UWorld* World, AFortSafeZoneIndicator* Indicator)
{
    const auto& State = GCustomSafeZoneWorldState;
    return World && State.World == World && State.Plan &&
        State.bActivated && State.bPhysicalPublished && State.PublishedIndicator &&
...
FCustomSafeZoneRuntimeStatus CustomSafeZoneRuntime::GetStatus()
{
    ...
    for (;;)
    {
        const uint64 Before = Visible.Sequence.load(std::memory_order_acquire);
        if (Before & 1)
            continue;

        FCustomSafeZoneRuntimeStatus Status;
        Status.Failure = Visible.Failure.load(std::memory_order_relaxed);
        Status.World = Visible.World.load(std::memory_order_relaxed);
        Status.RuntimeStartPhase = Visible.RuntimeStartPhase.load(std::memory_order_relaxed);
        Status.NodeCount = Visible.NodeCount.load(std::memory_order_relaxed);
        Status.PhaseCapacity = Visible.PhaseCapacity.load(std::memory_order_relaxed);
        Status.bCloseFinalCircle = Visible.CloseFinalCircle.load(std::memory_order_relaxed);
        Status.OffendingEdge = Visible.OffendingEdge.load(std::memory_order_relaxed);
        Status.bCanCorrectBeforeListen = Visible.CanCorrectBeforeListen.load(
                std::memory_order_relaxed);

        const uint64 After = Visible.Sequence.load(std::memory_order_acquire);
        if (Before == After)
            return Status;
    }
}

const char* CustomSafeZoneRuntime::GetFailureName(ECustomSafeZoneRuntimeFailure Failure)
{
    switch (Failure)
    {
...
void CustomSafeZoneRuntime::RunMappingSelfTests()
{
    ...
    assert(HasCapacityForPlan(Start, EffectiveNodeCount(Nodes, false), 6));
    assert(!HasCapacityForPlan(Start, EffectiveNodeCount(Nodes, true), 6));
    assert(HasCapacityForPlan(Start, EffectiveNodeCount(Nodes, true), 7));
    ...
    EdgeStatus.Failure = ECustomSafeZoneRuntimeFailure::RadiusIncreaseNotAllowed;
    ...
    assert(TimingSourceNodeIndexForPhase(Start, (int32)ClosingPlan.Nodes.size(), Start) == 0);
    assert(GeometryNodeIndexForPhase(Source, (int32)ClosingPlan.Nodes.size(), Source) == 0);
    assert(GeometryNodeIndexForPhase(Source, (int32)ClosingPlan.Nodes.size(), Start) == 1);
    assert(ClosingPlan.Nodes.back().Radius == 0.f);
    assert(ClosingPlan.Nodes.back().Center.X == ClosingPlan.Nodes.front().Center.X);

    const auto OriginalSnapshot = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
    const FCustomSafeZoneSequence OriginalSequence = OriginalSnapshot ? *OriginalSnapshot
        : FCustomSafeZoneSequence{};
    const bool bOriginalMoving = OriginalSnapshot && OriginalSnapshot->bMovingZoneEnabled;
    const bool bOriginalCustomSafeZone = FConfiguration::bCustomSafeZone.load(
    ...
    assert(FConfiguration::PublishCustomSafeZoneSequence(PublishedSequence, true));
    auto FirstPublication = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
    ...
    assert(!FConfiguration::PublishCustomSafeZoneSequence(IncreasingSequence, true));
    assert(FConfiguration::GetCustomSafeZoneSequenceSnapshot() == FirstPublication);
    FCustomSafeZoneSequence EqualRadiusSequence = *FirstPublication;
    EqualRadiusSequence.Nodes[1].RadiusCm = EqualRadiusSequence.Nodes[0].RadiusCm;
    assert(FConfiguration::PublishCustomSafeZoneSequence(EqualRadiusSequence, true));
    ...
    assert(FConfiguration::PublishCustomSafeZoneSequence(NewerSequence, true));
    assert(!FConfiguration::PublishCustomSafeZoneSequenceIfCurrent(
        FirstPublication, PublishedSequence, true));
    auto AcceptedPublication = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
    assert(FConfiguration::FreezeCustomSafeZoneSequenceForMatch(AcceptedPublication));
    assert(!FConfiguration::SetCustomSafeZoneEnabled(false));
    assert(!FConfiguration::PublishCustomSafeZoneSequence(PublishedSequence, true));
    assert(FConfiguration::GetCustomSafeZoneSequenceSnapshot() == AcceptedPublication);

    FConfiguration::ReleaseCustomSafeZoneSequenceForMatch();
    FCustomSafeZoneSequence EmptySequence;
    EmptySequence.Nodes.clear();
    assert(!FConfiguration::PublishCustomSafeZoneSequence(EmptySequence, true));
    ...
    assert(FConfiguration::PublishCustomSafeZoneSequence(ConcurrentBaseline, true));
    ...
    auto PublishRange = [&](int Begin, int End)
    {
        ...
        for (int Marker = Begin; Marker < End; ++Marker)
        {
            ...
            if (!FConfiguration::PublishCustomSafeZoneSequence(std::move(Candidate), true))
            {
                ConcurrentSnapshotFailed.store(true, std::memory_order_release);
                return;
            }
        }
    };
    ...
    std::thread Reader([&]()
    {
        ...
        {
            const auto View = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
            ...
            if (fabsf(View->Nodes[0].RadiusCm - (20000.f + Marker)) > 0.01f ||
                fabsf((float)View->Nodes[1].Center.X + Marker) > 0.01f ||
                fabsf(View->Nodes[1].RadiusCm - (10000.f + Marker)) > 0.01f)
            {
                ConcurrentSnapshotFailed.store(true, std::memory_order_release);
                break;
            }
    ...
    WriterB.join();
    Reader.join();
    assert(!ConcurrentSnapshotFailed.load(std::memory_order_acquire));

    for (int Iteration = 0; Iteration < 32; ++Iteration)
    {
        FConfiguration::ReleaseCustomSafeZoneSequenceForMatch();
        assert(FConfiguration::SetCustomSafeZoneEnabled(true));
        const auto RaceSnapshot = FConfiguration::GetCustomSafeZoneSequenceSnapshot();
        ...
        std::thread Freezer([&]()
        {
            while (!BeginFreezeRace.load(std::memory_order_acquire))
                std::this_thread::yield();
            bFreezeWon = FConfiguration::FreezeCustomSafeZoneSequenceForMatch(RaceSnapshot);
        });
        std::thread Disabler([&]()
        {
            while (!BeginFreezeRace.load(std::memory_order_acquire))
                std::this_thread::yield();
            bDisableWon = FConfiguration::SetCustomSafeZoneEnabled(false);
        });
    ...
    FConfiguration::ReleaseCustomSafeZoneSequenceForMatch();
    assert(FConfiguration::PublishCustomSafeZoneSequence(OriginalSequence, bOriginalMoving));
    assert(FConfiguration::SetCustomSafeZoneEnabled(bOriginalCustomSafeZone));
}
#endif
