...
int GetLevel(const FDataTableCategoryHandle& CategoryHandle)
{
    ...
    for (auto& LootLevelDataPair : (TMap<FName, FFortLootLevelData*>)CategoryHandle.DataTable->RowMap)
    {
        ...
        if ((FFortLootLevelData::HasCategory() ? LootLevelDataPair.Value()->Category : LootLevelDataPair.Value()->category) != CategoryHandle.RowContents ||
            LootLevelDataPair.Value()->LootLevel > GameState->WorldLevel ||
...
static int32 ResolveLootLevel(const UFortItemDefinition* Definition)
{
    if (!Definition->IsA(UFortWorldItemDefinition::StaticClass()) ||
        !Definition->HasLootLevelData())
    {
        return 0;
    }

    const int32 MaxLevel = Definition->MaxLevel > 0 ? Definition->MaxLevel : 99999;
    return std::clamp(GetLevel(Definition->LootLevelData), Definition->MinLevel, MaxLevel);
}
...
void UFortLootPackage::SetupLDSForPackage(TArray<FFortItemEntry*>& LootDrops, SDK::FName Package,
    int i, FName TierGroup, int WorldLevel, ABuildingContainer* Container)
{
    ...
    if (LootPackage->LootPackageCall.Num() > 1)
    {
        for (int i = 0; i < LootPackage->Count; i++)
            SetupLDSForPackage(LootDrops,
                UKismetStringLibrary::Conv_StringToName(LootPackage->LootPackageCall), 0, TierGroup,
                WorldLevel);
            ...
            if (auto AmmoDefinition = AmmoDef->Cast<UFortAmmoItemDefinition>())
            {
                ...
                if (FinalCount > 0.f)
                    AmmoEntry = AFortInventory::MakeItemEntry(AmmoDefinition, (int)FinalCount,
                        ResolveLootLevel(AmmoDefinition));
    ...
    {
        if ( LootDrop->ItemDefinition == ItemDefinition && LootDrop->Count < ItemDefinition->GetMaxStackSize())
        {
            ...
            if (LootDrop->Count > ItemDefinition->GetMaxStackSize())
            {
                ...
                LootDrops.Add(AFortInventory::MakeItemEntry(ItemDefinition,
                    OGCount - (int32)ItemDefinition->GetMaxStackSize(),
                    ResolveLootLevel(ItemDefinition)));
            }
        ...
        if (AmmoEntry && AmmoEntry->ItemDefinition && LootDrop->ItemDefinition == AmmoEntry->ItemDefinition)
        {
            ...
            if (LootDrop->Count > AmmoEntry->ItemDefinition->GetMaxStackSize())
            {
                ...
                LootDrops.Add(AFortInventory::MakeItemEntry(AmmoEntry->ItemDefinition,
                    OGCount - AmmoEntry->ItemDefinition->GetMaxStackSize(),
                    ResolveLootLevel(AmmoEntry->ItemDefinition)));
            }
...
void UFortLootPackage::ChooseLootForContainer(TArray<FFortItemEntry*>& LootDrops, FName TierGroup,
    int LootTier, int WorldLevel, ABuildingContainer* Container)
{
    ...
    if (VersionInfo.FortniteVersion == 19.40)
    {
        ...
        if (MinArr.Num() > 1 && MinArr[1] == 0 &&
            LootTierData->LootPackage.ToString().starts_with("WorldPKG.AthenaLoot.Weapon."))
        {
            MinArr[1] = 1;
...
bool UFortLootPackage::SpawnLootHook(ABuildingContainer* Container)
{
    ...
    UFortLootPackage::ChooseLootForContainer(LootDrops, RealTierGroup, -1,
        GameMode->GameState->WorldLevel, Container);
...
bool ServerOnAttemptInteract(ABuildingContainer* BuildingContainer,
    AFortPlayerPawnAthena* SearchingPawn)
{
    ...
    const FName OriginalTierGroup = BuildingContainer->SearchLootTierGroup;
    const bool bSpawnedLoot = UFortLootPackage::SpawnLootHook(BuildingContainer);
    if (bSpawnedLoot)
    {
        FFortAthenaScoreRoyaleCompatibility::HandleContainerSearched(BuildingContainer,
                SearchingPawn, OriginalTierGroup);
    }
...
static bool HasExactClassName(const UObject* Object, const FName& ExpectedClassName)
{
    return Object && Object->Class && Object->Class->Name == ExpectedClassName;
}
...
static const UClass* ResolveHopRockClass()
{
    ...
    if (!HopRockClass)
    {
        HopRockClass = FindObject<UClass>(L"/Game/Athena/Items/ForagedItems/LowGravity/"
...
{
    return Actor && Actor->HasbActorIsBeingDestroyed() && Actor->bActorIsBeingDestroyed;
}

static bool HasNearbyLiveActorOfClass(const UClass* ActorClass, const FVector& Location,
    double RadiusSquared, AActor** OutActor = nullptr)
{
    ...
    {
        if (!Actor || Actor->Class != ActorClass || IsActorBeingDestroyed(Actor))
        {
            continue;
        }

        const auto Delta = Actor->K2_GetActorLocation() - Location;
...
static void DisableAndScheduleActorRemoval(AActor* Actor)
{
    ...
    auto SetActorEnableCollision = Actor->GetFunction("SetActorEnableCollision");
    if (SetActorEnableCollision)
    {
        bool bEnableCollision = false;
        Actor->Call<void>(SetActorEnableCollision, bEnableCollision);
    }
    else
    {
        ...
        {
            SDK::DbgLog("[HopRock] SetActorEnableCollision missing; "
...
static int32 ScheduleHopRockClusterRemoval(ABuildingGameplayActorConsumable* ConsumedHopRock,
    const UClass* HopRockClass)
{
    ...
    const auto ConsumedLocation = ConsumedHopRock->K2_GetActorLocation();
    ...
    for (auto Actor : HopRocks)
    {
        ...
        const auto Delta = Actor->K2_GetActorLocation() - ConsumedLocation;
        if (Delta.SizeSquared() > DuplicateRadiusSquared || IsActorBeingDestroyed(Actor))
        {
            continue;
        }
    ...
    if (!bFoundConsumedActor && !IsActorBeingDestroyed(ConsumedHopRock))
    {
        DisableAndScheduleActorRemoval(ConsumedHopRock);
        ScheduledCount++;
    }

    SDK::DbgLog("[HopRock] removal-scheduled source=%p authority=%d "
        "cluster=%d loc=(%.1f,%.1f,%.1f)\n", (void*)ConsumedHopRock,
        ConsumedHopRock->HasAuthority(), ScheduledCount, ConsumedLocation.X, ConsumedLocation.Y,
        ConsumedLocation.Z);
    return ScheduledCount;
}

static void HopRockApplyGameplayEffectSpecToOwner(UObject* Context, FFrame& Stack,
    FActiveGameplayEffectHandle* Result)
{
    if (!ApplyGameplayEffectSpecToOwnerExecOG)
        return;

    static const FName GenericConsumeClassName(L"GA_GenericConsume_C");
    static const FName HopRockClassName(L"CBGA_LowGravity_UsingJump_C");

    UObject* SourceObject = nullptr;
    AFortPlayerControllerAthena* ScorePlayerController = nullptr;
    int32 ScoreBefore = 0;
    const bool bHasGenericConsumeName = HasExactClassName(Context, GenericConsumeClassName);
    const auto GenericConsumeClass = bHasGenericConsumeName
        ? ResolveGenericConsumeClass() : nullptr;
    const bool bIsGenericConsume = GenericConsumeClass && Context->Class == GenericConsumeClass;
    if (bIsGenericConsume)
    {
        auto GetCurrentSourceObject = Context->GetFunction("GetCurrentSourceObject");
        if (GetCurrentSourceObject)
        {
            SourceObject = Context->Call<UObject*>(GetCurrentSourceObject);
        }

        if (FFortAthenaScoreRoyaleCompatibility::IsActive())
        {
            auto Ability = Context->Cast<UFortGameplayAbility>();
            auto AbilitySystemComponent = Ability ? static_cast<UAbilitySystemComponent*>(Ability
                        ->GetAbilitySystemComponentFromActorInfo()) : nullptr;
            auto Avatar = AbilitySystemComponent && AbilitySystemComponent->HasAvatarActor()
                ? AbilitySystemComponent->AvatarActor : nullptr;
            auto Pawn = Avatar ? Avatar->Cast<AFortPlayerPawnAthena>() : nullptr;
            ScorePlayerController = Pawn && Pawn->Controller ? Pawn->Controller
                    ->Cast<AFortPlayerControllerAthena>() : nullptr;
            auto PlayerState = ScorePlayerController && ScorePlayerController->HasPlayerState()
                ? ScorePlayerController->PlayerState : nullptr;
            if (PlayerState && PlayerState->HasTotalPlayerScore())
                ScoreBefore = PlayerState->TotalPlayerScore;
        }
    }

    // The 4.20 effect-spec parameter is opaque and its stack layout differs between engine versions.
    ApplyGameplayEffectSpecToOwnerExecOG(Context, Stack, Result);

    const bool bHasHopRockName = HasExactClassName(SourceObject, HopRockClassName);
    const auto HopRockClass = bHasHopRockName ? ResolveHopRockClass() : nullptr;
    const bool bIsHopRock = HopRockClass && SourceObject->Class == HopRockClass;
    const bool bEffectApplied = Result && Result->bPassedFiltersAndWasExecuted;
    if (bEffectApplied && ScorePlayerController && SourceObject)
    {
        auto SourceActor = SourceObject->Cast<AActor>();
        if (SourceActor)
        {
            FFortAthenaScoreRoyaleCompatibility::HandleForagedItemConsumed(ScorePlayerController,
                    SourceActor, ScoreBefore);
        }
    }
    ...
    {
        SDK::DbgLog("[HopRock] consume-result context-exact=%d "
            "source=%p source-name-match=%d source-exact=%d "
            "handle=%d applied=%d\n", bIsGenericConsume, (void*)SourceObject, bHasHopRockName,
            bIsHopRock, Result ? Result->Handle : -1, bEffectApplied);
    }

    if (!bIsHopRock)
        return;

    auto HopRock = (ABuildingGameplayActorConsumable*)SourceObject;
    const bool bAlreadyDestroying = HopRock->HasbActorIsBeingDestroyed() &&
        HopRock->bActorIsBeingDestroyed;

    const bool bLegacyHopRockCleanup = VersionInfo.FortniteVersion >= 4.00 &&
        VersionInfo.FortniteVersion < 5.00;
    if (bLegacyHopRockCleanup && bEffectApplied && !bAlreadyDestroying && HopRock->HasAuthority())
    {
        ScheduleHopRockClusterRemoval(HopRock, HopRockClass);
    }
}

static void InstallHopRockConsumeHook()
{
    const bool bLegacyHopRockBuild = VersionInfo.FortniteVersion >= 4.00 &&
        VersionInfo.FortniteVersion < 5.00;
    if (!bLegacyHopRockBuild && !FFortAthenaScoreRoyaleCompatibility::IsSupportedBuild())
    {
        return;
    }

    auto GameplayAbilityClass = UFortGameplayAbility::StaticClass();
    auto GameplayAbilityDefault = GameplayAbilityClass
        ? GameplayAbilityClass->GetDefaultObj() : nullptr;
    auto ApplyEffectFunction = GameplayAbilityDefault ? GameplayAbilityDefault->GetFunction(
            "K2_ApplyGameplayEffectSpecToOwner") : nullptr;

    if (!ApplyEffectFunction)
    {
        SDK::DbgLog("[HopRock] probe=v3 apply-hook missing "
            "class=%p default=%p\n", (void*)GameplayAbilityClass, (void*)GameplayAbilityDefault);
        return;
    }

    auto ExecBefore = ApplyEffectFunction->ExecFunction;
    if (ExecBefore == (void*)HopRockApplyGameplayEffectSpecToOwner)
    {
        SDK::DbgLog("[HopRock] probe=v3 apply-hook already-installed "
            "fn=%p\n", (void*)ApplyEffectFunction);
        return;
    }

    Utils::ExecHook(ApplyEffectFunction, HopRockApplyGameplayEffectSpecToOwner,
        ApplyGameplayEffectSpecToOwnerExecOG);

    SDK::DbgLog("[HopRock] probe=v3 apply-hook fn=%p before=%p "
        "original=%p after=%p\n", (void*)ApplyEffectFunction, (void*)ExecBefore,
        (void*)ApplyGameplayEffectSpecToOwnerExecOG, (void*)ApplyEffectFunction->ExecFunction);
}
...
static void DestroyLoadedArsenalChests()
{
    ...
    auto ChestClass = FindObject<UClass>(L"/Game/Building/ActorBlueprints/Containers/"
...
void UFortLootPackage::SpawnFloorLootForContainer(const UClass* ContainerType)
{
    ...
    const bool bSuppressArsenalWorldLoot = FFortAthenaNativeLTMCompatibility::
...
static bool TryFindConsumableSpawnerSurface(ABGAConsumableSpawner* Spawner, FVector Start,
    FVector& OutImpactPoint)
{
    ...
    auto LineTraceSingle = KismetSystemLibrary->GetFunction("LineTraceSingle");
    if (!LineTraceSingle)
        LineTraceSingle = KismetSystemLibrary->GetFunction("LineTraceSingle_NEW");
    if (!LineTraceSingle)
        return false;

    TArray<AActor*> ActorsToIgnore{};
    if (Spawner->HasAssociatedBuildingActor() && Spawner->AssociatedBuildingActor)
    {
    ...
    const bool bHit = KismetSystemLibrary->Call<bool>(
        ...
        ActorsToIgnore, static_cast<uint8>(0), &Hit, true, FConsumableSpawnerTraceColor{},
    ...
    if (ImpactPointOffset == -2)
    {
        ...
        ImpactPointOffset = HitResultStruct ? static_cast<int32>(
                HitResultStruct->GetOffset("ImpactPoint")) : -1;
    }

    if (ImpactPointOffset < 0 || ImpactPointOffset + FVector::Size() >
            static_cast<int32>(sizeof(Hit.OpaqueStorage)))
    {
        return false;
    }

    auto& ImpactPoint = *(FVector*)(Hit.OpaqueStorage + ImpactPointOffset);
...
bool UFortLootPackage::SpawnConsumableActor(ABGAConsumableSpawner* Spawner)
{
    ...
    if (Spawner->HasSpawnLootTierGroup() && Spawner->SpawnLootTierGroup.ComparisonIndex != 0)
    {
        UFortLootPackage::ChooseLootForContainer(LootDrops, Spawner->SpawnLootTierGroup);
    }

    std::unordered_map<UClass*, AActor*> SpawnedActorsForSpawner;
    auto SpawnEntry = [&](FFortItemEntry& Entry) -> bool
    {
        ...
        const bool bAlignToSurface = Spawner->HasbAlignSpawnedActorsToSurface() &&
            Spawner->bAlignSpawnedActorsToSurface;
        if (bAlignToSurface)
        {
            FVector GroundLocation{};
            if (TryFindConsumableSpawnerSurface(Spawner, SpawnLocation, GroundLocation))
                SpawnLocation = GroundLocation;
        }

        UClass* ConsumableActorClass = nullptr;
        auto WrapperClass = UBGAConsumableWrapperItemDefinition::StaticClass();
        if (WrapperClass && Entry.ItemDefinition->IsA(WrapperClass))
        {
            auto WrapperDefinition = (UBGAConsumableWrapperItemDefinition*)Entry.ItemDefinition;
            if (WrapperDefinition->HasConsumableClass())
                ConsumableActorClass = WrapperDefinition->ConsumableClass.Get();
        }
        ...
        if (ConsumableActorClass)
        {
            ...
            auto SpawnedForClass = SpawnedActorsForSpawner.find(ConsumableActorClass);
            if (SpawnedForClass != SpawnedActorsForSpawner.end() && SpawnedForClass->second &&
                !IsActorBeingDestroyed(SpawnedForClass->second))
            {
                SDK::DbgLog("[ConsumableSpawner] duplicate-spawn-skipped "
                    "spawner=%p existing=%p reason=same-spawner\n", (void*)Spawner,
                    (void*)SpawnedForClass->second);
                return true;
            }
            if (HasNearbyLiveActorOfClass(ConsumableActorClass, SpawnLocation,
                    DuplicateRadiusSquared, &ExistingActor))
            {
                SpawnedActorsForSpawner[ConsumableActorClass] = ExistingActor;
                SDK::DbgLog("[ConsumableSpawner] duplicate-spawn-skipped "
                    "spawner=%p existing=%p "
                    "loc=(%.1f,%.1f,%.1f)\n", (void*)Spawner, (void*)ExistingActor, SpawnLocation.X,
                    SpawnLocation.Y, SpawnLocation.Z);
                return true;
            }

            auto ConsumableDefault = (ABuildingGameplayActorConsumable*)
                ConsumableActorClass->GetDefaultObj();
            if (ConsumableDefault && ConsumableDefault->HasbSpawnerCalculateRandomRotation() &&
                ConsumableDefault->bSpawnerCalculateRandomRotation)
            {
                SpawnRotation.Yaw = static_cast<float>(rand() % 360);
            }

            auto SpawnedActor = (AActor*)World->SpawnActor(ConsumableActorClass,
                FTransform(SpawnLocation, SpawnRotation));
            if (SpawnedActor)
            {
                SpawnedActorsForSpawner[ConsumableActorClass] = SpawnedActor;
                const auto ActualLocation = SpawnedActor->K2_GetActorLocation();
                SDK::DbgLog("[ConsumableSpawner] spawned actor=%p spawner=%p "
                    "requested=(%.1f,%.1f,%.1f) "
                    "actual=(%.1f,%.1f,%.1f)\n", (void*)SpawnedActor, (void*)Spawner,
                    SpawnLocation.X, SpawnLocation.Y, SpawnLocation.Z, ActualLocation.X,
                    ActualLocation.Y, ActualLocation.Z);
            }
    ...
    else if (Spawner->HasConsumablesToSpawn())
    {
        ...
        {
            auto& Entry = Consumables.Get(Index, FFortItemEntry::Size());
...
{
    // The BuildingContainer string below has no code xref in 4.20, so install this hook before any early return.
    ...
    if (VersionInfo.FortniteVersion >= 11.00)
    {
        ...
        auto OnAuthorityRandomUpgradeAppliedAddr = FindFunctionCall(
            L"OnAuthorityRandomUpgradeApplied", std::vector<uint8_t>{ 0x48, 0x89, 0x5C });
        Utils::Hook(OnAuthorityRandomUpgradeAppliedAddr, OnAuthorityRandomUpgradeApplied,
            OnAuthorityRandomUpgradeAppliedOG);
        return;
    }
    else
    {
