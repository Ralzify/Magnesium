    ...
    float InRadius, const FVector& InLocation) const
{
    ...
    for (const auto& Parameter : Parameters.NameOffsetMap)
    {
        ...
        if (*DestinationOffset != UINT32_MAX || Parameter.ElementSize != ExpectedSize ||
            Parameter.Offset > Parameters.Size || ExpectedSize > Parameters.Size - Parameter.Offset)
        {
            return false;
        }
    ...
    if (RadiusOffset == UINT32_MAX || LocationOffset == UINT32_MAX)
    {
        return false;
    }
    ...
    memcpy((PBYTE)Memory + RadiusOffset, &InRadius, sizeof(InRadius));
    memcpy((PBYTE)Memory + LocationOffset, &InLocation, LocationSize);
...
namespace
{
    ...
    struct FSafeZonePauseState
    {
        ...
        std::array<FSafeZoneFloatRestore, 16> FloatRestores{};
    ...
    void ResetSafeZonePauseStateForWorld(UWorld* World)
    {
        ...
        if (GSafeZonePauseState.World == World && GSafeZonePauseState.GameState == GameState)
        {
            return;
        }
    ...
    bool TryReadSafeZonePauseFlag(UObject* Object, const char* PropertyName, bool& OutValue)
    {
        if (!Object)
            return false;
    ...
    bool TryWriteSafeZonePauseFlag(UObject* Object, const char* PropertyName, bool Value,
    ...
    bool TryReadSafeZoneFloat(UObject* Object, const char* PropertyName, float& OutValue)
    {
        if (!Object || !PropertyName)
            return false;

        auto Property = Object->GetProperty(PropertyName, GUESS_PROP_FLAGS(float));
        if (!Property)
            return false;
    ...
    bool TryWriteSafeZoneFloat(UObject* Object, const char* PropertyName, float Value,
        bool* OutChanged = nullptr)
    {
        if (OutChanged)
            *OutChanged = false;
    ...
    bool IsOwnedBySafeZoneGameState(const UObject* Object, const AFortGameStateAthena* GameState)
    {
    ...
    UFortGameStateComponent_BattleRoyaleGamePhaseLogic* GetCurrentSafeZonePhaseLogic(UWorld* World)
    {
        ...
        auto Candidate = UFortGameStateComponent_BattleRoyaleGamePhaseLogic::Get(World);
        auto GameState = (AFortGameStateAthena*)World->GameState;
        return IsLiveSafeZoneObject(Candidate) && IsOwnedBySafeZoneGameState(Candidate, GameState)
            ? Candidate : nullptr;
    }

    AFortSafeZoneIndicator* GetCurrentSafeZoneIndicator(UWorld* World, AFortGameMode* GameMode)
    {
        auto PhaseLogic = GetCurrentSafeZonePhaseLogic(World);
        if (PhaseLogic && PhaseLogic->HasSafeZoneIndicator() && IsLiveSafeZoneObject(
    ...
    void MarkSafeZoneIndicatorDirty(AFortSafeZoneIndicator* Indicator, const wchar_t* PropertyName)
    {
    ...
    void RememberAndWriteSafeZoneFloat(UObject* Owner, const char* PropertyName,
        const wchar_t* ReplicatedPropertyName, float Value)
    {
            ...
            Index++)
        {
            ...
            if (Existing.Owner == Owner && Existing.PropertyName && strcmp(Existing.PropertyName,
                    PropertyName) == 0)
            {
                Restore = &Existing;
                break;
        ...
        if (!Restore && State.FloatRestoreCount <(int32)State.FloatRestores.size() &&
            TryReadSafeZoneFloat(Owner, PropertyName, PreviousValue))
        {
            Restore = &State.FloatRestores[State.FloatRestoreCount++];
            Restore->Owner = Owner;
            Restore->PropertyName = PropertyName;
            Restore->ReplicatedPropertyName = ReplicatedPropertyName;
        ...
        bool bChanged = false;
        if (TryWriteSafeZoneFloat(Owner, PropertyName, Value, &bChanged) &&
            bChanged && ReplicatedPropertyName)
        {
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(Owner, ReplicatedPropertyName);
        }
    }

    void ApplySafeZonePauseBookkeeping(AFortGameMode* GameMode, AFortGameStateAthena* GameState,
        UFortGameStateComponent_BattleRoyaleGamePhaseLogic* PhaseLogic,
        AFortSafeZoneIndicator* Indicator)
    {
        auto& State = GSafeZonePauseState;
        const float TimeRemaining = State.bHasFinishOffset ? (std::max)(0.f, State.FinishOffset)
                : 0.f;

        RememberAndWriteSafeZoneFloat(GameMode, "TimeRemainingWhenPhasePaused",
            L"TimeRemainingWhenPhasePaused", TimeRemaining);
        RememberAndWriteSafeZoneFloat(GameState, "TimeRemainingWhenPhasePaused",
            L"TimeRemainingWhenPhasePaused", TimeRemaining);
        RememberAndWriteSafeZoneFloat(PhaseLogic, "TimeRemainingWhenPhasePaused",
            L"TimeRemainingWhenPhasePaused", TimeRemaining);
        RememberAndWriteSafeZoneFloat(Indicator, "TimeRemainingWhenPhasePaused",
            L"TimeRemainingWhenPhasePaused", TimeRemaining);

        RememberAndWriteSafeZoneFloat(GameState, "SafeZonePauseTime", L"SafeZonePauseTime",
            State.PauseWorldTime);
        RememberAndWriteSafeZoneFloat(PhaseLogic, "SafeZonePauseTime", L"SafeZonePauseTime",
            State.PauseWorldTime);
        RememberAndWriteSafeZoneFloat(Indicator, "SafeZonePauseTime", L"SafeZonePauseTime",
            State.PauseWorldTime);
    ...
    void RestoreSafeZonePauseBookkeeping(AFortGameStateAthena* GameState,
        AFortSafeZoneIndicator* Indicator)
    {
        auto& State = GSafeZonePauseState;
            ...
            Index--)
        {
            ...
            if (!Restore.bHasValue || !IsLiveSafeZoneObject(Restore.Owner))
            {
                continue;
            }

            bool bChanged = false;
            if (TryWriteSafeZoneFloat(Restore.Owner, Restore.PropertyName, Restore.Value,
                &bChanged) && bChanged && Restore.ReplicatedPropertyName)
            {
                VersionFeatureAdapter::MarkReplicatedPropertyDirty(Restore.Owner,
    ...
    void RestoreSafeZonePauseBookkeepingForOwner(UObject* Owner)
    {
        if (!Owner)
            return;
    ...
    void CaptureLegacySafeZoneGeometry(AFortSafeZoneIndicator* Indicator)
    {
        ...
        State.bHadPreviousCenter = Indicator->HasPreviousCenter();
        State.bHadNextCenter = Indicator->HasNextCenter();
        State.bHadLastRadius = Indicator->HasLastRadius();
        State.bHadPreviousRadius = Indicator->HasPreviousRadius();
        ...
        float Alpha = 0.f;
        const float SegmentDuration = State.FinishOffset - State.StartOffset;
        if (State.bHasStartOffset && State.bHasFinishOffset && std::isfinite(SegmentDuration) &&
            SegmentDuration > 0.001f)
        {
            Alpha = (std::clamp)(-State.StartOffset / SegmentDuration, 0.f, 1.f);
        }

        if (auto GetCenter = Indicator->GetFunction("GetSafeZoneCenter"))
        {
            FVector LiveCenter = Indicator->Call<FVector>(GetCenter);
        ...
        {
            FVector ActorCenter = Indicator->K2_GetActorLocation();
        ...
        if (State.bHadLastCenter && IsFiniteSafeZoneVector(State.SavedLastCenter))
        {
            SourceCenter = &State.SavedLastCenter;
        }
        else if (State.bHadPreviousCenter && IsFiniteSafeZoneVector(State.SavedPreviousCenter))
        {
            SourceCenter = &State.SavedPreviousCenter;
        }

        if (!State.bHasFrozenCenter && SourceCenter && State.bHadNextCenter &&
            IsFiniteSafeZoneVector(State.SavedNextCenter))
        {
            State.FrozenCenter = *SourceCenter + (State.SavedNextCenter - *SourceCenter) * Alpha;
            State.bHasFrozenCenter = IsFiniteSafeZoneVector(State.FrozenCenter);
        }
        const float* SourceRadius = nullptr;
        if (State.bHadLastRadius && std::isfinite(State.SavedLastRadius) &&
            State.SavedLastRadius >= 0.f)
        {
            SourceRadius = &State.SavedLastRadius;
        }
        else if (State.bHadPreviousRadius && std::isfinite(State.SavedPreviousRadius) &&
        ...
        else if (SourceRadius && State.bHadNextRadius && std::isfinite(State.SavedNextRadius) &&
            State.SavedNextRadius >= 0.f)
        {
            State.FrozenRadius = *SourceRadius + (State.SavedNextRadius - *SourceRadius) * Alpha;
            State.bHasFrozenRadius = std::isfinite(State.FrozenRadius) && State.FrozenRadius >= 0.f;
        }
        State.bUsingLegacyGeometryFallback = State.bHasFrozenCenter && State.bHasFrozenRadius;
    }

    void ApplyLegacySafeZoneGeometryFreeze(AFortSafeZoneIndicator* Indicator)
    {
        auto& State = GSafeZonePauseState;
        if (!Indicator || Indicator != State.Indicator || !State.bUsingLegacyGeometryFallback)
        {
            return;
        }

        // 2.5-era indicators expose a native setter that moves the wall immediately; the 1.x builds do not.
        if (!State.bLegacyNativeSetterApplied)
        {
            State.bLegacyNativeSetterApplied = Indicator->TrySetSafeZoneRadiusAndCenter(
                    State.FrozenRadius, State.FrozenCenter);
        }
        ...
        if (State.bHasFrozenCenter)
        {
            if (State.bHadLastCenter)
            {
                ...
                MarkSafeZoneIndicatorDirty(Indicator, L"LastCenter");
            }
            if (State.bHadPreviousCenter)
            {
                Indicator->PreviousCenter = State.FrozenCenter;
                MarkSafeZoneIndicatorDirty(Indicator, L"PreviousCenter");
            }
            if (State.bHadNextCenter)
            {
                Indicator->NextCenter = State.FrozenCenter;
                MarkSafeZoneIndicatorDirty(Indicator, L"NextCenter");
            }
            Indicator->K2_SetActorLocation(State.FrozenCenter, false, nullptr, true);
        }
        ...
        if (State.bHasFrozenRadius)
        {
            if (State.bHadLastRadius)
            {
                ...
                MarkSafeZoneIndicatorDirty(Indicator, L"LastRadius");
            }
            if (State.bHadPreviousRadius)
            {
                Indicator->PreviousRadius = State.FrozenRadius;
                MarkSafeZoneIndicatorDirty(Indicator, L"PreviousRadius");
            }
            if (State.bHadNextRadius)
            {
                Indicator->NextRadius = State.FrozenRadius;
                MarkSafeZoneIndicatorDirty(Indicator, L"NextRadius");
            }
    ...
    void PrepareLegacySafeZoneResume(AFortSafeZoneIndicator* Indicator, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        if (!Indicator || Indicator != State.Indicator || !State.bUsingLegacyGeometryFallback ||
        ...
        // On 2.5-6.x, reflected fields alone do not refresh the wall's material or collision.
        Indicator->TrySetSafeZoneRadiusAndCenter(State.FrozenRadius, State.FrozenCenter);
        ...
        if (State.bHasStartOffset && State.bHasFinishOffset && State.StartOffset > 0.f)
        {
            ResumeStart = TimeSeconds + State.StartOffset;
            ResumeFinish = TimeSeconds + (std::max)(State.FinishOffset, State.StartOffset + 0.05f);
        }
        else if (State.bHasFinishOffset && State.FinishOffset > 0.f)
        {
            ResumeFinish = TimeSeconds + (std::max)(State.FinishOffset, 0.05f);
        }

        if (Indicator->HasSafeZoneStartShrinkTime())
        {
            Indicator->SafeZoneStartShrinkTime = ResumeStart;
            MarkSafeZoneIndicatorDirty(Indicator, L"SafeZoneStartShrinkTime");
        }
        if (Indicator->HasSafeZoneFinishShrinkTime())
        {
            Indicator->SafeZoneFinishShrinkTime = ResumeFinish;
            MarkSafeZoneIndicatorDirty(Indicator, L"SafeZoneFinishShrinkTime");
        }

        Indicator->ForceNetUpdate();
    }

    bool TrySetIndicatorPauseState(AFortSafeZoneIndicator* Indicator, bool bPaused)
    {
        if (!IsLiveSafeZoneObject(Indicator))
            return false;

        bool bTouchedIndicator = false;
        auto Function = Indicator->GetFunction("SetSafeZonePausedForPreview");
        if (Function)
        {
            struct
            {
        ...
        const bool bWroteWallPause = TryWriteSafeZonePauseFlag(Indicator, "bPaused", bPaused,
                &bWallPauseChanged);
        const bool bWrotePreviewPause = TryWriteSafeZonePauseFlag(Indicator, "bPausedForPreview",
                bPaused, &bPreviewPauseChanged);
        if (bWallPauseChanged)
        {
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(Indicator, L"bPaused");
        }
        if (bPreviewPauseChanged)
        {
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(Indicator, L"bPausedForPreview");
        }
        if (bTouchedIndicator || bWroteWallPause || bWrotePreviewPause)
        {
            Indicator->ForceNetUpdate();
        }

        bool bReadBackPaused = false;
        return bWroteWallPause && TryReadSafeZonePauseFlag(Indicator, "bPaused", bReadBackPaused) &&
            bReadBackPaused == bPaused;
    }

    bool ApplySafeZoneOwnerPauseFlags(AFortGameMode* GameMode, AFortGameStateAthena* GameState,
        UFortGameStateComponent_BattleRoyaleGamePhaseLogic* PhaseLogic, bool bPaused)
    {
        bool bApplied = false;
        bool bGameStateChanged = false;
        bool bPhaseLogicChanged = false;
        bApplied = TryWriteSafeZonePauseFlag(GameMode, "bSafeZonePaused", bPaused) || bApplied;
        bApplied = TryWriteSafeZonePauseFlag(GameState, "bSafeZonePaused", bPaused,
                &bGameStateChanged) || bApplied;
        bApplied = TryWriteSafeZonePauseFlag(PhaseLogic, "bSafeZonePaused", bPaused,
                &bPhaseLogicChanged) || bApplied;
        if (bGameStateChanged && GameState)
        {
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(GameState, L"bSafeZonePaused");
            GameState->ForceNetUpdate();
        }
        if (bPhaseLogicChanged && PhaseLogic)
        {
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(PhaseLogic, L"bSafeZonePaused");
            if (GameState)
                GameState->ForceNetUpdate();
        }
        return bApplied;
    ...
    void CaptureSafeZonePauseOffsets(AFortSafeZoneIndicator* Indicator, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        ...
        if (Indicator->HasSafeZoneFinishShrinkTime())
        {
            ...
            if (std::isfinite(FinishTime) && FinishTime >= 0.f)
            {
    ...
    void PinSafeZonesStartTime(AFortGameStateAthena* GameState,
        UFortGameStateComponent_BattleRoyaleGamePhaseLogic* PhaseLogic, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        UObject* Owner = nullptr;
        float CurrentStartTime = -1.f;

        // Component-driven seasons own this schedule; legacy seasons publish it on the GameState.
        if (PhaseLogic && PhaseLogic->HasSafeZonesStartTime())
        {
            Owner = PhaseLogic;
            CurrentStartTime = PhaseLogic->SafeZonesStartTime;
        }
        else if (GameState && GameState->HasSafeZonesStartTime())
        {
            Owner = GameState;
            CurrentStartTime = GameState->SafeZonesStartTime;
        }

        const bool bCurrentStartIsUsable = Owner && std::isfinite(CurrentStartTime) &&
            CurrentStartTime >= 0.f;
        if (Owner && State.SafeZonesStartOwner != Owner)
        {
            State.SafeZonesStartOwner = Owner;
            if (!State.bHasSafeZonesStartOffset && bCurrentStartIsUsable)
            {
                State.bHasSafeZonesStartOffset = true;
                State.SafeZonesStartOffset = CurrentStartTime - TimeSeconds;
            }
        }
        else if (Owner && !State.bHasSafeZonesStartOffset && bCurrentStartIsUsable)
        {
            State.bHasSafeZonesStartOffset = true;
            State.SafeZonesStartOffset = CurrentStartTime - TimeSeconds;
        }
        ...
        float PinnedStartTime = TimeSeconds + State.SafeZonesStartOffset;
        if (Owner == PhaseLogic)
        {
            PhaseLogic->SafeZonesStartTime = PinnedStartTime;
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(PhaseLogic, L"SafeZonesStartTime");
            if (GameState)
                GameState->ForceNetUpdate();
        }
        else if (Owner == GameState)
        {
            GameState->SafeZonesStartTime = PinnedStartTime;
            VersionFeatureAdapter::MarkReplicatedPropertyDirty(GameState, L"SafeZonesStartTime");
            GameState->ForceNetUpdate();
        }
    }

    void RestoreSafeZoneIndicatorPauseOffsets(AFortSafeZoneIndicator* Indicator, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        if (!std::isfinite(TimeSeconds) || Indicator != State.Indicator ||
            !IsLiveSafeZoneObject(Indicator))
        {
            return;
        }

        if (State.bHasStartOffset && Indicator->HasSafeZoneStartShrinkTime())
        {
            Indicator->SafeZoneStartShrinkTime = TimeSeconds + State.StartOffset;
            MarkSafeZoneIndicatorDirty(Indicator, L"SafeZoneStartShrinkTime");
        }
        if (State.bHasFinishOffset && Indicator->HasSafeZoneFinishShrinkTime())
        {
            Indicator->SafeZoneFinishShrinkTime = TimeSeconds + State.FinishOffset;
            MarkSafeZoneIndicatorDirty(Indicator, L"SafeZoneFinishShrinkTime");
        }
        Indicator->ForceNetUpdate();
    }

    void RestoreSafeZonePauseOffsets(AFortSafeZoneIndicator* Indicator,
        AFortGameStateAthena* GameState, UFortGameStateComponent_BattleRoyaleGamePhaseLogic*
            PhaseLogic, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        if (!std::isfinite(TimeSeconds))
            return;

        RestoreSafeZoneIndicatorPauseOffsets(Indicator, TimeSeconds);

        if (State.bHasSafeZonesStartOffset)
        {
            float RestoredStartTime = TimeSeconds + State.SafeZonesStartOffset;
            if (State.SafeZonesStartOwner == PhaseLogic)
            {
                PhaseLogic->SafeZonesStartTime = RestoredStartTime;
                VersionFeatureAdapter::MarkReplicatedPropertyDirty(PhaseLogic,
                        L"SafeZonesStartTime");
                if (GameState)
                    GameState->ForceNetUpdate();
            }
            else if (State.SafeZonesStartOwner == GameState)
            {
                GameState->SafeZonesStartTime = RestoredStartTime;
                VersionFeatureAdapter::MarkReplicatedPropertyDirty(GameState,
                        L"SafeZonesStartTime");
                GameState->ForceNetUpdate();
            }
        }
    ...
    void PinSafeZoneIndicatorDeadlines(AFortSafeZoneIndicator* Indicator, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        if (!Indicator || Indicator != State.Indicator || !std::isfinite(TimeSeconds))
        {
            return;
        }
    ...
    void RetirePausedSafeZoneIndicator(AFortSafeZoneIndicator* Indicator, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        ...
        {
            RestoreSafeZoneIndicatorPauseOffsets(Indicator, TimeSeconds);
            PrepareLegacySafeZoneResume(Indicator, TimeSeconds);
            TrySetIndicatorPauseState(Indicator, false);
            RestoreSafeZoneIndicatorPauseOffsets(Indicator, TimeSeconds);
            PrepareLegacySafeZoneResume(Indicator, TimeSeconds);
        }
        RestoreSafeZonePauseBookkeepingForOwner(Indicator);
        if (IsLiveSafeZoneObject(Indicator))
            Indicator->ForceNetUpdate();
        State.bWallPauseGateApplied = false;
    }

    void AdoptPausedSafeZoneIndicator(AFortSafeZoneIndicator* Indicator, AFortGameMode* GameMode,
        AFortGameStateAthena* GameState, UFortGameStateComponent_BattleRoyaleGamePhaseLogic*
            PhaseLogic, float TimeSeconds)
    {
        auto& State = GSafeZonePauseState;
        ...
        if (std::isfinite(TimeSeconds))
        {
            CaptureSafeZonePauseOffsets(Indicator, TimeSeconds);
            PinSafeZonesStartTime(GameState, PhaseLogic, TimeSeconds);
        }

        ApplySafeZonePauseBookkeeping(GameMode, GameState, PhaseLogic, Indicator);
        State.bWallPauseGateApplied = Indicator && TrySetIndicatorPauseState(Indicator, true);
        if (!State.bWallPauseGateApplied && Indicator)
        {
            ...
            PinSafeZoneIndicatorDeadlines(Indicator, TimeSeconds);
        }
    }
}
...
bool UFortGameStateComponent_BattleRoyaleGamePhaseLogic::IsSafeZonePaused()
{
    ...
    {
        GSafeZonePauseSnapshot.store(GSafeZonePauseState.bRequestedPaused,
    ...
    if (TryReadSafeZonePauseFlag(GameMode, "bSafeZonePaused", bNativePaused))
    {
        bPausedZone = bNativePaused;
        GSafeZonePauseSnapshot.store(bNativePaused, std::memory_order_release);
        return bNativePaused;
    }

    GSafeZonePauseSnapshot.store(bPausedZone, std::memory_order_release);
    return bPausedZone;
}

bool UFortGameStateComponent_BattleRoyaleGamePhaseLogic::GetSafeZonePausedSnapshot()
{
    return GSafeZonePauseSnapshot.load(std::memory_order_acquire);
}

void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::RequestSafeZonePaused(bool bPaused)
{
    GSafeZonePauseRequest.store(bPaused ? 1 : 0, std::memory_order_release);
}
...
void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::SetSafeZonePaused(bool bPaused)
{
    ...
    ResetSafeZonePauseStateForWorld(World);
    auto& State = GSafeZonePauseState;
    const bool bStateTransition = !State.bHasRequest || State.bRequestedPaused != bPaused;
    auto GameMode = World ? (AFortGameMode*)World->AuthorityGameMode : nullptr;
    auto GameState = World ? (AFortGameStateAthena*)World->GameState : nullptr;
    auto PhaseLogic = GetCurrentSafeZonePhaseLogic(World);
    auto Indicator = GetCurrentSafeZoneIndicator(World, GameMode);
    const float TimeSeconds = World ? (float)UGameplayStatics::GetTimeSeconds(World) : 0.f;

    if (State.bHasRequest && State.bRequestedPaused && Indicator != State.Indicator)
    {
        RetirePausedSafeZoneIndicator(State.Indicator, TimeSeconds);
        AdoptPausedSafeZoneIndicator(Indicator, GameMode, GameState, PhaseLogic, TimeSeconds);
    }
    ...
    if (bPaused)
    {
        if (bStateTransition)
        {
            ...
            State.FloatRestores = {};
            ResetLegacySafeZoneGeometrySnapshot();
            State.PauseWorldTime = std::isfinite(TimeSeconds) ? TimeSeconds : 0.f;
            if (std::isfinite(TimeSeconds))
            {
                CaptureSafeZonePauseOffsets(Indicator, TimeSeconds);
                PinSafeZonesStartTime(GameState, PhaseLogic, TimeSeconds);
        ...
        State.bPhasePauseGateApplied = ApplySafeZoneOwnerPauseFlags(GameMode, GameState, PhaseLogic,
                true);
        State.bWallPauseGateApplied = Indicator && TrySetIndicatorPauseState(Indicator, true);
    ...
    {
        RestoreSafeZonePauseOffsets(Indicator, GameState, PhaseLogic, TimeSeconds);
        PrepareLegacySafeZoneResume(Indicator, TimeSeconds);

        const bool bWallCleared = !Indicator || TrySetIndicatorPauseState(Indicator, false);
        ApplySafeZoneOwnerPauseFlags(GameMode, GameState, PhaseLogic, false);

        RestoreSafeZonePauseOffsets(Indicator, GameState, PhaseLogic, TimeSeconds);
        PrepareLegacySafeZoneResume(Indicator, TimeSeconds);
        RestoreSafeZonePauseBookkeeping(GameState, Indicator);

        State.bPhasePauseGateApplied = false;
        if (State.bWallPauseGateApplied && Indicator && !bWallCleared)
        {
            SDK::DbgLog("[SafeZone] warning: replicated wall pause "
    ...
    bPausedZone = bPaused;
    GSafeZonePauseSnapshot.store(bPaused, std::memory_order_release);
...
void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::ResetSafeZonePauseForMatch(UWorld* World)
{
    ResetSafeZonePauseStateForWorld(World);
    if (GSafeZonePauseState.bHasRequest && GSafeZonePauseState.bRequestedPaused)
    {
    ...
    GSafeZonePauseState = {};
    GSafeZonePauseState.World = World;
    GSafeZonePauseState.GameState = World ? (AFortGameStateAthena*)World->GameState : nullptr;
    bPausedZone = false;
    GSafeZonePauseSnapshot.store(false, std::memory_order_release);
    SDK::DbgLog("[SafeZone] pause state reset for match world=%p\n", (void*)World);
}

void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::TickSafeZonePause()
{
    auto World = UWorld::GetWorld();
    ResetSafeZonePauseStateForWorld(World);
    if (World)
    {
        const int32 RequestedPause = GSafeZonePauseRequest.exchange(-1, std::memory_order_acq_rel);
        if (RequestedPause >= 0)
        {
            SetSafeZonePaused(RequestedPause != 0);
        }
    }
    auto& State = GSafeZonePauseState;
    if (!World || !State.bHasRequest || !State.bRequestedPaused)
    {
        return;
    }
    ...
    else
    {
        ...
        const bool bIndicatorWallPauseApplied = Indicator && TryWriteSafeZonePauseFlag(Indicator,
                "bPaused", true, &bIndicatorWallPauseChanged);
        if (Indicator)
        {
            TryWriteSafeZonePauseFlag(Indicator, "bPausedForPreview", true,
            ...
            if (bIndicatorWallPauseChanged || bIndicatorPreviewPauseChanged)
            {
                Indicator->ForceNetUpdate();
            }
        }
        bool bReadBackWallPaused = false;
        State.bWallPauseGateApplied = bIndicatorWallPauseApplied && TryReadSafeZonePauseFlag(
                Indicator, "bPaused", bReadBackWallPaused) && bReadBackWallPaused;
    }
    ...
    if (State.bWallPauseGateApplied)
    {
        if (!State.bLoggedWallPause)
        {
            ...
            SDK::DbgLog("[SafeZone] replicated wall pause armed "
                "version=%.2f indicator=%p phaseGate=%d\n", VersionInfo.FortniteVersion,
                (void*)Indicator, State.bPhasePauseGateApplied ? 1 : 0);
        }
        return;
    }
    ...
    PinSafeZonesStartTime((AFortGameStateAthena*)World->GameState, PhaseLogic, TimeSeconds);

    if (!State.bHasStartOffset && !State.bHasFinishOffset)
    {
        CaptureSafeZonePauseOffsets(Indicator, TimeSeconds);
    }
    ...
    if (!State.bLoggedBackstop)
    {
        ...
        SDK::DbgLog("[SafeZone] exact legacy pause fallback armed "
            "version=%.2f indicator=%p phaseGate=%d "
            "startOffset=%.2f finishOffset=%.2f\n", VersionInfo.FortniteVersion, (void*)Indicator,
            State.bPhasePauseGateApplied ? 1 : 0, State.bHasStartOffset ? State.StartOffset : -1.f,
...
void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::HandleMatchHasStarted(AFortGameMode* GameMode)
{
    ...
    if (!bSkipWarmup)
    {
        ...
        const float WarmupDuration = FConfiguration::bAutoBusStart.load()
                ? FConfiguration::BusStartDelay.load() : 99999999.f;
...
FVector ClampToPlayableBounds(const FVector& Candidate, float Radius, const FBoxSphereBounds& Bounds)
{
    ...
    Clamped.X = std::clamp(Clamped.X, Bounds.Origin.X - Bounds.BoxExtent.X + Radius,
        Bounds.Origin.X + Bounds.BoxExtent.X - Radius);
    Clamped.Y = std::clamp(Clamped.Y, Bounds.Origin.Y - Bounds.BoxExtent.Y + Radius,
        Bounds.Origin.Y + Bounds.BoxExtent.Y - Radius);
...
void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::StartNewSafeZonePhase(int NewSafeZonePhase, bool bInitial)
{
    ...
    if (Array.IsValidIndex(NewSafeZonePhase))
    {
        ...
        if (bMovingCustomZoneRequested)
        {
            auto World = UWorld::GetWorld();
            auto GameState = World ? (AFortGameStateAthena*)World->GameState : nullptr;
            bMovingCustomZone = CustomSafeZoneRuntime::PublishManagedPhase(World,
                GameState && GameState->HasMapInfo() ? GameState->MapInfo : nullptr,
                SafeZoneIndicator, NewSafeZonePhase, "component-phase-start", &Array);
        }
        ...
        if (bMovingCustomZone)
        {
            MarkSafeZoneIndicatorDirty(SafeZoneIndicator, L"SafeZoneStartShrinkTime");
            MarkSafeZoneIndicatorDirty(SafeZoneIndicator, L"SafeZoneFinishShrinkTime");
...
void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::StartAircraftPhase()
{
    ...
    if (GameState->MapInfo->FlightInfos.Num() > 0)
    {
        ...
        if (FConfiguration::bLateGame)
        {
            ...
            const int32 SourceCircleIndex = CustomSafeZoneRuntime::ResolveRuntimeStartPhase() - 1;
            if (SourceCircleIndex < 0 || (size_t)SourceCircleIndex >= StormCircles.size())
            {
                printf("LateGame source phase %d is not supported by "
            ...
            if (FConfiguration::bCustomSafeZone)
            {
                ...
                if (CustomSafeZoneRuntime::IsMovingModeRequested())
                {
                    ...
                    CustomSafeZoneRuntime::TryGetSourceCircle(
                        UWorld::GetWorld(), GameState->MapInfo, Loc, SourceRadius,
            ...
            if (FConfiguration::bMovingBus)
            {
                ...
                FlightInfo.TimeTillDropEnd = 10.f ;
            }
            else
            {
        ...
        if (!GameState->MapInfo->AircraftClass.Get())
            return;
        auto Aircraft = AFortAthenaAircraft::SpawnAircraft(UWorld::GetWorld(),
            GameState->MapInfo->AircraftClass, FlightInfo);
        ...
        for (auto& Player__Uncasted : ((AFortGameMode*)UWorld::GetWorld()->AuthorityGameMode)->AlivePlayers)
        {
            ...
            AFortPlayerControllerAthena::ClearWarmupShieldForAircraft(Player, "component-phase");
...
void UFortGameStateComponent_BattleRoyaleGamePhaseLogic::Tick()
{
    ...
    const bool bReturnedToWarmup = TickState.Owner == this && TickState.bHasLastPhase &&
        _GamePhase <= EAthenaGamePhase::Warmup && TickState.LastPhase > EAthenaGamePhase::Warmup;
    ...
    if (!bSkipAircraft)
    {
        if (_GamePhase <= EAthenaGamePhase::Warmup)
        {
            ...
            {
                if (((AFortGameMode*)UWorld::GetWorld()->AuthorityGameMode)->AlivePlayers.Num() > 0 &&
                    WarmupEarlyCountdownDuration != -1 && WarmupEarlyCountdownDuration < Time)
                {
                    gettingReady = true;
        ...
        if (_GamePhase == EAthenaGamePhase::Aircraft)
        {
            ...
            {
                if (((AFortGameMode*)UWorld::GetWorld()->AuthorityGameMode)->AlivePlayers.Num() > 0 &&
                    Aircrafts_GameState[0].Get() && Aircrafts_GameState[0]->DropStartTime < Time)
                {
                    busUnlocked = true;
            ...
            {
                if (((AFortGameMode*)UWorld::GetWorld()->AuthorityGameMode)->AlivePlayers.Num() > 0 &&
                    Aircrafts_GameState[0].Get() && Aircrafts_GameState[0]->DropEndTime != -1 &&
        ...
        {
            if (((AFortGameMode*)UWorld::GetWorld()->AuthorityGameMode)->AlivePlayers.Num() > 0 &&
                Aircrafts_GameState[0].Get() && Aircrafts_GameState[0]->FlightEndTime != -1 &&
    ...
    if (bEnableZones)
    {
        if (_GamePhase == EAthenaGamePhase::SafeZones)
        {
            ...
            {
                if (((AFortGameMode*)UWorld::GetWorld()->AuthorityGameMode)->AlivePlayers.Num() > 0 &&
                    SafeZonesStartTime != -1 && SafeZonesStartTime < Time)
                {
                    formedZone = true;
                    auto SafeZoneIndicator = SetupSafeZoneIndicator();
                    StartNewSafeZonePhase(FConfiguration::bLateGame
