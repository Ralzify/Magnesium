...
public:
    ...
    // Single entry point for changing a player's kill count: writes whichever
    // of KillScore/Kills this build actually has, notifies the scoreboard, and
    // pushes the value to the stat that reactive cosmetics watch. Defined in
    // FortPlayerControllerAthena.cpp because the stat push needs the controller
    // type, which cannot be included from here.
    //
    // Not named SetKillScore: that is already the put accessor generated by
    // DEFINE_PROP(KillScore, int32), and a second overload makes every
    // `PlayerState->KillScore = X` assignment ambiguous.
    void ApplyKillScore(int32 NewScore);
    // Effective kill count for this build, reading KillScore or Kills.
