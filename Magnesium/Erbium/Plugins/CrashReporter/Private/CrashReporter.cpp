...
namespace
{
    ...
    void InitializeCrashOutputDirectory()
    {
        ...
        if (CurrentDirectoryLength > 0 &&
            CurrentDirectoryLength < std::size(CurrentDirectory))
        {
            _snwprintf_s(
                GCrashOutputDirectory,
                std::size(GCrashOutputDirectory),
                _TRUNCATE,
                L"%ls\\MagnesiumCrashes",
                CurrentDirectory);
    ...
    void InitializeReporterModuleRange()
    {
        ...
        auto NtHeaders = reinterpret_cast<IMAGE_NT_HEADERS*>(
            reinterpret_cast<uintptr_t>(ReporterModule) +
            DosHeader->e_lfanew);
        ...
        GReporterModuleStart = reinterpret_cast<uintptr_t>(
            ReporterModule);
        GReporterModuleEnd = GReporterModuleStart +
        ...
        const LPEXCEPTION_POINTERS ExceptionInfo)
    {
        ...
        if ((ExceptionCode & 0x80000000) == 0 ||
            (ExceptionCode & 0x30000000) != 0 ||
    ...
    {
        return GReporterModuleStart &&
            Address >= GReporterModuleStart &&
            Address < GReporterModuleEnd;
    }

    bool IsMagnesiumRelatedException(
        const LPEXCEPTION_POINTERS ExceptionInfo)
    {
        ...
        const uintptr_t StackPointer =
            ExceptionInfo->ContextRecord->Rsp;
        MEMORY_BASIC_INFORMATION StackRegion{};
        if (!StackPointer ||
            !VirtualQuery(
                reinterpret_cast<const void*>(StackPointer),
                &StackRegion,
                sizeof(StackRegion)) ||
        ...
        const size_t AvailableAddresses =
            (RegionEnd - StackPointer) / sizeof(uintptr_t);
        const size_t AddressesToInspect =
            (std::min)(
                AvailableAddresses,
                MaxStackAddressesToInspect);
        auto StackAddresses = reinterpret_cast<const uintptr_t*>(
            StackPointer);
        for (size_t Index = 0;
        ...
        FCrashArtifactPaths& Paths)
    {
        ...
        const LONG Sequence = InterlockedIncrement(
            &GCrashArtifactSequence);
        wchar_t Stem[160]{};
        _snwprintf_s(
            Stem,
            std::size(Stem),
            _TRUNCATE,
            L"Magnesium-%04u%02u%02u-%02u%02u%02u-"
            L"p%lu-t%lu-%ld-%ls",
            LocalTime.wYear,
            LocalTime.wMonth,
            LocalTime.wDay,
        ...
        size_t TextLength)
    {
        ...
        const bool bWritten = WriteFileBytes(
            File,
            Text,
            TextLength);
    ...
    void WriteBasicCrashHeader(
        const wchar_t* Path,
        const char* Kind,
        const LPEXCEPTION_POINTERS ExceptionInfo)
    {
        const auto Record = ExceptionInfo->ExceptionRecord;
        const bool bMemoryFault =
            (Record->ExceptionCode == EXCEPTION_ACCESS_VIOLATION ||
             Record->ExceptionCode == EXCEPTION_IN_PAGE_ERROR) &&
            Record->NumberParameters > 1;
        const unsigned long long FaultOperation = bMemoryFault
            ? Record->ExceptionInformation[0]
            : 0;
        const unsigned long long FaultAddress = bMemoryFault
    ...
    bool WriteMiniDump(
        const wchar_t* Path,
        const LPEXCEPTION_POINTERS ExceptionInfo,
        const MINIDUMP_TYPE PreferredDumpType,
        DWORD& Error)
    {
        Error = ERROR_SUCCESS;
        ...
        BOOL bWritten = MiniDumpWriteDump(
            GetCurrentProcess(),
            GetCurrentProcessId(),
            DumpFile,
            PreferredDumpType,
    ...
    void AppendDumpStatus(
        const wchar_t* TextPath,
        const wchar_t* DumpPath,
        const bool bDumpWritten,
        const DWORD DumpError)
    {
        char Status[1024]{};
        const int StatusLength = _snprintf_s(
            Status,
            std::size(Status),
            _TRUNCATE,
            "MinidumpWritten: %d\r\n"
    ...
    void WriteInitialCrashArtifacts(
        const wchar_t* Kind,
        const char* KindText,
        const LPEXCEPTION_POINTERS ExceptionInfo,
        FCrashArtifactPaths& Paths,
        ...
        LPEXCEPTION_POINTERS ExceptionInfo)
    {
        ...
        else
        {
            if (!IsMagnesiumRelatedException(ExceptionInfo))
                return EXCEPTION_CONTINUE_SEARCH;
            const LONG ReportIndex = InterlockedIncrement(
                &GFirstChanceReportCount);
        ...
        FCrashArtifactPaths Paths{};
        WriteInitialCrashArtifacts(
            bEventException ? L"event-active" : L"first-chance",
            bEventException
                ? "fatal exception while event active"
                : "first-chance Magnesium-related exception",
            ExceptionInfo,
            Paths,
            bEventException);
        SDK::DbgLog(
            "[CrashReporter] %s artifact text=%ls dump=%ls "
            "code=0x%08lX\n",
            bEventException ? "event-active" : "first-chance",
            Paths.TextPath,
            Paths.DumpPath,
            ExceptionInfo->ExceptionRecord->ExceptionCode);
...
LONG WINAPI ErbiumUnhandledExceptionFilter(
    LPEXCEPTION_POINTERS ExceptionInfo)
{
    if (!IsReportableException(ExceptionInfo))
        return EXCEPTION_CONTINUE_SEARCH;
    if (InterlockedCompareExchange(
            &GUnhandledCaptureStarted,
            1,
            0) != 0)
    {
        return EXCEPTION_CONTINUE_SEARCH;
    ...
    SymSetOptions(
        ...
        SYMOPT_LOAD_LINES |
        SYMOPT_UNDNAME);
    const BOOL bSymbolsInitialized = SymInitialize(
        CurrentProcess,
        nullptr,
        TRUE);
    ...
    char SymbolStorage[
        sizeof(IMAGEHLP_SYMBOL64) + sizeof(SymbolName)]{};
    auto Symbol = reinterpret_cast<IMAGEHLP_SYMBOL64*>(
        SymbolStorage);
    ...
    for (size_t FrameIndex = 0;
         ...
         ++FrameIndex)
    {
        const BOOL bWalked = StackWalk64(
        ...
        const DWORD64 ModuleBase = SymGetModuleBase64(
            CurrentProcess,
            StackFrame.AddrPC.Offset);
        if (ModuleBase)
        {
            ...
            ReportStream << " ("
                << (FileName ? FileName + 1 : ModulePath)
                << "+" << Offset << ")";
        }
        ReportStream << ": ";

        DWORD64 SymbolDisplacement = 0;
        const BOOL bSymbolResolved =
            bSymbolsInitialized &&
            SymGetSymFromAddr64(
                CurrentProcess,
                StackFrame.AddrPC.Offset,
                ...
                &ImageLine))
        {
            const char* FileName = strrchr(
                ...
                '\\');
            ReportStream << " ["
                << (FileName ? FileName + 1 : ImageLine.FileName)
    ...
    if (bSymbolsInitialized)
        SymCleanup(CurrentProcess);
    SDK::DbgLog(
        "[CrashReporter] unhandled artifacts text=%ls dump=%ls "
        "code=0x%08lX\n",
        Paths.TextPath,
        Paths.DumpPath,
        ExceptionInfo->ExceptionRecord->ExceptionCode);
...
void FCrashReporter::Register()
{
    ...
    const PVOID VectoredHandler = AddVectoredExceptionHandler(
        1,
        ErbiumFirstChanceExceptionHandler);
    SetUnhandledExceptionFilter(
        ErbiumUnhandledExceptionFilter);
    SDK::DbgLog(
        "[CrashReporter] handlers installed output=%ls "
        "module=%p-%p vectored=%p\n",
        GCrashOutputDirectory,
        reinterpret_cast<void*>(GReporterModuleStart),
        reinterpret_cast<void*>(GReporterModuleEnd),
        VectoredHandler);
}
