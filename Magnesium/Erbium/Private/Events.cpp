...
namespace
{
    ...
    constexpr uint32 InvalidParamOffset = (std::numeric_limits<uint32>::max)();
    constexpr int32 UnobservedPhase = (std::numeric_limits<int32>::min)();

    enum class EEventStartStage : uint8
    {
        Idle, LoadContent, WaitForLoader, WaitForTargets, Started,
    };
    ...
    int32 SendDurianFinalPhaseCue()
    {
        ...
        if (!IsLiveEventObject(World) || !World->HasNetDriver() ||
        ...
        for (int32 Index = 0; Index < Connections.Num(); ++Index)
        {
            ...
            if (!IsLiveEventObject(Connection) || !Connection->HasPlayerController())
            {
                continue;
            }

            auto PlayerController = Connection->PlayerController;
            if (!IsLiveEventObject(PlayerController) || !PlayerController->IsA(ControllerClass))
            {
                continue;
            }
    ...
    bool TryGetEventObjectIdentity(const UObject* Object, FEventObjectIdentity& OutIdentity)
    {
        OutIdentity = {};
    ...
    bool HasTrackedIdentity(const std::vector<FEventObjectIdentity>& Identities,
        const FEventObjectIdentity& Identity)
    {
        return std::find(Identities.begin(), Identities.end(), Identity) != Identities.end();
    }

    UObject* ResolveEventObjectIdentity(const FEventObjectIdentity& Identity)
    {
        if (Identity.ObjectIndex < 0 || Identity.ObjectIndex >= TUObjectArray::Num())
        {
            return nullptr;
        }

        auto Item = TUObjectArray::GetItemByIndex(Identity.ObjectIndex);
    ...
    bool IsSaneArray(const void* Data, int32 Num, int32 Max, size_t ElementSize,
        int32 MaximumElements)
    {
        ...
        if (Num == 0)
            return true;
        if (!Data || static_cast<size_t>(Num) > (std::numeric_limits<size_t>::max)() / ElementSize)
        {
            return false;
        }
    ...
    bool HasParameterPropertyType(UFunction* Function, const UFunction::ParamNamed& Parameter,
        uint64 CastFlags)
    {
        if (!Function || Parameter.Name.empty())
            return false;
        const auto NamedProperty = Function->GetProperty(Parameter.Name.c_str());
        const auto TypedProperty = Function->GetProperty(Parameter.Name.c_str(), CastFlags);
    ...
    bool ValidateParameterBuffer(UFunction* Function)
    {
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            if (!(Parameter.PropertyFlags & CPF_Parm))
                continue;
            if (Parameter.ElementSize == 0 || Parameter.Offset > Parameters.Size ||
    ...
    bool CanInvokeWithZeroedScalarInput(UFunction* Function)
    {
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            const bool bSupportedScalar = (Parameter.ElementSize == sizeof(uint8) &&
                    HasParameterPropertyType(Function, Parameter, CastClassBoolProperty)) ||
                (Parameter.ElementSize == sizeof(int32) && HasParameterPropertyType(
                        Function, Parameter, CastClassIntProperty)) ||
                (Parameter.ElementSize == sizeof(float) && HasParameterPropertyType(
                        Function, Parameter, CastClassFloatProperty)) ||
                (Parameter.ElementSize == sizeof(double) && HasParameterPropertyType(
    ...
    {
        if (!IsLiveEventObject(Target) || !CanInvokeWithZeroedScalarInput(Function))
        {
            return false;
        }
    ...
    {
        if (!IsLiveEventObject(Target) || !ValidateParameterBuffer(Function))
        {
            return false;
        }
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            bBoolInput = InputSize == sizeof(uint8) && HasParameterPropertyType(
                    Function, Parameter, CastClassBoolProperty);
            bIntInput = InputSize == sizeof(int32) && HasParameterPropertyType(
                    Function, Parameter, CastClassIntProperty);
        }
        if (InputCount > 1 || (InputCount == 1 && !bBoolInput && !bIntInput))
        {
            return false;
        }
        ...
        if (InputCount == 1)
        {
            if (bBoolInput)
            {
                ...
                memcpy((PBYTE)Memory + InputOffset, &Enabled, sizeof(Enabled));
            }
            else
            {
    ...
    bool InvokeBoolNoInput(UObject* Target, UFunction* Function, bool& OutValue)
    {
        OutValue = false;
        if (!IsLiveEventObject(Target) || !ValidateParameterBuffer(Function))
        {
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            {
                if (ReturnOffset != InvalidParamOffset || Parameter.ElementSize != sizeof(uint8) ||
                    !HasParameterPropertyType(Function, Parameter, CastClassBoolProperty))
                {
                    return false;
                }
        ...
        memset(Memory, 0, Parameters.Size);
        Target->ProcessEvent(Function, Memory);
        OutValue = *reinterpret_cast<const uint8*>((PBYTE)Memory + ReturnOffset) != 0;
        FMemory::Free(Memory);
        return true;
    }

    bool ResolveOnReadyLayout(UFunction* Function, FOnReadyLayout& OutLayout)
    {
        OutLayout = {};
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            if (*Destination != InvalidParamOffset || Parameter.ElementSize != ExpectedSize ||
                !HasParameterPropertyType(Function, Parameter, ExpectedCastFlags))
            {
                return false;
            }
    ...
    bool InvokeOnReady(UObject* Target, UFunction* Function, const FEventContext& Context)
    {
        FOnReadyLayout Layout{};
        if (!IsLiveEventObject(Target) || !IsLiveEventObject(Context.GameState) ||
            !IsLiveEventObject(Context.Playlist) || !ResolveOnReadyLayout(Function, Layout))
        {
            return false;
        }
        ...
        memcpy((PBYTE)Memory + Layout.GameStateOffset, &GameState, sizeof(GameState));
        memcpy((PBYTE)Memory + Layout.PlaylistOffset, &Playlist, sizeof(Playlist));
        memcpy((PBYTE)Memory + Layout.ContextTagsOffset, &ContextTags, sizeof(ContextTags));
        Target->ProcessEvent(Function, Memory);
        FMemory::Free(Memory);
        return true;
    }

    bool ResolveStartAtIndexLayout(UFunction* Function, FStartAtIndexLayout& OutLayout)
    {
        OutLayout = {};
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            else if (Name && (strstr(Name, "TimeOffset") || strstr(Name, "SequenceTime")))
            {
                if (OutLayout.TimeOffset != InvalidParamOffset ||
                    ...
                    (Parameter.ElementSize == sizeof(float) ? !HasParameterPropertyType(
                            Function, Parameter, CastClassFloatProperty)
                        : !HasParameterPropertyType(Function, Parameter, CastClassDoubleProperty)))
                {
                    return false;
                }
    ...
    bool InvokeStartAtIndex(UObject* Target, UFunction* Function)
    {
        ...
        if (!IsLiveEventObject(Target) || !ResolveStartAtIndexLayout(Function, Layout) ||
            Layout.Size == 0)
        {
            return false;
        }
        ...
        memcpy((PBYTE)Memory + Layout.IndexOffset, &StartingIndex, sizeof(StartingIndex));
    ...
    bool InvokeOptionalName(UObject* Target, UFunction* Function, const FName& Value)
    {
        if (!IsLiveEventObject(Target) || !ValidateParameterBuffer(Function))
        {
            return false;
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            if (!IsInputParameter(Parameter))
                continue;
            ++InputCount;
            if (Parameter.ElementSize != sizeof(FName) || !HasParameterPropertyType(
    ...
    const FEvent* FindCurrentEvent()
    {
        ...
        {
            if (fabs(Event.EventVersion - VersionInfo.FortniteVersion) < 0.001)
            {
                return &Event;
            }
        }
    ...
    {
        return Event && fabs(Event->EventVersion - 17.30) < 0.001 && Event->ScriptingClass &&
            wcscmp(Event->ScriptingClass,
    ...
    {
        return Event && fabs(Event->EventVersion - 17.50) < 0.001 && Event->ScriptingClass &&
            wcscmp(Event->ScriptingClass,
    ...
    template <size_t ByteCount> bool MatchesEventCode(uint64 Address,
        const uint8 (&Expected)[ByteCount])
    {
        return Address && SDK::MemReadable(reinterpret_cast<const void*>(Address), ByteCount) &&
            memcmp(reinterpret_cast<const void*>(Address), Expected, ByteCount) == 0;
    }
    ...
    void InstallRiftTourMeshPredicateHooks()
    {
        ...
        {
            0x40, 0x53, 0x48, 0x83, 0xEC, 0x20, 0x48, 0x8B, 0x81, 0xA8, 0x00, 0x00, 0x00,
        };
        constexpr uint8 RootPredicateCode[] =
        {
            0x48, 0x83, 0xEC, 0x28, 0x48, 0x8B, 0x89, 0x28, 0x02, 0x00, 0x00,
            0x48, 0x85, 0xC9, 0x74,
        };
        constexpr uint8 EdgeWorldPredicateCode[] =
        {
            0x48, 0x89, 0x5C, 0x24, 0x08, 0x48, 0x89, 0x74, 0x24, 0x10,
            0x57, 0x48, 0x83, 0xEC, 0x20, 0x48, 0x8B, 0xF1, 0x33, 0xDB,
        ...
        const uint64 MeshNodeType = ImageBase + MeshNodeTypeRva;
        const uint64 RootPredicate = ImageBase + RootPredicateRva;
        const uint64 EdgeWorldPredicate = ImageBase + EdgeWorldPredicateRva;
        const uint64 EdgeActorPredicate = ImageBase + EdgeActorPredicateRva;
        const uint64 ClientPredicate = ImageBase + ClientPredicateRva;
        const bool bMeshNodeMatched = MatchesEventCode(MeshNodeType, MeshNodeTypeCode);
        const bool bRootPredicateMatched = MatchesEventCode(RootPredicate, RootPredicateCode);
        ...
        if (bMeshNodeMatched)
        {
            Utils::Hook(MeshNodeType, RiftTourMeshNodeTypeEdge);
        }
        if (bRootPredicateMatched)
        {
            Utils::Hook(RootPredicate, RiftTourMeshPredicateTrue);
        }
        if (bEdgeWorldPredicateMatched)
        {
            Utils::Hook(EdgeWorldPredicate, RiftTourMeshPredicateTrue);
        }
        if (bEdgeActorPredicateMatched)
        {
            Utils::Hook(EdgeActorPredicate, RiftTourMeshPredicateTrue);
        }
        if (bClientPredicateMatched)
        {
            Utils::Hook(ClientPredicate, RiftTourMeshPredicateFalse);
        }
        SDK::DbgLog("[Events] Rift Tour compatibility hooks mesh=%d root=%d "
            "edgeWorld=%d edgeActor=%d client=%d\n", bMeshNodeMatched ? 1 : 0,
            bRootPredicateMatched ? 1 : 0, bEdgeWorldPredicateMatched ? 1 : 0,
            bEdgeActorPredicateMatched ? 1 : 0, bClientPredicateMatched ? 1 : 0);
    }
    ...
    AActor* FindLiveActor(const wchar_t* ClassPath)
    {
        ...
        if (IsSaneArray(Actors.Data, Actors.Num(), Actors.Max(), sizeof(AActor*), 4096))
        {
    ...
    template <typename ValueType> bool ReadEventProperty(UObject* Object, const char* PropertyName,
        uint64 CastFlags, ValueType& OutValue)
    {
        OutValue = {};
        if (!IsLiveEventObject(Object) || !PropertyName)
            return false;

        const uint32 Offset = Object->GetOffset(PropertyName, CastFlags);
        const int32 ObjectSize = Object->Class->GetPropertiesSize();
        if (Offset == InvalidParamOffset || ObjectSize <= 0 ||
            Offset > static_cast<uint32>(ObjectSize) || sizeof(ValueType) >
    ...
    template <typename ValueType> bool WriteEventProperty(UObject* Object, const char* PropertyName,
        uint64 CastFlags, const ValueType& Value)
    {
        if (!IsLiveEventObject(Object) || !PropertyName)
            return false;
    ...
    bool InvokeSingleInput(UObject* Target, UFunction* Function, const void* Value,
        uint32 ValueSize, uint64 CastFlags = 0)
    {
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            if (!IsInputParameter(Parameter))
                continue;
            ++InputCount;
            if (Parameter.ElementSize != ValueSize || (CastFlags && !HasParameterPropertyType(
        ...
        if (InputCount != 1 || InputOffset == InvalidParamOffset || Parameters.Size == 0)
        {
            return false;
        }
        ...
        if (!Memory)
            return false;
        memset(Memory, 0, Parameters.Size);
        memcpy(reinterpret_cast<uint8*>(Memory) + InputOffset, Value, ValueSize);
        Target->ProcessEvent(Function, Memory);
        FMemory::Free(Memory);
        return true;
    }

    bool InvokeSmallIntegerInput(UObject* Target, UFunction* Function, int32 Value)
    {
        if (!IsLiveEventObject(Target) || !ValidateParameterBuffer(Function))
        {
            return false;
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            if (!IsInputParameter(Parameter))
                continue;
            ++InputCount;
            if (Parameter.ElementSize != sizeof(uint8) && Parameter.ElementSize != sizeof(uint16) &&
        ...
        if (!Memory)
            return false;
        memset(Memory, 0, Parameters.Size);
        memcpy(reinterpret_cast<uint8*>(Memory) + InputOffset, &Value, InputSize);
        Target->ProcessEvent(Function, Memory);
        FMemory::Free(Memory);
        return true;
    }

    bool InvokeObjectInputReturningObject(UObject* Target, UFunction* Function,
        const void* InputValue, uint64 InputCastFlags, UObject*& OutValue)
    {
        OutValue = nullptr;
        if (!IsLiveEventObject(Target) || !InputValue || !ValidateParameterBuffer(Function))
        {
            return false;
        }
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            {
                ++ReturnCount;
                if (Parameter.ElementSize != sizeof(UObject*) || !HasParameterPropertyType(
                        Function, Parameter, CastClassObjectProperty))
                {
                    return false;
                }
                ReturnOffset = Parameter.Offset;
        ...
        if (InputCount != 1 || ReturnCount != 1 || InputOffset == InvalidParamOffset ||
            ReturnOffset == InvalidParamOffset || Parameters.Size == 0)
        {
            return false;
        }
        ...
        if (!Memory)
            return false;
        memset(Memory, 0, Parameters.Size);
        memcpy(reinterpret_cast<uint8*>(Memory) + InputOffset, InputValue, sizeof(UObject*));
        Target->ProcessEvent(Function, Memory);
        memcpy(&OutValue, reinterpret_cast<uint8*>(Memory) + ReturnOffset, sizeof(OutValue));
        FMemory::Free(Memory);
        return true;
    }

    bool InvokeObjectAndFloatInputs(UObject* Target, UFunction* Function, UObject* ObjectValue,
        float FloatValue)
    {
        if (!IsLiveEventObject(Target) || !IsLiveEventObject(ObjectValue) ||
            !ValidateParameterBuffer(Function))
        {
            return false;
        }
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            else if (Parameter.ElementSize == sizeof(float) && HasParameterPropertyType(
                    Function, Parameter, CastClassFloatProperty))
            {
        ...
        if (InputCount != 2 || ObjectOffset == InvalidParamOffset ||
            FloatOffset == InvalidParamOffset || Parameters.Size == 0)
        {
            return false;
        }
        ...
        if (!Memory)
            return false;
        memset(Memory, 0, Parameters.Size);
        memcpy(reinterpret_cast<uint8*>(Memory) + ObjectOffset, &ObjectValue, sizeof(ObjectValue));
        memcpy(reinterpret_cast<uint8*>(Memory) + FloatOffset, &FloatValue, sizeof(FloatValue));
        Target->ProcessEvent(Function, Memory);
        FMemory::Free(Memory);
        return true;
    }

    bool InvokeObjectInputReturningTransform(UObject* Target, UFunction* Function,
        UObject* InputValue, FTransform& OutValue)
    {
        OutValue = {};
        if (!IsLiveEventObject(Target) || !IsLiveEventObject(InputValue) ||
            !ValidateParameterBuffer(Function))
        {
            return false;
        }
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            {
                ++ReturnCount;
                if (Parameter.ElementSize != TransformSize || !HasParameterPropertyType(
                        Function, Parameter, CastClassStructProperty))
                {
                    return false;
                }
                ReturnOffset = Parameter.Offset;
        ...
        if (!Memory)
            return false;
        memset(Memory, 0, Parameters.Size);
        memcpy(reinterpret_cast<uint8*>(Memory) + InputOffset, &InputValue, sizeof(InputValue));
        Target->ProcessEvent(Function, Memory);
        memcpy(&OutValue, reinterpret_cast<uint8*>(Memory) + ReturnOffset, TransformSize);
        FMemory::Free(Memory);
        return
    ...
    bool InvokeFloatNoInput(UObject* Target, UFunction* Function, float& OutValue)
    {
        OutValue = 0.f;
        if (!IsLiveEventObject(Target) || !ValidateParameterBuffer(Function))
        {
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            if (!(Parameter.PropertyFlags & CPF_ReturnParm))
                continue;
            ++ReturnCount;
            if (Parameter.ElementSize != sizeof(float) || !HasParameterPropertyType(
                    Function, Parameter, CastClassFloatProperty))
            {
                return false;
            }
        ...
        if (ReturnCount != 1 || ReturnOffset == InvalidParamOffset || Parameters.Size == 0)
        {
            return false;
        }
    ...
    UObject* InvokeAddComponentByClass(AActor* Actor, const UClass* ComponentClass)
    {
        if (!IsLiveEventObject(Actor) || !IsLiveEventObject(ComponentClass))
        {
            return nullptr;
        }
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            ++InputCount;
            if (Parameter.Name == "Class" || Parameter.Name == "ComponentClass")
            {
                if (ClassOffset != InvalidParamOffset || Parameter.ElementSize != sizeof(UClass*) ||
                    !HasParameterPropertyType(Function, Parameter, CastClassClassProperty))
                {
                    return nullptr;
                }
            ...
            {
                if (Parameter.ElementSize != sizeof(uint8) || !HasParameterPropertyType(
                        Function, Parameter, CastClassBoolProperty))
                {
                    return nullptr;
                }
            ...
            {
                if (Parameter.ElementSize != FTransform::Size() || !HasParameterPropertyType(
                        Function, Parameter, CastClassStructProperty))
                {
                    return nullptr;
                }
        ...
        if (InputCount != 4 || ReturnCount != 1 || ClassOffset == InvalidParamOffset ||
            ManualAttachmentOffset == InvalidParamOffset || TransformOffset == InvalidParamOffset ||
            DeferredFinishOffset == InvalidParamOffset || ReturnOffset == InvalidParamOffset ||
            Parameters.Size == 0)
        {
            return nullptr;
        }
        ...
        memcpy(reinterpret_cast<uint8*>(Memory) + ClassOffset, &ClassValue, sizeof(ClassValue));
        ...
        memcpy(&Component, reinterpret_cast<uint8*>(Memory) + ReturnOffset, sizeof(Component));
        FMemory::Free(Memory);
        return IsLiveEventObject(Component) && Component->IsA(ComponentClass) ? Component : nullptr;
    }

    UObject* FindRiftTourComponent(AActor* Actor, const UClass* ComponentClass)
    {
        if (!IsLiveEventObject(Actor) || !IsLiveEventObject(ComponentClass))
        {
            return nullptr;
        ...
        UObject* Component = nullptr;
        auto GetComponentFunction = Actor->GetFunction("GetComponentByClass");
        const UClass* ClassValue = ComponentClass;
        if (InvokeObjectInputReturningObject(Actor, GetComponentFunction, &ClassValue,
                CastClassClassProperty, Component) && IsLiveEventObject(Component) &&
    ...
    UObject* EnsureRiftTourComponent(AActor* Actor, const UClass* ComponentClass)
    {
        auto Component = FindRiftTourComponent(Actor, ComponentClass);
        if (Component)
            return Component;
    ...
    void SetRiftTourComponentEnabled(UObject* Component, bool bEnabled)
    {
        ...
        if (bEnabled)
        {
            InvokeSingleInput(Component, Component->GetFunction("SetIsReplicated"), &Enabled,
                sizeof(Enabled), CastClassBoolProperty);
        }
        InvokeSingleInput(Component, Component->GetFunction("SetComponentTickEnabled"), &Enabled,
            sizeof(Enabled), CastClassBoolProperty);
        if (bEnabled)
        {
            InvokeSingleInput(Component, Component->GetFunction("Activate"), &Enabled,
                sizeof(Enabled), CastClassBoolProperty);
        }
        else
        {
            InvokeZeroed(Component, Component->GetFunction("Deactivate"));
        }
    }

    bool ResolveKiwiComponentClasses(const UClass*& OutControllerComponentClass,
        const UClass*& OutPawnComponentClass)
    {
        OutControllerComponentClass = nullptr;
        OutPawnComponentClass = nullptr;

        auto Mutator = FindLiveActor(L"/Script/KiwiPlaylistRuntime.FortAthenaMutator_Kiwi");
        if (!IsLiveEventObject(Mutator))
        {
            Mutator = FindLiveActor(L"/Script/KiwiRuntime.FortAthenaMutator_Kiwi");
        }
        if (!IsLiveEventObject(Mutator))
        {
            Mutator = FindLiveActor(L"/Script/FortniteGame.FortAthenaMutator_Kiwi");
        }
        if (IsLiveEventObject(Mutator))
        {
            ReadEventProperty(Mutator, "KiwiControllerComponentClass", CastClassObjectProperty,
                OutControllerComponentClass);
            ReadEventProperty(Mutator, "KiwiPawnComponentClass", CastClassObjectProperty,
        ...
        if (!IsLiveEventObject(OutControllerComponentClass))
        {
            ...
            {
                OutControllerComponentClass = FindObject<UClass>(ClassPath);
                if (IsLiveEventObject(OutControllerComponentClass))
                    break;
            }
        }
        if (!IsLiveEventObject(OutPawnComponentClass))
        {
            constexpr const wchar_t* PawnClassPaths[] =
            {
                ...
                L"/Script/KiwiRuntime.KiwiPawnComponent", L"/Script/FortniteGame.KiwiPawnComponent",
            };
    ...
    bool EnsureKiwiPlayerComponents(AFortPlayerControllerAthena* PlayerController,
        AFortPlayerPawnAthena* Pawn, const UClass* ControllerComponentClass,
        const UClass* PawnComponentClass)
    {
        if (!IsLiveEventObject(PlayerController) || !IsLiveEventObject(Pawn) ||
            !IsLiveEventObject(ControllerComponentClass) || !IsLiveEventObject(PawnComponentClass))
        {
            return false;
        }
        ...
        auto PawnComponent = EnsureRiftTourComponent(Pawn, PawnComponentClass);
        if (!IsLiveEventObject(ControllerComponent) || !IsLiveEventObject(PawnComponent))
        {
            return false;
        }
    ...
    bool TryGetKiwiPrisonStartTag(AActor* SquadStart, FGameplayTag& OutBlockTag)
    {
        OutBlockTag = {};
        FGameplayTagContainer Tags{};
        if (!ReadEventProperty(SquadStart, "GameplayTags", CastClassStructProperty, Tags) ||
            !IsSaneArray(Tags.GameplayTags.Data, Tags.GameplayTags.Num(), Tags.GameplayTags.Max(),
                FGameplayTag::Size(), 64))
        {
            return false;
        }
        ...
        {
            const auto& Tag = Tags.GameplayTags.Get(TagIndex, FGameplayTag::Size());
            const std::string TagName = Tag.TagName.ToString().c_str();
        ...
        return bIsPrisonStart && OutBlockTag.TagName.ToString().rfind("Kiwi.Prison.Block.", 0) == 0;
    }

    bool CollectKiwiPrisonStarts(const UClass* SquadStartClass, std::vector<AActor*>& OutStarts)
    {
        OutStarts.clear();
        ...
        if (IsSaneArray(Starts.Data, Starts.Num(), Starts.Max(), sizeof(AActor*), 1024))
        {
                 ...
                 StartIndex < Starts.Num(); ++StartIndex)
            {
                ...
                if (IsLiveEventObject(Start) && Start->IsA(SquadStartClass) &&
        ...
        Starts.Free();
        std::sort(OutStarts.begin(), OutStarts.end(), [](const AActor* Left, const AActor* Right)
            {
    ...
    bool EnsureKiwiPrisonStart(AFortPlayerControllerAthena* PlayerController,
        AFortPlayerPawnAthena* Pawn, const UClass* ControllerComponentClass, int32 PlayerIndex,
        bool bPlaceInChamber)
    {
        ...
        auto ControllerComponent = FindRiftTourComponent(
            PlayerController, ControllerComponentClass);
        auto SquadStartClass = FindObject<UClass>(L"/Script/FortniteGame.FortSquadStart");
        if (!IsLiveEventObject(ControllerComponent) || !IsLiveEventObject(SquadStartClass))
        {
            return false;
        }

        UObject* ExistingStart = nullptr;
        ReadEventProperty(ControllerComponent, "PrisonTeleportSquadStart", CastClassObjectProperty,
            ExistingStart);
        AActor* SelectedStart = IsLiveEventObject(ExistingStart) &&
            ExistingStart->IsA(SquadStartClass) ? static_cast<AActor*>(ExistingStart) : nullptr;

        bool bAssignedStart = false;
        if (!SelectedStart)
        {
            std::vector<AActor*> PrisonStarts;
            if (!CollectKiwiPrisonStarts(SquadStartClass, PrisonStarts))
            {
                return false;
            }
            const size_t StartIndex = static_cast<size_t>(PlayerIndex >= 0 ? PlayerIndex : 0) %
                PrisonStarts.size();
            SelectedStart = PrisonStarts[StartIndex];
            UObject* SelectedStartObject = SelectedStart;
            bAssignedStart = WriteEventProperty(ControllerComponent, "PrisonTeleportSquadStart",
                CastClassObjectProperty, SelectedStartObject);
            if (!bAssignedStart)
                return false;
        }
        ...
        {
            WriteEventProperty(ControllerComponent, "PrisonBlockIdentificationTag",
                CastClassStructProperty, BlockTag);
        }

        if (bAssignedStart)
        {
            SDK::DbgLog("[Events] Kiwi authored prison start assigned "
                "controller=%p component=%p start=%s block=%s\n",
                static_cast<void*>(PlayerController), static_cast<void*>(ControllerComponent),
                SelectedStart->Name.ToString().c_str(), BlockTag.TagName.ToString().c_str());
        }
        ...
        if (!TryGetEventObjectIdentity(Pawn, PawnIdentity))
            return false;
        if (HasTrackedIdentity(GEventState.KiwiPrisonPlayers, PawnIdentity))
        {
            return true;
        }

        FTransform PlayerStartTransform{};
        if (!InvokeObjectInputReturningTransform(SelectedStart,
                SelectedStart->GetFunction("GetPlayerStartTransform"), PlayerController,
                PlayerStartTransform))
        {
            return false;
        }

        const bool bPlaced = Pawn->K2_TeleportTo(PlayerStartTransform.Translation,
            PlayerStartTransform.Rotation.Rotator());
        if (!bPlaced)
            return false;

        InvokeZeroed(Pawn, Pawn->GetFunction("ForceNetUpdate"));
        InvokeZeroed(PlayerController, PlayerController->GetFunction("ForceNetUpdate"));
        GEventState.KiwiPrisonPlayers.push_back(PawnIdentity);
        SDK::DbgLog("[Events] Kiwi player placed at authored chamber "
    ...
    bool InvokeKiwiGameplayCueOperation(UAbilitySystemComponent* AbilitySystemComponent,
        const char* FunctionName, const FGameplayTag& CueTag,
        const FGameplayEffectContextHandle* EffectContext)
    {
        if (!IsLiveEventObject(AbilitySystemComponent) || !FunctionName)
        {
            return false;
        }

        auto Function = AbilitySystemComponent->GetFunction(FunctionName);
        if (!ValidateParameterBuffer(Function))
            return false;
        ...
        for (const auto& Parameter : Parameters.NameOffsetMap)
        {
            ...
            if (Parameter.Name.find("GameplayCueTag") != std::string::npos &&
                Parameter.ElementSize <= sizeof(FGameplayTag) && HasParameterPropertyType(Function,
                    Parameter, CastClassStructProperty))
            {
            ...
            if (EffectContext && Parameter.Name.find("EffectContext") != std::string::npos &&
                Parameter.ElementSize <= sizeof(FGameplayEffectContextHandle) &&
                HasParameterPropertyType(Function, Parameter, CastClassStructProperty))
            {
        ...
        if (InputCount != ExpectedInputs || CueTagOffset == InvalidParamOffset || CueTagSize == 0 ||
            (EffectContext && (EffectContextOffset == InvalidParamOffset ||
                 EffectContextSize == 0)) || Parameters.Size == 0)
        {
            return false;
        }
        ...
        if (!Memory)
            return false;
        memset(Memory, 0, Parameters.Size);
        memcpy(reinterpret_cast<uint8*>(Memory) + CueTagOffset, &CueTag, CueTagSize);
        if (EffectContext)
        {
            memcpy(reinterpret_cast<uint8*>(Memory) + EffectContextOffset, EffectContext,
                EffectContextSize);
        }
    ...
    bool RemoveKiwiInjectedCue(UAbilitySystemComponent* AbilitySystemComponent,
        const wchar_t* TagName)
    {
        const bool bRemoved = InvokeKiwiGameplayCueOperation(AbilitySystemComponent,
            "RemoveGameplayCue", MakeKiwiGameplayCueTag(TagName), nullptr);
    ...
    int32 RemoveKiwiLegacyLowGravityLayers(UAbilitySystemComponent* AbilitySystemComponent)
    {
        if (!IsLiveEventObject(AbilitySystemComponent))
            return 0;

        RemoveKiwiInjectedCue(AbilitySystemComponent,
            L"GameplayCue.Skyfire.Phase.Arena.LowGrav.Trail");
        RemoveKiwiInjectedCue(AbilitySystemComponent, L"GameplayCue.Athena.Alpaca.Loop");
        ...
        {
            auto& Effect = Effects.Get(EffectIndex, FActiveGameplayEffect::Size());
            if (!Effect.HasSpec() || !Effect.Spec.HasDef())
                continue;
            if (!Effect.Spec.Def || !Effect.Spec.Def->IsA(LegacyEffectClass))
            {
                continue;
            }

            const auto Handle = *reinterpret_cast<FActiveGameplayEffectHandle*>(
                    reinterpret_cast<uint8*>(&Effect) + 0xc);
            if (Handle.Handle > 0)
                HandlesToRemove.push_back(Handle);
        }

        auto RemoveFunction = AbilitySystemComponent->GetFunction("RemoveActiveGameplayEffect");
        ...
        {
            if (AbilitySystemComponent->Call<bool>(RemoveFunction, Handle, -1))
            {
                ++RemovedEffects;
            }
        }
    ...
    bool SetKiwiVortexActive(FKiwiVortexState& State, AFortPlayerPawnAthena* Pawn, bool bActive)
    {
        ...
        {
            auto PawnObject = ResolveEventObjectIdentity(State.PawnIdentity);
        ...
        if (!IsLiveEventObject(Pawn) || !IsLiveEventObject(Pawn->GetFunction("SetInVortex")))
        {
            if (!bActive)
                State.bActive = false;
            return false;
        ...
        if (bActive && IsLiveEventObject(Pawn->CharacterMovement) &&
        ...
        if (!bActive)
        {
            const bool bStillSkydiving = (Pawn->HasbIsSkydiving() && Pawn->bIsSkydiving) ||
                (Pawn->HasbIsSkydivingFromBus() && Pawn->bIsSkydivingFromBus);
            if (bStillSkydiving)
                InvokeZeroed(Pawn, Pawn->GetFunction("EndSkydiving"));
        }
        Pawn->ForceNetUpdate();
        State.bActive = bActive;
        SDK::DbgLog("[Events] Kiwi island vortex skydive %s pawn=%p "
            "surfaceZ=%.1f\n", bActive ? "enabled" : "disabled", static_cast<void*>(Pawn),
            static_cast<double>(State.IslandSurfaceZ));
        return true;
    }

    bool ApplyKiwiLowGravity(AFortPlayerControllerAthena* PlayerController,
        AFortPlayerPawnAthena* Pawn)
    {
        if (!IsLiveEventObject(PlayerController) || !IsLiveEventObject(Pawn) ||
            !IsLiveEventObject(PlayerController->PlayerState) || !IsLiveEventObject(
        ...
        FEventObjectIdentity AbilitySystemIdentity{};
        if (!TryGetEventObjectIdentity(AbilitySystemComponent, AbilitySystemIdentity))
        {
            return false;
        }
        const bool bHasGravityEffect = std::find_if(GEventState.KiwiLowGravityEffects.begin(),
                GEventState.KiwiLowGravityEffects.end(), [&AbilitySystemIdentity](
                    const FTrackedGameplayEffect& Effect)
                {
                    return Effect.AbilitySystemIdentity == AbilitySystemIdentity;
                }) != GEventState.KiwiLowGravityEffects.end();
        if (bHasGravityEffect)
            return true;

        const int32 RemovedLegacyEffects = RemoveKiwiLegacyLowGravityLayers(AbilitySystemComponent);

        auto GameplayEffect = FindObject<UClass>(L"/MotherGameplay/Items/Alpaca/"
        ...
        SDK::DbgLog("[Events] Kiwi island low gravity applied "
            "controller=%p pawn=%p handle=%d effect=%p "
            "removedLegacy=%d\n", static_cast<void*>(PlayerController), static_cast<void*>(Pawn),
            Handle.Handle, static_cast<const void*>(GameplayEffect), RemovedLegacyEffects);
        return true;
    }
    ...
    void CleanupKiwiLowGravity()
    {
        ...
        {
            if (VortexState.bActive && SetKiwiVortexActive(VortexState, nullptr, false))
                ++DisabledVortexes;
        }
        GEventState.KiwiVortexPlayers.clear();

        int32 RemovedEffects = 0;
        for (const auto& TrackedEffect : GEventState.KiwiLowGravityEffects)
        {
            auto AbilitySystemObject = ResolveEventObjectIdentity(
                TrackedEffect.AbilitySystemIdentity);
            if (!IsLiveEventObject(AbilitySystemObject) || TrackedEffect.Handle.Handle <= 0)
            {
                continue;
            }

            auto AbilitySystemComponent = static_cast<UAbilitySystemComponent*>(
                    AbilitySystemObject);
            RemoveKiwiLegacyLowGravityLayers(AbilitySystemComponent);
            auto RemoveFunction = AbilitySystemComponent->GetFunction("RemoveActiveGameplayEffect");
            if (IsLiveEventObject(RemoveFunction) && AbilitySystemComponent->Call<bool>(
                    RemoveFunction, TrackedEffect.Handle, -1))
            {
                ++RemovedEffects;
            }
        }
        if (!GEventState.KiwiLowGravityEffects.empty() || DisabledVortexes > 0)
        {
            SDK::DbgLog("[Events] Kiwi island low gravity removed "
                "effects=%d vortexes=%d tracked=%zu\n", RemovedEffects, DisabledVortexes,
    ...
    void DestroyRiftTourComponent(UObject* Component)
    {
        ...
        auto DestroyFunction = Component->GetFunction("K2_DestroyComponent");
        UObject* ComponentObject = Component;
        if (!InvokeSingleInput(Component, DestroyFunction, &ComponentObject,
                sizeof(ComponentObject), CastClassObjectProperty))
        {
    ...
    void StopRiftTourSplineMovement(UObject* MovementComponent)
    {
        ...
        WriteEventProperty(MovementComponent, "bIsMovingAlongSpline", CastClassBoolProperty,
            IsMoving);
        InvokeSingleInput(MovementComponent,
            MovementComponent->GetFunction("SetIsMovingAlongSpline"), &IsMoving, sizeof(IsMoving),
            CastClassBoolProperty);
        InvokeZeroed(MovementComponent, MovementComponent->GetFunction(
    ...
    bool ConfigureRiftTourSlide(double Now)
    {
        ...
        if (!Script || !ReadEventProperty(Script, "SplineActor", CastClassObjectProperty,
                SplineActor) || !IsLiveEventObject(SplineActor) || !WriteEventProperty(Script,
                "PawnLocation", CastClassObjectProperty, SplineActor))
        {
            return false;
        }
        ...
        if (!IsLiveEventObject(PlayerComponentClass) ||
            ...
            !IsLiveEventObject(GEventState.GameMode) || !GEventState.GameMode->HasAlivePlayers())
        {
            return false;
        }

        auto& Players = GEventState.GameMode->AlivePlayers;
        if (!IsSaneArray(Players.Data, Players.Num(), Players.Max(), sizeof(AActor*), 256))
        {
            return false;
        }
        ...
        {
            InvokeFloatNoInput(GEventState.GameState, GEventState.GameState->GetFunction(
                    "GetServerWorldTimeSeconds"), StartServerWorldTime);
        }

        const UClass* ControllerClass = AFortPlayerControllerAthena::StaticClass();
        int32 TargetPlayers = 0;
        int32 ConfiguredPlayers = 0;
        for (int32 PlayerIndex = 0;
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            auto PlayerController = static_cast<AFortPlayerControllerAthena*>(Players[PlayerIndex]);
            if (!IsLiveEventObject(PlayerController) || !ControllerClass ||
            ...
            if (!TryGetEventObjectIdentity(Pawn, PawnIdentity))
                continue;
            ++TargetPlayers;
            if (HasTrackedIdentity(GEventState.RiftTourSlidePlayers, PawnIdentity))
            {
                auto MovementComponent = FindRiftTourComponent(Pawn, MovementComponentClass);
                if (IsLiveEventObject(MovementComponent))
                {
                    const uint8 IsMoving = 1;
                    SetRiftTourComponentEnabled(MovementComponent, true);
                    WriteEventProperty(MovementComponent, "bIsMovingAlongSpline",
                        CastClassBoolProperty, IsMoving);
                    InvokeSingleInput(MovementComponent, MovementComponent->GetFunction(
                            "SetIsMovingAlongSpline"), &IsMoving, sizeof(IsMoving),
                        CastClassBoolProperty);
                    ++ConfiguredPlayers;
                }
                continue;
            ...
            auto PlayerComponent = EnsureRiftTourComponent(Pawn, PlayerComponentClass);
            auto MovementComponent = EnsureRiftTourComponent(Pawn, MovementComponentClass);
            if (!IsLiveEventObject(PlayerComponent) || !IsLiveEventObject(MovementComponent))
            {
                continue;
            }
            ...
            InvokeObjectInputReturningObject(Script, Script->GetFunction("GetWrapManagerForPlayer"),
                &PawnObject, CastClassObjectProperty, WrapManager);

            const float TotalSplineTime = 59.793846f;
            const float SplineInterpStrength = 1.3f;
            UObject* SplineObject = SplineActor;
            const uint8 IsMoving = 1;
            const bool bConfigured = WriteEventProperty(PlayerComponent, "OwningPlayerController",
                    CastClassObjectProperty, PlayerController) && WriteEventProperty(
                    MovementComponent, "bIsMovingAlongSpline", CastClassBoolProperty, IsMoving) &&
                WriteEventProperty(MovementComponent, "ReplicatedTotalSplineTime",
                    CastClassFloatProperty, TotalSplineTime) && WriteEventProperty(
                    MovementComponent, "TargetSplineActor", CastClassObjectProperty, SplineActor) &&
                InvokeSingleInput(MovementComponent,
                    MovementComponent->GetFunction("SetSplineActor"), &SplineObject,
                    sizeof(SplineObject), CastClassObjectProperty) && WriteEventProperty(
                    MovementComponent, "ReplicatedSplineInterpStrength", CastClassFloatProperty,
                    SplineInterpStrength) && WriteEventProperty(MovementComponent,
                    "StartServerWorldTime", CastClassFloatProperty, StartServerWorldTime) &&
                InvokeSingleInput(MovementComponent, MovementComponent->GetFunction(
                        "SetIsMovingAlongSpline"), &IsMoving, sizeof(IsMoving),
                    CastClassBoolProperty) && InvokeZeroed(MovementComponent,
                    MovementComponent->GetFunction("OnRep_TargetSplineActor")) &&
                WriteEventProperty(PlayerComponent, "MovementComponent", CastClassObjectProperty,
                    MovementComponent) && InvokeSmallIntegerInput(Pawn,
                    Pawn->GetFunction("SetStasisMode"), 3);
            if (!bConfigured)
                continue;
            InvokeZeroed(MovementComponent, MovementComponent->GetFunction(
                    "OnRep_bIsMovingAlongSpline"));
            ...
            {
                WriteEventProperty(PlayerComponent, "WrapManager", CastClassObjectProperty,
                    WrapManager);
            }
            InvokeZeroed(Pawn, Pawn->GetFunction("ForceNetUpdate"));
            InvokeZeroed(PlayerController, PlayerController->GetFunction("ForceNetUpdate"));
            GEventState.RiftTourSlidePlayers.push_back(PawnIdentity);
            ++ConfiguredPlayers;
            SDK::DbgLog("[Events] Rift Tour slide attached controller=%p "
                "pawn=%p spline=%p playerComponent=%p "
                "movementComponent=%p wrapManager=%p start=%.3f\n",
                static_cast<void*>(PlayerController), static_cast<void*>(Pawn),
                static_cast<void*>(SplineActor), static_cast<void*>(PlayerComponent),
                static_cast<void*>(MovementComponent), static_cast<void*>(WrapManager),
                StartServerWorldTime);
        }
        return TargetPlayers > 0 && ConfiguredPlayers == TargetPlayers;
    }
    ...
    bool PrimeRiftTourStarsGoal(AActor* Goal, bool* OutTickEnabled = nullptr,
        bool* OutPositionPrimed = nullptr, AActor** OutStarsCamera = nullptr)
    {
        if (OutTickEnabled)
        ...
        const bool bTickEnabled = InvokeSingleInput(Goal, Goal->GetFunction("SetActorTickEnabled"),
            &TickEnabled, sizeof(TickEnabled), CastClassBoolProperty);
        const float DeltaSeconds = 0.f;
        const bool bPositionPrimed = InvokeSingleInput(Goal, Goal->GetFunction("ReceiveTick"),
            &DeltaSeconds, sizeof(DeltaSeconds), CastClassFloatProperty);
        AActor* StarsCamera = nullptr;
        ReadEventProperty(Goal, "StarsCamera", CastClassObjectProperty, StarsCamera);
        if (OutTickEnabled)
    ...
    AActor* EnsureRiftTourStarsGoal(const FVector& InitialLocation)
    {
        ...
        auto TrackedGoal = ResolveEventObjectIdentity(GEventState.RiftTourStarsFallbackGoal);
        if (IsLiveEventObject(TrackedGoal) && TrackedGoal->IsA(AActor::StaticClass()))
        {
        ...
        Goal = UWorld::SpawnActor<AActor>(GoalClass, InitialLocation, {}, GEventState.Script);
        FEventObjectIdentity GoalIdentity{};
        if (!IsLiveEventObject(Goal) || !Goal->IsA(GoalClass) ||
        ...
        AActor* StarsCamera = nullptr;
        PrimeRiftTourStarsGoal(Goal, &bTickEnabled, &bPositionPrimed, &StarsCamera);
        InvokeZeroed(Goal, Goal->GetFunction("ForceNetUpdate"));
        SDK::DbgLog("[Events] Rift Tour stars authored goal fallback spawned "
            "goal=%p camera=%p tick=%d primed=%d\n", static_cast<void*>(Goal),
            static_cast<void*>(StarsCamera), bTickEnabled ? 1 : 0, bPositionPrimed ? 1 : 0);
        return Goal;
    }

    bool HasRiftTourStarsEffect(const FEventObjectIdentity& AbilitySystemIdentity)
    {
        return std::find_if(GEventState.RiftTourStarsEffects.begin(),
    ...
    bool ApplyRiftTourStarsEffect(AFortPlayerControllerAthena* PlayerController,
        AFortPlayerPawnAthena* Pawn)
    {
        if (!IsLiveEventObject(PlayerController) || !IsLiveEventObject(Pawn) ||
            !IsLiveEventObject(PlayerController->PlayerState) || !IsLiveEventObject(
        ...
        GEventState.RiftTourStarsEffects.push_back(
            { AbilitySystemIdentity, Handle });
        SDK::DbgLog("[Events] Rift Tour stars authored pull effect applied "
            "controller=%p pawn=%p asc=%p handle=%d\n", static_cast<void*>(PlayerController),
            static_cast<void*>(Pawn), static_cast<void*>(AbilitySystemComponent), Handle.Handle);
        return true;
    }

    bool EnsureRiftTourStarsPullAbility(UAbilitySystemComponent* AbilitySystemComponent)
    {
        ...
        if (!IsLiveEventObject(AbilitySystemComponent) || !IsLiveEventObject(PullAbilityClass) ||
        ...
        if (AbilitySpecSize <= 0 || AbilitySpecSize > 0x400 || !IsSaneArray(AbilitySpecs.Data,
                AbilitySpecs.Num(), AbilitySpecs.Max(), static_cast<size_t>(AbilitySpecSize), 2048))
        {
            return false;
        }
        ...
        {
            auto& AbilitySpec = AbilitySpecs.Get(AbilityIndex, AbilitySpecSize);
            if (!IsLiveEventObject(AbilitySpec.Ability) ||
                !AbilitySpec.Ability->IsA(PullAbilityClass) || AbilitySpec.Handle.Handle <= 0)
            {
                continue;
            }

            if (AbilitySpec.HasActiveCount() && AbilitySpec.ActiveCount > 0)
            {
                return true;
            }
        ...
        auto PredictionKey = static_cast<FPredictionKey*>(FMemory::Malloc(PredictionKeySize));
        if (!PredictionKey)
            return false;
        memset(PredictionKey, 0, PredictionKeySize);
        UAbilitySystemComponent::InternalServerTryActivateAbility(AbilitySystemComponent,
            PullHandle, true, PredictionKey, nullptr);
        FMemory::Free(PredictionKey);

        if (!IsSaneArray(AbilitySpecs.Data, AbilitySpecs.Num(), AbilitySpecs.Max(),
                static_cast<size_t>(AbilitySpecSize), 2048))
        {
            return false;
        }
             ...
             AbilityIndex < AbilitySpecs.Num(); ++AbilityIndex)
        {
            ...
            const bool bActive = !AbilitySpec.HasActiveCount() || AbilitySpec.ActiveCount > 0;
            if (bActive)
            {
                SDK::DbgLog("[Events] Rift Tour stars authored pull ability "
    ...
    bool ConfigureRiftTourStars(double PhaseElapsed)
    {
        ...
        if (!IsLiveEventObject(Script) || !IsLiveEventObject(StarsComponentClass) ||
            !IsLiveEventObject(MovementMutatorClass) || !IsLiveEventObject(GEventState.GameMode) ||
             ...
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            ...
            if (!FindRiftTourComponent(PlayerController->MyFortPawn, StarsComponentClass))
            {
        ...
        if (bMissingAuthoredComponent && !GEventState.bRiftTourStarsServerActivationInvoked)
        {
            GEventState.bRiftTourStarsServerActivationInvoked = InvokeZeroed(Script,
                    Script->GetFunction("OnPhaseActivation_Server"));
            SDK::DbgLog("[Events] Rift Tour stars authored server activation "
                "repair invoked=%d script=%p\n", GEventState.bRiftTourStarsServerActivationInvoked
                    ? 1 : 0, static_cast<void*>(Script));
        }
        if (!GEventState.bRiftTourStarsClientActivationInvoked)
        {
            GEventState.bRiftTourStarsClientActivationInvoked = InvokeZeroed(Script,
                    Script->GetFunction("OnPhaseActivation_Client"));
            SDK::DbgLog("[Events] Rift Tour stars authored client activation "
             ...
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            ...
            if (IsLiveEventObject(PlayerController) &&
                IsLiveEventObject(PlayerController->MyFortPawn))
            {
                GoalSpawnLocation = PlayerController->MyFortPawn->K2_GetActorLocation();
                break;
            }
        }
        AActor* Goal = nullptr;
             ...
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            ...
            auto StarsComponent = EnsureRiftTourComponent(Pawn, StarsComponentClass);
            auto MovementMutator = EnsureRiftTourComponent(Pawn, MovementMutatorClass);
            if (!IsLiveEventObject(StarsComponent) || !IsLiveEventObject(MovementMutator))
            {
                continue;
            }
            SetRiftTourComponentEnabled(StarsComponent, true);
            SetRiftTourComponentEnabled(MovementMutator, true);
            InvokeSmallIntegerInput(Pawn, Pawn->GetFunction("SetStasisMode"), 3);

            WriteEventProperty(StarsComponent, "OwningPawn", CastClassObjectProperty, Pawn);
            InvokeZeroed(StarsComponent, StarsComponent->GetFunction(
                    "TryToInitializeWithPhaseScript"));
            InvokeZeroed(StarsComponent, StarsComponent->GetFunction("TryFindPlayerLocation"));
            AActor* PlayerLocation = nullptr;
            if (ReadEventProperty(StarsComponent, "PlayerLocation", CastClassObjectProperty,
                    PlayerLocation) && IsLiveEventObject(PlayerLocation))
            {
                ++AnchoredPlayers;
            }

            InvokeZeroed(Pawn, Pawn->GetFunction("ForceNetUpdate"));
            if (!HasTrackedIdentity(GEventState.RiftTourStarsPlayers, PawnIdentity))
            {
                GEventState.RiftTourStarsPlayers.push_back(PawnIdentity);
                SDK::DbgLog("[Events] Rift Tour stars movement attached "
                    "controller=%p pawn=%p stars=%p mutator=%p\n",
                    static_cast<void*>(PlayerController), static_cast<void*>(Pawn),
                    static_cast<void*>(StarsComponent), static_cast<void*>(MovementMutator));
            }
            ++ConfiguredPlayers;
            if (IsLiveEventObject(Goal) && ApplyRiftTourStarsEffect(PlayerController, Pawn))
            {
                ++EffectPlayers;
                auto AbilitySystemComponent = PlayerController->PlayerState
                        ? PlayerController->PlayerState->AbilitySystemComponent : nullptr;
                if (EnsureRiftTourStarsPullAbility(AbilitySystemComponent))
                {
                    ++PullPlayers;
                }

                if (!HasTrackedIdentity(GEventState.RiftTourStarsFloatingPlayers, PawnIdentity) &&
                    InvokeZeroed(StarsComponent, StarsComponent->GetFunction("StartFloating")))
                {
                    GEventState.RiftTourStarsFloatingPlayers.push_back(PawnIdentity);
                }
                if (HasTrackedIdentity(GEventState.RiftTourStarsFloatingPlayers, PawnIdentity))
                {
                    ++FloatingPlayers;
        ...
        if (TargetPlayers == 0 || ConfiguredPlayers != TargetPlayers ||
        ...
        {
            GEventState.bRiftTourStarsFloatingStarted = InvokeZeroed(Script,
                    Script->GetFunction("StartFloating"));
            SDK::DbgLog("[Events] Rift Tour stars floating cue catch-up "
                "invoked=%d elapsed=%.3f script=%p goal=%p\n",
                GEventState.bRiftTourStarsFloatingStarted ? 1 : 0, PhaseElapsed,
                static_cast<void*>(Script), static_cast<void*>(Goal));
        }

        return GEventState.bRiftTourStarsClientActivationInvoked &&
            GEventState.bRiftTourStarsFloatingStarted && ConfiguredPlayers == TargetPlayers &&
            AnchoredPlayers == TargetPlayers && EffectPlayers == TargetPlayers &&
            PullPlayers == TargetPlayers && FloatingPlayers == TargetPlayers;
    }
    ...
    AActor* FindRiftTourReflectScript()
    {
        ...
        {
            auto Script = const_cast<AActor*>(FindObject<AActor>(Path));
            if (IsLiveEventObject(Script))
                return Script;
        }
        return nullptr;
    ...
    bool ApplyRiftTourBubbleRelevancy()
    {
        ...
        {
            "Set Bubble Player Relevancy", "SetBubblePlayerRelevancy",
        };
        for (const auto FunctionName : FunctionNames)
        {
            ...
            SDK::DbgLog("[Events] Rift Tour bubble relevancy applied "
    ...
    bool ConfigureRiftTourBubbles()
    {
        ...
        if (!IsLiveEventObject(BubbleMovementClass) || !IsLiveEventObject(GEventState.GameMode) ||
            !GEventState.GameMode->HasAlivePlayers())
        {
            return false;
        }
             ...
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            ...
            auto BubbleMovement = FindRiftTourComponent(Pawn, BubbleMovementClass);

            UObject* AuthoredSpline = nullptr;
            if (!IsLiveEventObject(BubbleMovement) || !ReadEventProperty(BubbleMovement,
                    "SplineComponent", CastClassObjectProperty, AuthoredSpline) ||
                !IsLiveEventObject(AuthoredSpline))
            {
                continue;
            }

            const bool bWasTracked = HasTrackedIdentity(GEventState.RiftTourBubblePlayers,
                PawnIdentity);
            if (!bWasTracked)
            {
                ...
                uint8 IsMoving = 0;
                ReadEventProperty(BubbleMovement, "StartServerWorldTime", CastClassFloatProperty,
                    StartServerWorldTime);
                ReadEventProperty(BubbleMovement, "ReplicatedTotalSplineTime",
                    CastClassFloatProperty, TotalSplineTime);
                ReadEventProperty(BubbleMovement, "bIsMovingAlongSpline", CastClassBoolProperty,
                    IsMoving);
                GEventState.RiftTourBubblePlayers.push_back(PawnIdentity);
                SDK::DbgLog("[Events] Rift Tour authored bubble movement observed "
                    "controller=%p pawn=%p movement=%p spline=%p "
                    "start=%.3f duration=%.3f moving=%d\n", static_cast<void*>(PlayerController),
                    static_cast<void*>(Pawn), static_cast<void*>(BubbleMovement),
                    static_cast<void*>(AuthoredSpline), StartServerWorldTime, TotalSplineTime,
                    IsMoving ? 1 : 0);
            }
            ++ConfiguredPlayers;
        }
    ...
    bool IsNearRiftTourEscherStart(const FVector& PawnLocation,
        const std::vector<AActor*>& StartReferences)
    {
        ...
        for (auto StartReference : StartReferences)
        {
            if (!IsLiveEventObject(StartReference))
                continue;
            if ((PawnLocation - StartReference->K2_GetActorLocation()).SizeSquared() <=
    ...
    bool ConfigureRiftTourEscher(double PhaseElapsed)
    {
        ...
        if (!ReadEventProperty(Script, "StartingTeleportRefs", 0, AuthoredReferences) ||
            !IsSaneArray(AuthoredReferences.Data, AuthoredReferences.Num(),
                AuthoredReferences.Max(), sizeof(AActor*), 64))
        {
            return false;
        }
             ...
             Index < AuthoredReferences.Num(); ++Index)
        {
            ...
            if (!IsLiveEventObject(StartReference))
                continue;
            const FVector Location = StartReference->K2_GetActorLocation();
        ...
        const auto ReferenceCount = static_cast<double>(StartReferences.size());
             ...
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            ...
            ++TargetPlayers;
            if (IsNearRiftTourEscherStart(PlayerController->MyFortPawn->K2_GetActorLocation(),
                    StartReferences))
            {
                ++PlacedPlayers;
            }
        ...
        {
            GEventState.bRiftTourEscherTeleportInvoked = InvokeZeroed(Script,
                    Script->GetFunction("Teleport Players"));
            SDK::DbgLog("[Events] Rift Tour Escher authored teleport "
                "catch-up invoked=%d script=%p refs=%d\n",
                GEventState.bRiftTourEscherTeleportInvoked ? 1 : 0, static_cast<void*>(Script),
             ...
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            ...
            auto Pawn = PlayerController->MyFortPawn;
            if (!IsNearRiftTourEscherStart(Pawn->K2_GetActorLocation(), StartReferences))
            {
                FVector Destination = Center;
                FRotator DestinationRotation = Rotation;
                if (TargetPlayers > 1)
                {
                    auto StartReference = StartReferences[static_cast<size_t>(ValidPlayerIndex) %
                        StartReferences.size()];
                    Destination = StartReference->K2_GetActorLocation();
                    DestinationRotation = StartReference->K2_GetActorRotation();
                }
                const bool bTeleported = Pawn->K2_TeleportTo(Destination, DestinationRotation);
                InvokeZeroed(Pawn, Pawn->GetFunction("ForceNetUpdate"));
                InvokeZeroed(PlayerController, PlayerController->GetFunction("ForceNetUpdate"));
                SDK::DbgLog("[Events] Rift Tour Escher live-reference "
                    "fallback controller=%p pawn=%p success=%d "
                    "destination=(%.3f, %.3f, %.3f)\n", static_cast<void*>(PlayerController),
                    static_cast<void*>(Pawn), bTeleported ? 1 : 0,
                    static_cast<double>(Destination.X), static_cast<double>(Destination.Y),
    ...
    void CleanupRiftTourSlide()
    {
        ...
        for (const auto& PawnIdentity : GEventState.RiftTourSlidePlayers)
        {
            auto PawnObject = ResolveEventObjectIdentity(PawnIdentity);
            if (!IsLiveEventObject(PawnObject) || !PawnObject->IsA(AActor::StaticClass()))
            {
                continue;
            }

            auto Pawn = static_cast<AActor*>(PawnObject);
            auto PlayerComponent = FindRiftTourComponent(Pawn, PlayerComponentClass);
            auto MovementComponent = FindRiftTourComponent(Pawn, MovementComponentClass);
            InvokeSmallIntegerInput(Pawn, Pawn->GetFunction("SetStasisMode"), 0);
        ...
        SDK::DbgLog("[Events] Rift Tour slide components removed players=%d\n", CleanedPlayers);
    ...
    {
        int32 RemovedEffects = 0;
        for (const auto& TrackedEffect : GEventState.RiftTourStarsEffects)
        {
            auto AbilitySystemObject = ResolveEventObjectIdentity(
                TrackedEffect.AbilitySystemIdentity);
            if (!IsLiveEventObject(AbilitySystemObject) || TrackedEffect.Handle.Handle <= 0)
        ...
        auto StarsComponentClass = FindObject<UClass>(
            L"/Buffet/Gameplay/Blueprints/Stars/BP_Buffet_Stars_PlayerComponent.BP_Buffet_Stars_PlayerComponent_C");
        int32 CleanedPlayers = 0;
        for (const auto& PawnIdentity : GEventState.RiftTourStarsPlayers)
        {
            auto PawnObject = ResolveEventObjectIdentity(PawnIdentity);
            if (!IsLiveEventObject(PawnObject) || !PawnObject->IsA(AActor::StaticClass()))
            {
            ...
            InvokeSmallIntegerInput(Pawn, Pawn->GetFunction("SetStasisMode"), 0);
            DestroyRiftTourComponent(FindRiftTourComponent(Pawn, StarsComponentClass));
            InvokeZeroed(Pawn, Pawn->GetFunction("ForceNetUpdate"));
            ++CleanedPlayers;
        }

        SDK::DbgLog("[Events] Rift Tour stars component removed; shared movement "
            "preserved players=%d effects=%d\n", CleanedPlayers, RemovedEffects);
        GEventState.RiftTourStarsPlayers.clear();
        GEventState.RiftTourStarsFloatingPlayers.clear();
        bool bDestroyedFallbackGoal = false;
        auto FallbackGoalObject = ResolveEventObjectIdentity(GEventState.RiftTourStarsFallbackGoal);
        if (IsLiveEventObject(FallbackGoalObject) && FallbackGoalObject->IsA(AActor::StaticClass()))
        {
        ...
        SDK::DbgLog("[Events] Rift Tour stars fallback goal removed=%d\n",
            bDestroyedFallbackGoal ? 1 : 0);
    }

    void CleanupRiftTourBubbles()
    {
        SDK::DbgLog("[Events] Rift Tour released authored bubble tracking players=%d\n",
            static_cast<int32>(GEventState.RiftTourBubblePlayers.size()));
    ...
    void TickRiftTourPhaseRepair(double Now, int32 CurrentPhase)
    {
        ...
        if (CurrentPhase > 2 && !GEventState.RiftTourSlidePlayers.empty())
        {
            CleanupRiftTourSlide();
        }
        if (CurrentPhase > 6 && (!GEventState.RiftTourStarsPlayers.empty() ||
             !GEventState.RiftTourStarsEffects.empty() ||
             GEventState.RiftTourStarsFallbackGoal.ObjectIndex >= 0))
        {
            CleanupRiftTourStars();
        }
        if (CurrentPhase > 8 && !GEventState.RiftTourBubblePlayers.empty())
        {
            CleanupRiftTourBubbles();
        }

        const double PhaseElapsed = Now - GEventState.RiftTourPhaseEnteredTime;
        if (CurrentPhase == 7 && !GEventState.bRiftTourBubbleRelevancyApplied &&
            PhaseElapsed >= 26.0)
        {
            GEventState.bRiftTourBubbleRelevancyApplied = ApplyRiftTourBubbleRelevancy();
        }
        if ((CurrentPhase != 2 && CurrentPhase != 6 && CurrentPhase != 8 && CurrentPhase != 10) ||
            Now < GEventState.NextRiftTourRepairTime)
        {
            return;
        }
        if ((CurrentPhase == 2 && GEventState.bRiftTourSlideRepairComplete) || (CurrentPhase == 6 &&
                 GEventState.bRiftTourStarsRepairComplete) || (CurrentPhase == 8 &&
                 GEventState.bRiftTourBubbleRepairComplete) || (CurrentPhase == 10 &&
        ...
        if (CurrentPhase == 2)
        {
            RepairComplete = &GEventState.bRiftTourSlideRepairComplete;
            PhaseName = "slide";
            bComplete = ConfigureRiftTourSlide(Now);
        }
        else if (CurrentPhase == 6)
        {
            RepairComplete = &GEventState.bRiftTourStarsRepairComplete;
            PhaseName = "stars";
            bComplete = ConfigureRiftTourStars(PhaseElapsed);
        }
        else if (CurrentPhase == 8)
        {
            RepairComplete = &GEventState.bRiftTourBubbleRepairComplete;
            PhaseName = "bubbles";
            bComplete = ConfigureRiftTourBubbles();
        }
        else
        {
            RepairComplete = &GEventState.bRiftTourEscherRepairComplete;
            PhaseName = "Escher";
            bComplete = ConfigureRiftTourEscher(PhaseElapsed);
        }

        GEventState.NextRiftTourRepairTime = Now + (*RepairComplete ? 1.0 : 0.25);
        if (bComplete && !*RepairComplete)
        {
            *RepairComplete = true;
            SDK::DbgLog("[Events] Rift Tour phase %d %s repair complete\n", CurrentPhase,
                PhaseName);
        }
        else if (!bComplete && Now >= GEventState.NextRiftTourRepairLogTime)
        {
            GEventState.NextRiftTourRepairLogTime = Now + 2.0;
            SDK::DbgLog("[Events] Rift Tour phase %d waiting for authored "
    ...
    UObject* GetFunctionTarget(const FEventFunction& EventFunction,
        const FResolvedEventActors& Actors)
    {
        return EventFunction.bIsLoaderFunction ? static_cast<UObject*>(Actors.Loader)
    ...
    bool ResolveEventContext(FEventContext& OutContext)
    {
        OutContext = {};
        auto World = UWorld::GetWorld();
        if (!IsLiveEventObject(World) || !World->HasAuthorityGameMode())
        {
            return false;
        }
        ...
        const UFortPlaylistAthena* Playlist = nullptr;
        const UStruct* PlaylistInfoStruct = FPlaylistPropertyArray::StaticStruct();
        if (GameState->HasCurrentPlaylistInfo() && PlaylistInfoStruct &&
            FPlaylistPropertyArray::HasBasePlaylist())
        {
            Playlist = GameState->CurrentPlaylistInfo.BasePlaylist;
        }
        if (!IsLiveEventObject(Playlist) && GameState->HasCurrentPlaylistData())
        {
            Playlist = GameState->CurrentPlaylistData;
        }
        if (!IsLiveEventObject(Playlist))
        {
            // Some dumps (18.40) omit both published-playlist slots; use the configured asset.
    ...
    void ResetEventState(const FEventContext& Context)
    {
        ...
        if (GPreparedLoaderWorld != Context.World || GPreparedLoaderGameMode != Context.GameMode ||
        ...
        FConfiguration::bEventStarted.store(false, std::memory_order_release);
    }
    ...
    bool PlaylistMatchesEvent(const FEvent& Event, const FEventContext& Context)
    {
        if (!Event.PlaylistPath)
            return true;
        auto Expected = FindObject<UFortPlaylistAthena>(Event.PlaylistPath);
        if (IsLiveEventObject(Expected) && IsLiveEventObject(Context.Playlist) &&
            (Expected == Context.Playlist || Expected->Name == Context.Playlist->Name))
        {
            return true;
        }

        const bool bConfiguredLegacyPlaylist = Event.EventVersion <= 14.60 &&
            FConfiguration::Playlist && wcscmp(FConfiguration::Playlist, Event.PlaylistPath) == 0;
        if (bConfiguredLegacyPlaylist)
        {
            SDK::DbgLog("[Events] using configured legacy playlist fallback "
                "for version %.2f\n", Event.EventVersion);
            return true;
        }
    ...
    bool CollectConnectedEventPlayers(UWorld* World, AFortGameMode* GameMode,
        std::vector<AFortPlayerControllerAthena*>& OutPlayers)
    {
        ...
        auto AddPlayer = [&](AFortPlayerControllerAthena* PlayerController)
            {
                if (!IsLiveEventObject(PlayerController) ||
                    !PlayerController->IsA(ControllerClass) || std::find(
                        OutPlayers.begin(), OutPlayers.end(), PlayerController) != OutPlayers.end())
                {
                    return;
                }
        ...
        auto AddConnection = [&](UNetConnection* Connection)
            {
                ...
                if (!IsSaneArray(Children.Data, Children.Num(), Children.Max(),
            ...
            IsLiveEventObject(World->NetDriver))
        {
            ...
            {
                auto& Connections = Driver->ClientConnections;
                if (IsSaneArray(Connections.Data, Connections.Num(), Connections.Max(),
                        sizeof(UNetConnection*), 256))
                {
        ...
        if (IsLiveEventObject(GameMode) && GameMode->HasAlivePlayers())
        {
            auto& AlivePlayers = GameMode->AlivePlayers;
            if (IsSaneArray(AlivePlayers.Data, AlivePlayers.Num(), AlivePlayers.Max(),
                    sizeof(AActor*), 256))
            {
                for (int32 PlayerIndex = 0;
                     PlayerIndex < AlivePlayers.Num(); ++PlayerIndex)
                {
                    AddPlayer(static_cast<AFortPlayerControllerAthena*>(AlivePlayers[PlayerIndex]));
                }
            }
        }
    ...
    float ResolveKiwiIslandSurfaceZ(AFortPlayerPawnAthena* FallbackPawn)
    {
        ...
        auto KiwiLevelScript = FindLiveActor(L"/Kiwi/Levels/Kiwi_P.Kiwi_P_C");
        FTransform FarmTransform{};
        if (IsLiveEventObject(KiwiLevelScript) && ReadEventProperty(KiwiLevelScript,
                "FarmPOITransform", CastClassStructProperty, FarmTransform) &&
            FPlatformMath::IsFinite(FarmTransform.Translation.Z) &&
            fabs(FarmTransform.Translation.Z) > 1.f)
        {
            GEventState.KiwiIslandSurfaceZ = FarmTransform.Translation.Z;
            return GEventState.KiwiIslandSurfaceZ;
        }

        return IsLiveEventObject(FallbackPawn) ? FallbackPawn->K2_GetActorLocation().Z : 0.f;
    }

    bool TickKiwiIslandVortex(const FEventContext& Context, double Now)
    {
        const FEvent* Event = UsesKiwi1750Compatibility(GEventState.Event) ? GEventState.Event
            : FindCurrentEvent();
        if (!UsesKiwi1750Compatibility(Event) || Now < GEventState.NextKiwiVortexUpdateTime)
        {
            return false;
        }
        GEventState.NextKiwiVortexUpdateTime = Now + 0.1;

        if (!PlaylistMatchesEvent(*Event, Context) || !IsLiveEventObject(Context.GameMode))
        {
            return false;
        }
        ...
        if (!CollectConnectedEventPlayers(Context.World, Context.GameMode, Players))
        {
            return false;
        }
        ...
        for (auto PlayerController : Players)
        {
            ...
            auto State = std::find_if(GEventState.KiwiVortexPlayers.begin(),
            ...
            if (State == GEventState.KiwiVortexPlayers.end())
            {
                ...
                NewState.IslandSurfaceZ = ResolveKiwiIslandSurfaceZ(Pawn);
            ...
            const bool bGrounded = IsLiveEventObject(Pawn->CharacterMovement) &&
                Pawn->CharacterMovement->IsMovingOnGround();
            if (!State->bActive && bGrounded && FPlatformMath::IsFinite(Location.Z))
            {
                State->IslandSurfaceZ = Location.Z;
                State->LastGroundedLocation = FVector(Location.X, Location.Y, Location.Z);
                State->bHasGroundedLocation = true;
            }
            const bool bFalling = IsLiveEventObject(Pawn->CharacterMovement) &&
                Pawn->CharacterMovement->IsFalling();
            const bool bDescending = bFalling && Pawn->CharacterMovement->Velocity.Z < -50.f;
            const double GroundDeltaX = Location.X - State->LastGroundedLocation.X;
            const double GroundDeltaY = Location.Y - State->LastGroundedLocation.Y;
            const bool bOutsideIslandFootprint = !State->bHasGroundedLocation ||
                GroundDeltaX * GroundDeltaX + GroundDeltaY * GroundDeltaY >= 250000.0;
            const bool bBelowIsland = Location.Z <= State->IslandSurfaceZ - 750.f;
            const bool bReachedLandingHeight = State->bActive &&
                IsLiveEventObject(Pawn->CharacterMovement) &&
                Pawn->CharacterMovement->Velocity.Z > 0.f &&
                Location.Z >= State->IslandSurfaceZ - 150.f;
            const bool bLandedOnIsland = bGrounded && Location.Z >= State->IslandSurfaceZ - 500.f;

            if (!State->bActive && bBelowIsland && bDescending && bOutsideIslandFootprint)
            {
                SetKiwiVortexActive(*State, Pawn, true);
            }
            else if (State->bActive && (bReachedLandingHeight || bLandedOnIsland))
    ...
    bool EquipKiwiBackpack(AFortPlayerControllerAthena* PlayerController,
        AFortPlayerPawnAthena* Pawn);

    bool TickKiwiPlayerSetup(const FEventContext& Context, double Now)
    {
        const FEvent* Event = UsesKiwi1750Compatibility(GEventState.Event) ? GEventState.Event
            : FindCurrentEvent();
        if (!UsesKiwi1750Compatibility(Event) || Now < GEventState.NextKiwiPlayerSetupTime)
        {
            return false;
        }
        ...
        if (!ResolveKiwiComponentClasses(ControllerComponentClass, PawnComponentClass))
        {
            if (Now >= GEventState.NextKiwiSetupLogTime)
            {
                GEventState.NextKiwiSetupLogTime = Now + 2.0;
                SDK::DbgLog("[Events] Kiwi player setup waiting for native "
        ...
        if (KiwiPhase == UnobservedPhase)
            KiwiPhase = GEventState.ObservedPhaseIndex;
        const bool bPrisonPhase = KiwiPhase != UnobservedPhase && KiwiPhase >= 1;
        if (bPrisonPhase && (!GEventState.KiwiLowGravityEffects.empty() ||
             ...
             PlayerIndex < Players.size(); ++PlayerIndex)
        {
            ...
            ++TargetPlayers;
            auto Pawn = PlayerController->MyFortPawn;
            const bool bHadControllerComponent = IsLiveEventObject(FindRiftTourComponent(
                    PlayerController, ControllerComponentClass));
            const bool bHadPawnComponent = IsLiveEventObject(FindRiftTourComponent(
                    Pawn, PawnComponentClass));
            if (!EnsureKiwiPlayerComponents(PlayerController, Pawn, ControllerComponentClass,
                    PawnComponentClass))
            {
                continue;
            }
            ...
            ++ConfiguredPlayers;
            EnsureKiwiPrisonStart(PlayerController, Pawn, ControllerComponentClass,
                static_cast<int32>(PlayerIndex), bPrisonPhase);
            if (bPrisonPhase)
            {
            ...
            if (!bHadControllerComponent || !bHadPawnComponent)
            {
                InvokeZeroed(Pawn, Pawn->GetFunction("ForceNetUpdate"));
                InvokeZeroed(PlayerController, PlayerController->GetFunction("ForceNetUpdate"));
                SDK::DbgLog("[Events] Kiwi player components restored "
                    "controller=%p pawn=%p controllerComponent=%d "
                    "pawnComponent=%d\n", static_cast<void*>(PlayerController),
                    static_cast<void*>(Pawn), bHadControllerComponent ? 0 : 1,
        ...
        AFortPlayerPawnAthena* Pawn)
    {
        ...
        if (!TryGetEventObjectIdentity(Pawn, PawnIdentity))
            return false;
        if (HasTrackedIdentity(GEventState.KiwiBackpackPlayers, PawnIdentity))
        {
            return true;
        }
        ...
        InvokeZeroed(Pawn, Pawn->GetFunction("OnCharacterPartsReinitialized"));
        if (IsLiveEventObject(PlayerController->PlayerState))
        {
            InvokeZeroed(PlayerController->PlayerState, PlayerController->PlayerState->GetFunction(
        ...
        SDK::DbgLog("[Events] Kiwi event backpack equipped "
            "controller=%p pawn=%p part=%p\n", static_cast<void*>(PlayerController),
            static_cast<void*>(Pawn), static_cast<void*>(Backpack));
        return true;
    }
    ...
    void PrepareKiwiPrisonPlayers(bool bPlaceInAuthoredChamber)
    {
        if (!UsesKiwi1750Compatibility(FindCurrentEvent()) ||
            !IsLiveEventObject(GEventState.World) || !IsLiveEventObject(GEventState.GameMode))
        {
            return;
        }

        std::vector<AFortPlayerControllerAthena*> Players;
        if (!CollectConnectedEventPlayers(GEventState.World, GEventState.GameMode, Players))
        {
            return;
        }
             ...
             PlayerIndex < Players.size(); ++PlayerIndex)
        {
            ...
            auto Pawn = PlayerController->MyFortPawn;
            EnsureKiwiPlayerComponents(PlayerController, Pawn, ControllerComponentClass,
                PawnComponentClass);
            EnsureKiwiPrisonStart(PlayerController, Pawn, ControllerComponentClass,
                static_cast<int32>(PlayerIndex), bPlaceInAuthoredChamber);
            EquipKiwiBackpack(PlayerController, Pawn);
        }
    }
    ...
    bool TryInvokeLoader(const FEvent& Event)
    {
        ...
        UObject* LoaderTarget = Actors.Loader ? static_cast<UObject*>(Actors.Loader)
            : static_cast<UObject*>(Actors.Script);
        auto LoaderFunction = const_cast<UFunction*>(FindObject<UFunction>(Event.LoaderFuncPath));
        return IsLiveEventObject(LoaderTarget) && InvokeContentLoader(LoaderTarget, LoaderFunction);
    }

    bool IsContentLoaderPrepared(const FEvent& Event, const FEventContext& Context)
    {
    ...
    bool AreEventClientsStreamingReady(const FEventContext& Context, bool& OutHadPredicate)
    {
        OutHadPredicate = false;
        if (!IsLiveEventObject(Context.GameMode) || !Context.GameMode->HasAlivePlayers())
        {
            return false;
        }
        ...
        {
            auto PlayerController = static_cast<AFortPlayerControllerAthena*>(Players[Index]);
            if (!IsLiveEventObject(PlayerController) || !PlayerController->IsA(ControllerClass))
            {
                continue;
            }
            ...
            bool bInvoked = InvokeBoolNoInput(PlayerController, StreamingReadyFunction,
                bPlayerReady);
            if (!bInvoked && NativeStreamingReadyAddress)
            {
                auto NativeStreamingReady = reinterpret_cast<
                    bool (*)(AFortPlayerControllerAthena*)>(NativeStreamingReadyAddress);
    ...
    bool IsContentLoaderReady(const FEventContext& Context, double Now)
    {
        if (Now < GPreparedLoaderReadyTime)
            return false;

        bool bHadStreamingPredicate = false;
        const bool bStreamingReady = AreEventClientsStreamingReady(Context, bHadStreamingPredicate);
        if (bHadStreamingPredicate)
            return bStreamingReady;

        // Some legacy builds do not expose the readiness UFunction; fall back to a timed window.
        return Now >= GPreparedLoaderFallbackTime;
    }

    bool ReadActivePhase(ASpecialEventScript* Script, int32& OutPhase)
    {
        if (!IsLiveEventObject(Script) || !Script->HasReplicatedActivePhaseIndex())
        {
            return false;
        }
    ...
    bool ResolveDataLayerStates(int32& OutUnloaded, int32& OutActivated)
    {
        ...
        if (Unloaded < 0 || Unloaded > 0xff || Activated < 0 || Activated > 0xff)
        {
            return false;
        }
    ...
    bool ApplyPhaseDataLayers(ASpecialEventScript* Script, int32 OldPhaseIndex, int32 NewPhaseIndex)
    {
        if (VersionInfo.FortniteVersion < 23.0 || !IsLiveEventObject(Script) ||
        ...
        if (!PhaseInfoStruct || !EntryStruct || !FPhaseInfo::HasDataLayers() ||
            !FPhaseDataLayerEntry::HasDataLayerAsset() || !FPhaseDataLayerEntry::HasbIsRecursive())
        {
            return false;
        }

        const int32 PhaseInfoSize = PhaseInfoStruct->GetPropertiesSize();
        const int32 EntrySize = EntryStruct->GetPropertiesSize();
        if (PhaseInfoSize <= 0 || PhaseInfoSize > 0x400 || EntrySize <= 0 || EntrySize > 0x100)
        {
            return false;
        }

        auto& Phases = Script->PhaseInfoArray;
        if (!IsSaneArray(Phases.Data, Phases.Num(), Phases.Max(), PhaseInfoSize, 128) ||
        ...
        if (!IsLiveEventObject(DataLayerManager) || !IsLiveEventObject(
                DataLayerManager->GetFunction("SetDataLayerRuntimeState")))
            return false;

        int32 UnloadedState = 0;
        int32 ActivatedState = 0;
        if (!ResolveDataLayerStates(UnloadedState, ActivatedState))
        {
            return false;
        }
        ...
        auto ApplyState = [&](int32 PhaseIndex, int32 RuntimeState)
        {
            ...
            if (!IsSaneArray(Layers.Data, Layers.Num(), Layers.Max(), EntrySize, 256))
            {
                return false;
            }
    ...
    void PrepareBattleRoyalePhase(const FEventContext& Context)
    {
        ...
        auto PhaseLogic = PhaseLogicClass ? UFortGameStateComponent_BattleRoyaleGamePhaseLogic::
                Get(Context.World) : nullptr;
        if (IsLiveEventObject(PhaseLogic))
        {
            const int32 PhaseOffset = PhaseLogic->GetOffset("GamePhase");
            const int32 StepOffset = PhaseLogic->GetOffset("GamePhaseStep");
            if (PhaseOffset >= 0 && StepOffset >= 0 && SDK::MemReadable(
                    (PBYTE)PhaseLogic + PhaseOffset, sizeof(uint8)) && SDK::MemReadable(
                    (PBYTE)PhaseLogic + StepOffset, sizeof(uint8)))
            {
                PhaseLogic->SetGamePhase(EAthenaGamePhase::SafeZones);
                PhaseLogic->SetGamePhaseStep(EAthenaGamePhaseStep::StormHolding);
                return;
            }
        }

        if (!IsLiveEventObject(Context.GameState) || !Context.GameState->HasGamePhase() ||
            !Context.GameState->HasGamePhaseStep())
        {
            return;
        }
        Context.GameState->GamePhase = static_cast<uint8>(EAthenaGamePhase::SafeZones);
        Context.GameState->GamePhaseStep = static_cast<uint8>(EAthenaGamePhaseStep::StormHolding);
        auto OnRep = Context.GameState->GetFunction("OnRep_GamePhase");
        InvokeZeroed(Context.GameState, OnRep);
        InvokeZeroed(Context.GameState, Context.GameState->GetFunction("ForceNetUpdate"));
    }

    void ShortenValidatedContentDelay(ASpecialEventScript* Script)
    {
        if (!IsLiveEventObject(Script) || !Script->HasDelayAfterConentLoad())
        {
            return;
        }
    ...
    bool WasMeshActorInitialized(const ASpecialEventScriptMeshActor* MeshActor)
    {
        FEventObjectIdentity Identity{};
        if (!TryGetEventObjectIdentity(MeshActor, Identity))
            return false;
        return HasTrackedIdentity(GEventState.InitializedMeshActors, Identity);
    }

    int32 PrepareMeshActors(bool bInitializeSingleRoot, bool bKeepClientNodeType)
    {
        const UClass* MeshClass = ASpecialEventScriptMeshActor::StaticClass();
        ...
        if (!IsSaneArray(MeshActors.Data, MeshActors.Num(), MeshActors.Max(),
        ...
        UObject* MeshNetworkSubsystem = const_cast<UObject*>(
        ...
        {
            const int32 NodeTypeOffset = MeshNetworkSubsystem->GetOffset("NodeType");
            const int32 ObjectSize = MeshNetworkSubsystem->Class->GetPropertiesSize();
            if (NodeTypeOffset >= 0 && NodeTypeOffset < ObjectSize)
            {
                auto Candidate = (PBYTE)MeshNetworkSubsystem + NodeTypeOffset;
        ...
        for (int32 Index = 0; Index < MeshActors.Num(); ++Index)
        {
            ...
            if (!IsLiveEventObject(MeshActor) || WasMeshActorInitialized(MeshActor))
                continue;
            if (InvokeZeroed(MeshActor, MeshActor->GetFunction("MeshRootStartEvent")))
            {
        ...
        for (auto MeshActor : RootInitializedActors)
        {
            if (!IsLiveEventObject(MeshActor))
                continue;
            const bool bRootReplicated = InvokeZeroed(MeshActor,
                MeshActor->GetFunction("OnRep_RootStartTime"));
            if (!bKeepClientNodeType)
            {
                InvokeZeroed(MeshActor, MeshActor->GetFunction("FlushNetDormancy"));
                InvokeZeroed(MeshActor, MeshActor->GetFunction("ForceNetUpdate"));
            }
            if (bRootReplicated)
            {
                FEventObjectIdentity Identity{};
        ...
        if (NodeTypeAddress)
        {
            *NodeTypeAddress = bKeepClientNodeType && InitializedActors > 0 ? 2 : SavedNodeType;
        }
        if (InitializedActors > 0)
        {
            printf("[Events] Initialized %d event mesh actor(s).\n", InitializedActors);
            SDK::DbgLog("[Events] mesh initialization actors=%d node=%u->%u "
                "single=%d keepClient=%d\n", InitializedActors,
                static_cast<unsigned>(SavedNodeType), static_cast<unsigned>(
                    NodeTypeAddress ? *NodeTypeAddress : SavedNodeType),
                bInitializeSingleRoot ? 1 : 0, bKeepClientNodeType ? 1 : 0);
        }
        MeshActors.Free();
    ...
    bool GrantEventActivator(AFortGameMode* GameMode)
    {
        ...
        const bool bEquipActivator = UsesRiftTour1730Compatibility(GEventState.Event);
        int32 TargetPlayers = 0;
             ...
             PlayerIndex < Players.Num(); ++PlayerIndex)
        {
            ...
            if (!TryGetEventObjectIdentity(PlayerController, PlayerIdentity))
            {
                continue;
            }
            ++TargetPlayers;
            const bool bGrantTracked = HasTrackedIdentity(GEventState.GrantedActivatorPlayers,
                PlayerIdentity);
            const bool bEquipTracked = HasTrackedIdentity(GEventState.EquippedActivatorPlayers,
                PlayerIdentity);

            const int32 InventoryOffset = PlayerController->GetOffset("WorldInventory");
            if (InventoryOffset < 0 || !SDK::MemReadable((PBYTE)PlayerController + InventoryOffset,
            ...
            if (!IsLiveEventObject(Inventory) || !Inventory->IsA(InventoryClass) ||
            ...
            if (!IsSaneArray(Entries.Data, Entries.Num(), Entries.Max(), EntrySize, 512))
            {
                continue;
            }
            ...
            if (!bAlreadyGranted)
            {
                if (bEquipActivator)
                {
                    ...
                    {
                        auto& Entry = Entries.Get(EntryIndex, EntrySize);
                        if (Entry.ItemDefinition && Entry.ItemDefinition != EventModeActivator &&
                            Entry.ItemDefinition->Cast<UFortWeaponMeleeItemDefinition>())
                        {
                    ...
                    if (bFoundMelee)
                    {
                        ...
                        SDK::DbgLog("[Events] removed Rift Tour melee before "
                            "activator controller=%p\n", static_cast<void*>(PlayerController));
                    }
                }

                auto GrantedItem = Inventory->GiveItem(EventModeActivator);
                if (!IsLiveEventObject(GrantedItem))
                    continue;
                ActivatorGuid = GrantedItem->ItemEntry.ItemGuid;
            }

            const bool bHasActivatorGuid = ActivatorGuid.A || ActivatorGuid.B ||
            ...
            if (!bGrantTracked)
            {
                GEventState.GrantedActivatorPlayers.push_back(PlayerIdentity);
            }

            if (bEquipActivator && (!bEquipTracked || !bAlreadyGranted))
            {
                if (!IsLiveEventObject(PlayerController->MyFortPawn) ||
                    !IsLiveEventObject(PlayerController->GetFunction(
                        "ServerExecuteInventoryItem")) ||
                    !IsLiveEventObject(PlayerController->GetFunction("ClientEquipItem")))
                {
                    continue;
                }
                PlayerController->ServerExecuteInventoryItem(ActivatorGuid);
                PlayerController->ClientEquipItem(ActivatorGuid, true);
                PlayerController->ForceNetUpdate();
                GEventState.EquippedActivatorPlayers.push_back(PlayerIdentity);
                SDK::DbgLog("[Events] equipped Rift Tour activator controller=%p "
                    "guid=%08x-%08x-%08x-%08x\n", static_cast<void*>(PlayerController),
                    static_cast<unsigned>(ActivatorGuid.A), static_cast<unsigned>(ActivatorGuid.B),
                    static_cast<unsigned>(ActivatorGuid.C), static_cast<unsigned>(ActivatorGuid.D));
            }
            ++GrantedPlayers;
        }
    ...
    bool CanInvokeEventFunction(UObject* Target, UFunction* Function, const wchar_t* FunctionPath,
        const FEventContext& Context)
    {
        if (!IsLiveEventObject(Target) || !IsLiveEventObject(Function))
        {
            return false;
        }
        ...
        {
            FOnReadyLayout Layout{};
            return Context.Playlist && ResolveOnReadyLayout(Function, Layout);
        }
    ...
    bool TryDispatchJerky1241(const FEvent& Event)
    {
        ...
        auto StartFunction = const_cast<UFunction*>(FindObject<UFunction>(StartFunctionPath));
        if (!IsLiveEventObject(Loader) || !IsLiveEventObject(StartFunction))
        {
            return false;
        }

        Loader->Call<void>(StartFunction, 0.f);
        SDK::DbgLog("[Events] dispatched legacy Jerky start target=%p function=%ls\n",
            static_cast<void*>(Loader), StartFunctionPath);
        return true;
    }

    bool TryDispatchEvent(const FEvent& Event, const FEventContext& Context)
    {
        ...
        for (const auto& EventFunction : Event.EventFunctions)
        {
            ...
            if (!CanInvokeEventFunction(Target, Function, EventFunction.FunctionPath, Context))
            {
                return false;
            }
        }
        ...
        for (const auto& EventFunction : Event.EventFunctions)
        {
            ...
            auto Function = const_cast<UFunction*>(
                FindObject<UFunction>(EventFunction.FunctionPath));
            SDK::DbgLog("[Events] dispatch begin version=%.2f target=%p function=%ls\n",
                Event.EventVersion, static_cast<void*>(Target), EventFunction.FunctionPath);
            bool bInvoked = false;
            ...
            {
                const bool bRiftTour1730 = UsesRiftTour1730Compatibility(&Event);
                PrepareBattleRoyalePhase(Context);
                ShortenValidatedContentDelay(static_cast<ASpecialEventScript*>(Target));
                const int32 InitializedMeshActors = PrepareMeshActors(bRiftTour1730, bRiftTour1730);
                GEventState.bMeshInitializationPending = !bRiftTour1730;

                auto Script = static_cast<ASpecialEventScript*>(Target);
                ...
                if (bRiftTour1730)
                {
                    ...
                    if (bInvoked)
                    {
                        ...
                        SDK::DbgLog("[Events] Rift Tour reference mesh sequence completed; "
                ...
                if (bInvoked && !bRiftTour1730)
                {
                    ...
                    {
                        const bool bLayersApplied = ApplyPhaseDataLayers(Script,
                            OldPhase == UnobservedPhase ? -1 : OldPhase, NewPhase);
                        GEventState.ObservedPhaseIndex = bLayersApplied ? NewPhase
                                : UnobservedPhase;
                    }
                }
            }
            ...
            if (!bInvoked)
            {
                SDK::DbgLog("[Events] dispatch failed version=%.2f function=%ls\n",
                    Event.EventVersion, EventFunction.FunctionPath);
                return false;
            }
            SDK::DbgLog("[Events] dispatch complete version=%.2f function=%ls\n",
                Event.EventVersion, EventFunction.FunctionPath);
        }
    ...
    {
        const bool bObserveRiftTour1730 = UsesRiftTour1730Compatibility(GEventState.Event);
        const bool bUsesModernDataLayers = VersionInfo.FortniteVersion >= 23.0;
        if (!GEventState.Event || (!bObserveRiftTour1730 && !bUsesModernDataLayers) ||
            Now < GEventState.NextPhasePollTime)
        {
            return;
        }
        GEventState.NextPhasePollTime = Now + (bUsesModernDataLayers ? 0.05 : 0.25);
            ...
            GEventState.ObservedPhaseIndex != CurrentPhase)
        {
            ...
            {
                SDK::DbgLog("[Events] Rift Tour phase changed old=%d new=%d\n",
                    GEventState.ObservedPhaseIndex, CurrentPhase);
            ...
            {
                const bool bLayersApplied = ApplyPhaseDataLayers(Script,
                    GEventState.ObservedPhaseIndex == UnobservedPhase ? -1
                        : GEventState.ObservedPhaseIndex, CurrentPhase);
                if (bLayersApplied)
                    GEventState.ObservedPhaseIndex = CurrentPhase;
            }
    ...
    {
        if (!GEventState.bMeshInitializationPending || Now < GEventState.NextMeshPollTime)
        {
            return;
        }
    ...
    {
        if (!GEventState.bActivatorGrantPending || Now < GEventState.NextActivatorPollTime)
        {
            return;
        }
    ...
    {
        if (!GEventState.bUnvaultPending || Now < GEventState.UnvaultDueTime || !GEventState.Event)
        {
            return;
        }
        ...
        if (!IsLiveEventObject(Script) || !ValidateParameterBuffer(SetUnvault) ||
...
void Events::PrepareEventContent()
{
    ...
    if (!Event || !Event->LoaderFuncPath || !ResolveEventContext(Context) ||
    ...
    if (TryInvokeLoader(*Event))
    {
        ...
        GPreparedLoaderReadyTime = UGameplayStatics::GetTimeSeconds(Context.World) + 2.0;
        GPreparedLoaderFallbackTime = GPreparedLoaderReadyTime + 6.0;
...
{
    GStartRequestWorld.store(GActiveEventWorld.load(std::memory_order_acquire),
...
bool Events::RequiresNativeStreamingReadiness()
{
    ...
    if (GEventState.World == World && (GEventState.Stage == EEventStartStage::LoadContent ||
         GEventState.Stage == EEventStartStage::WaitForLoader))
    {
        if (GPreparedLoaderWorld != World || GPreparedLoaderFallbackTime <= 0.0)
        {
            return true;
        }
    }
    ...
    return UGameplayStatics::GetTimeSeconds(World) <GPreparedLoaderFallbackTime;
}

bool Events::IsEventSessionActive()
{
    return GEventState.Event && GEventState.Stage != EEventStartStage::Idle;
}
...
void Events::Tick()
{
    ...
    if (GEventState.World != Context.World || GEventState.GameMode != Context.GameMode ||
        GEventState.GameState != Context.GameState)
    {
    ...
    if (FConfiguration::bAutoStartEvent.load(std::memory_order_acquire) &&
        !FConfiguration::bEventStarted.load(std::memory_order_acquire) &&
        !GEventState.bAutoRequestIssued && GEventState.Stage == EEventStartStage::Idle)
    {
        const float BaseTime = FConfiguration::EventStartBaseTime.load(std::memory_order_acquire);
        const float Delay = FConfiguration::EventStartTime.load(std::memory_order_acquire);
        if (BaseTime > 0.f && Now >= BaseTime + Delay)
        {
            GEventState.bAutoRequestIssued = true;
            GStartRequestWorld.store(Context.World, std::memory_order_release);
    ...
    if (GStartRequested.exchange(false, std::memory_order_acq_rel))
    {
        ...
        else
        {
            ...
            {
                const bool bNeedsLoader = GEventState.Event->LoaderFuncPath &&
                    !IsContentLoaderPrepared(*GEventState.Event, Context);
                if (bNeedsLoader)
                {
                ...
                {
                    GEventState.LoaderReadyTime = GPreparedLoaderReadyTime;
                    GEventState.Stage = EEventStartStage::WaitForLoader;
                }
                else
                {
    ...
    if (!GEventState.Event || GEventState.Stage == EEventStartStage::Idle ||
    ...
    if (GEventState.Stage == EEventStartStage::LoadContent)
    {
        ...
        GPreparedLoaderGameMode = Context.GameMode;
        GPreparedLoaderGameState = Context.GameState;
        GPreparedLoaderVersion = GEventState.Event->EventVersion;
    ...
    if (GEventState.Stage == EEventStartStage::WaitForTargets)
    {
        ...
        {
            LogWaiting(Now, "streamed event actors and reflected function schemas");
            GEventState.NextAttemptTime = Now + 0.25;
            return;
        }

        GEventState.bActivatorGrantPending = VersionInfo.FortniteVersion >= 16.0;
        GrantEventActivator(Context.GameMode);
        GEventState.Stage = EEventStartStage::Started;
        FConfiguration::bEventStarted.store(true, std::memory_order_release);
        SDK::DbgLog("[Events] started version=%.2f world=%p\n", GEventState.Event->EventVersion,
...
void (*ActivatePhaseAtIndexOG)(ASpecialEventScript* Script, int IndexToActivate);

void ActivatePhaseAtIndex(ASpecialEventScript* Script, int IndexToActivate)
{
    if (!IsLiveEventObject(Script))
        return;
    ...
    if (Script->HasReplicatedActivePhaseIndex())
    {
        ...
        if (SDK::MemReadable(PhaseAddress, sizeof(*PhaseAddress)))
        {
    ...
    const bool bOnRepInvoked = bPhaseCommitted && InvokeZeroed(Script,
        Script->GetFunction("OnRep_ReplicatedActivePhaseIndex"));

    SDK::DbgLog("[Events] Rift Tour authored phase dispatcher old=%d new=%d "
        "committed=%d onRep=%d\n", OldPhaseIndex, IndexToActivate, bPhaseCommitted ? 1 : 0,
        bOnRepInvoked ? 1 : 0);
    if (ActivatePhaseAtIndexOG)
        ActivatePhaseAtIndexOG(Script, IndexToActivate);
}

void (*ActivatePhaseOG)(ASpecialEventScript* Script, int IndexToActivate, float SequenceTimeOffset);

void ActivatePhase(ASpecialEventScript* Script, int IndexToActivate, float SequenceTimeOffset)
{
    if (!IsLiveEventObject(Script))
        return;
    ...
    const bool bHasNativeOldPhase = ReadActivePhase(Script, NativeOldPhaseIndex) &&
        NativeOldPhaseIndex >= 0;
    int32 OldPhaseIndex = bHasNativeOldPhase ? NativeOldPhaseIndex
        : GEventState.AuthoritativePhaseIndex;
    if (OldPhaseIndex == UnobservedPhase)
        OldPhaseIndex = -1;
    const bool bKiwiPrisonTransition = UsesKiwi1750Compatibility(FindCurrentEvent()) &&
    ...
    {
        SDK::DbgLog("[Events] legacy phase activation old=%d new=%d offset=%.3f\n", OldPhaseIndex,
            IndexToActivate, SequenceTimeOffset);
    }
    else
    {
        const bool bLayersApplied = ApplyPhaseDataLayers(Script, OldPhaseIndex, IndexToActivate);
        SDK::DbgLog("[Events] modern phase data layers old=%d new=%d applied=%d\n", OldPhaseIndex,
            IndexToActivate, bLayersApplied ? 1 : 0);
    }
        ...
        IndexToActivate == 5 && OldPhaseIndex != IndexToActivate)
    {
        ...
        SDK::DbgLog("[Events] Durian final-phase cue sent recipients=%d\n", RecipientCount);
    }

    if (ActivatePhaseOG)
    {
        ActivatePhaseOG(Script, IndexToActivate, SequenceTimeOffset);
        if (bKiwiPrisonTransition)
        ...
        {
            if (VersionInfo.FortniteVersion < 23.0 && NativePhaseIndex != IndexToActivate &&
                Script->HasReplicatedActivePhaseIndex())
            {
                ...
                {
                    *PhaseAddress = IndexToActivate;
                    const bool bOnRepInvoked = InvokeZeroed(Script, Script->GetFunction(
                            "OnRep_ReplicatedActivePhaseIndex"));
                    NativePhaseIndex = IndexToActivate;
                    SDK::DbgLog("[Events] legacy phase fallback committed "
                        "requested=%d onRep=%d\n", IndexToActivate, bOnRepInvoked ? 1 : 0);
                }
            }
            SDK::DbgLog("[Events] native phase activation completed requested=%d "
                "active=%d tracked=%d\n", IndexToActivate, NativePhaseIndex,
                GEventState.AuthoritativePhaseIndex);
        }
    }
}
...
void Events::Hook()
{
    ...
    if (fabs(VersionInfo.FortniteVersion - 27.11) < 0.001)
    {
        ...
        {
            Utils::Hook(ActivatePhaseAddress, ActivatePhase, ActivatePhaseOG);
            SDK::DbgLog("[Events] Durian ActivatePhase hook installed RVA=0x%llX\n",
                ActivatePhaseAddress - ImageBase);
        }
        else
        {
            SDK::DbgLog("[Events] Durian ActivatePhase hook target was not found\n");
        }
        return;
    }
    ...
    if (fabs(VersionInfo.FortniteVersion - 17.30) < 0.001)
    {
        ...
        const uint64 ActivatePhaseAtIndexAddress = ImageBase + ActivatePhaseAtIndexRva;
        if (MatchesEventCode(ActivatePhaseAtIndexAddress, ActivatePhaseAtIndexCode))
        {
            Utils::Hook(ActivatePhaseAtIndexAddress, ActivatePhaseAtIndex, ActivatePhaseAtIndexOG);
            SDK::DbgLog("[Events] Rift Tour authored phase dispatcher hook "
                "installed RVA=0x%llX\n", ActivatePhaseAtIndexRva);
            return;
        }
        SDK::DbgLog("[Events] Rift Tour authored phase dispatcher signature "
            "did not match; using legacy outer hook\n");
    }

    if (VersionInfo.FortniteVersion >= 17.0 && VersionInfo.FortniteVersion < 23.0)
    {
        const auto ActivatePhaseAddress = FindActivatePhase();
        if (ActivatePhaseAddress)
        {
            Utils::Hook(ActivatePhaseAddress, ActivatePhase, ActivatePhaseOG);
            SDK::DbgLog("[Events] legacy ActivatePhase hook installed RVA=0x%llX\n",
                ActivatePhaseAddress - ImageBase);
        }
        else
        {
            SDK::DbgLog("[Events] legacy ActivatePhase hook target was not found\n");
        }
        return;
    }
