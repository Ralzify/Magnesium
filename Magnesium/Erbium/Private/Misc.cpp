...
bool Misc::InstallPreStartSafeZoneTick()
{
    ...
    {
        SDK::DbgLog("[SafeZoneMap] pre-Start hook MH_Initialize failed: %s\n",
            MH_StatusToString(InitializeStatus));
        return false;
    }

    const auto CreateStatus = MH_CreateHook(reinterpret_cast<LPVOID>(Target),
        SafeZoneTickGetMaxTickRate, reinterpret_cast<LPVOID*>(&SafeZoneTickGetMaxTickRateOG));
    if (CreateStatus != MH_OK)
    {
        SDK::DbgLog("[SafeZoneMap] pre-Start hook MH_CreateHook failed: %s\n",
    ...
    {
        SDK::DbgLog("[SafeZoneMap] pre-Start hook MH_EnableHook failed: %s\n",
    ...
    SDK::DbgLog("[SafeZoneMap] pre-Start game-thread pump installed at +0x%llX\n",
        Target - Memcury::PE::GetModuleBase());
    return true;
}

float Misc::SafeZoneTickGetMaxTickRate(UEngine* Engine, float DeltaTime,
    bool bAllowFrameRateSmoothing)
{
    if (!bServerGetMaxTickRateActive.load(std::memory_order_acquire))
    {
        ...
        return SafeZoneTickGetMaxTickRateOG
            ...
            : FConfiguration::MaxTickRate.load(std::memory_order_relaxed);
    }
...
float Misc::GetMaxTickRate(UEngine* Engine, float DeltaTime, bool bAllowFrameRateSmoothing)
{
    if (auto World = UWorld::GetWorld(); !World || !World->NetDriver)
        GUI::SafeZoneMapGameTick();
    const float ConfiguredTickRate = FConfiguration::GetClampedMaxTickRate();
    if (VersionInfo.FortniteVersion >= 20.00)
    {
        auto World = UWorld::GetWorld();
        auto NetDriver = World ? static_cast<UNetDriver*>(World->NetDriver) : nullptr;
        if (NetDriver)
        {
            int32 NetworkTickRate = static_cast<int32>(ConfiguredTickRate);
...
void Misc::ApplyHomebaseEffectsOnPlayerSetup(__int64* a1, __int64 a2, __int64 a3, __int64 a4,
    UObject* a5, char a6, unsigned __int8 a7)
{
    ...
    for (auto i = 0ul; i < sizeOfImage - 5; ++i)
    {
        if (scanBytes[i] == 0xE8 || scanBytes[i] == 0xE9)
        {
            if (Memcury::PE::Address(&scanBytes[i]).RelativeOffset(1).GetAs<void*>() == (void*)AttemptDeriveFromURL)
            {
                ...
                {
                    const auto Base = i + j;
                    const auto Rex = scanBytes[Base];
                    const auto Opcode = scanBytes[Base + 1];
                    const auto ModRm = scanBytes[Base + 2];

                    // A 64-bit compare of World->NetDriver against zero: REX.W,
                    // then either the group-1 immediate form (80..83) or a
                    // register compare (38..3F), reaching a disp32 field at 0x38.
                    const bool bRexW = (Rex & 0xF8) == 0x48;
                    const bool bCompareImmediate = (Opcode & 0xFC) == 0x80;
                    const bool bCompareRegister = (Opcode & 0xF8) == 0x38;
                    const bool bMemoryOperand = (ModRm & 0xF0) != 0xC0 && (ModRm & 0xF0) != 0xE0 &&
                        ModRm != 0x65 && ModRm != 0xBB;
                    const bool bNetDriverOffset = scanBytes[Base + 3] == 0x38;
                    const bool bComparedToZero = !bCompareImmediate || scanBytes[Base + 4] == 0x0;

                    if (bRexW && (bCompareImmediate || bCompareRegister) &&
                        bMemoryOperand && bNetDriverOffset && bComparedToZero)
                    {
                        bool found = false;
                        for (auto k = 4; k < 0x104; k++)
                        {
                            ...
                            {
                                auto AfterBranch = __int64(&scanBytes[i + j + k + 5]);

                                if (*(uint32_t*)AfterBranch != 0xF0 &&
                                    (scanBytes[i + j + k + 4] != 0xC ||
                                        scanBytes[i + j + k + 5] != 0xB) &&
                                    scanBytes[i + j + k + 4] != 0x09)
                                {
                                    continue;
                                }
                            ...
                            {
                                auto BranchTarget = __int64(&scanBytes[i + j + k]);
                                BranchTarget = (BranchTarget + 2) + *(int8_t*)(BranchTarget + 1);

                                if (*(uint32_t*)(BranchTarget + 3) != 0xF0 &&
                                    (*(uint8_t*)(BranchTarget + 2) != 0xC ||
                                        *(uint8_t*)(BranchTarget + 3) != 0xB) &&
                                    *(uint8_t*)(BranchTarget + 2) != 0x09)
                                {
                                    continue;
                                }
                            ...
                            {
                                auto AfterBranch = __int64(&scanBytes[i + j + k + 9]);

                                if (*(uint32_t*)AfterBranch != 0xF0 &&
                                    (scanBytes[i + j + k + 8] != 0xC ||
                                        scanBytes[i + j + k + 9] != 0xB) &&
                                    scanBytes[i + j + k + 8] != 0x09)
                                {
                                    continue;
                                }
                            ...
                            {
                                auto BranchTarget = __int64(&scanBytes[i + j + k]);
                                BranchTarget = (BranchTarget + 6) + *(int32_t*)(BranchTarget + 2);
...
void (*TestOG)(AFortLightweightProjectileManager* ProjectileManager,
    TWeakObjectPtr<AFortPlayerPawnAthena> WeakPawn, TWeakObjectPtr<AFortWeapon> WeakWeapon,
    TSubclassOf<AFortLightweightProjectileConfig>& ConfigClass, FVector Location, FVector Direction,
    uint8_t Type, int a8, int a9);
void Test(AFortLightweightProjectileManager* ProjectileManager,
    TWeakObjectPtr<AFortPlayerPawnAthena> WeakPawn, TWeakObjectPtr<AFortWeapon> WeakWeapon,
    TSubclassOf<AFortLightweightProjectileConfig>& ConfigClass, FVector Location, FVector Direction,
    uint8_t Type, int a8, int a9)
{
...
template <typename T> std::pair<FName, T*> PickWeighted(UEAllocatedMap<FName, T*>& Map,
    float (*RandFunc)(float), bool bCheckZero = true)
{
...
void InitializeCosmeticLoadout(UFortAthenaAIBotCustomizationData* BotData,
    AFortPlayerPawnAthena* Pawn, FFortAthenaLoadout& OutLoadout,
...
void Misc::Hook()
{
    if (VersionInfo.FortniteVersion == 23.00 ||
        (VersionInfo.FortniteVersion >= 24.30 && VersionInfo.FortniteVersion != 28.30 && VersionInfo.FortniteVersion != 29.40) ||
    ...
    const uint64 GetMaxTickRateTarget = bSafeZoneTickHookInstalled ? SafeZoneTickHookTarget
    ...
    if (VersionInfo.EngineVersion < 4.20)
    {
        ...
        Utils::Hook(ApplyHomebaseEffectsOnPlayerSetupAddr, ApplyHomebaseEffectsOnPlayerSetup,
            ApplyHomebaseEffectsOnPlayerSetupOG);
    }
    ...
    auto PedestalBeginPlay = Memcury::Scanner::FindStringRef(
        L"AFortTeamMemberPedestal::BeginPlay - Begun play on pedestal %s", true, 0,
    ...
    if (VersionInfo.FortniteVersion >= 23)
    {
        ...
        // FN 30.00 is already UE 5.5, whose prologue is 40 53; scanning for 48 89 5C lands mid-function.
        const std::vector<uint8_t> FunctionPrologue = VersionInfo.EngineVersion < 5.5
                ? std::vector<uint8_t>{ 0x48, 0x89, 0x5C }
                : std::vector<uint8_t>{ 0x40, 0x53 };
        const uintptr_t PatternAddress = pattern.Get();
        auto FunctionStartScanner = pattern.ScanFor(FunctionPrologue, false);
        const uintptr_t FunctionStart = FunctionStartScanner.Get();
        auto PatchPointScanner = FunctionStartScanner.ScanFor({ 0x83, 0xF8, 0x02 });
        const uintptr_t PatchPoint = PatchPointScanner.Get();
        const bool bValidPatchPoint = PatternAddress && FunctionStart &&
            FunctionStart != PatternAddress && PatchPoint &&
        ...
        if (bValidPatchPoint)
            ...
            SDK::DbgLog("  [Misc] G! playercount patch validation failed "
                "pattern=%p start=%p candidate=%p - skipped\n",
                reinterpret_cast<void*>(PatternAddress), reinterpret_cast<void*>(FunctionStart),
